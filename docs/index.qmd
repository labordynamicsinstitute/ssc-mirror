...
See [mirror.repec.sh](https://github.com/labordynamicsinstitute/ssc-mirror/blob/main/mirror.repec.sh). The process can take up to 60 minutes. The downloaded mirror is in the [releases branch](https://github.com/labordynamicsinstitute/ssc-mirror/tree/releases).

## Date-stamped tags

Tags are created every time the archive is mirrored. See the [Tags page](tags.qmd) for an overview of all available tags, including contiguous date ranges.
...
- For bugs in the set of tags, see [ERRATA](https://github.com/labordynamicsinstitute/ssc-mirror/blob/main/ERRATA.md).
...
where the date `2021-12-21` corresponds to an existing tag (see [Tags page](tags.qmd)).

Also see [`ssc2`](https://github.com/labordynamicsinstitute/stata-ssc2/) for more streamlined functionality:
...
See the [Packages page](packages.qmd) for a searchable, dynamic table of all 6,000+ packages currently in the SSC archive.
...
The code that creates this repository is under a [BSD-3 license](https://github.com/labordynamicsinstitute/ssc-mirror/blob/main/LICENSE). All other code (the mirror) is under whatever license the original authors granted.
