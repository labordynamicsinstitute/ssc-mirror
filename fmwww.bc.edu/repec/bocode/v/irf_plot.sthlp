...
{p2colset 1 16 18 2}{...}
{p2col:{bf:[VAR-NR] irf_plot()} {hline 2}}plot impulse response(s) with confidence bands
{p_end}
{p2colreset}{...}
...
{p 8 12 2}
{it:void} 
{cmd:irf_plot(}{it:transmorphic scalar irs, struct irf_bands_struct scalar irb, struct var_struct scalar VAR, struct opt_struct scalar opts}{cmd:)}
...
{pstd}
{cmd:irf_plot()} plots the impulse responses to the structural shocks, {it:irs}, 
identified as specified in {it:opts} for the SVAR stored in {it:VAR}. It also 
plots confidence intervals, which are entered as {it:irb}.

{pstd}
If the SVAR was identified using short-run or long-run zero restrictions, 
{it:irs} should be filled with the output from {help irf_funct:irf_funct()} and 
{it:irb} should be filled with the ouptut from 
{help irf_bands_funct:irf_bands_funct()}.

{pstd}
If the SVAR was identified using [narrative] sign restrictions, {it:irs} should 
be filled with the object indexed by the key string "median" in the associative 
array output by {help sr_analysis_funct:sr_analysis_funct()} with the "irf" 
option specified. {it:irb} should be filled with the object indexed by "bands" 
...
{help irf_funct:irf_funct()} and {help irf_bands_funct:irf_bands_funct()} 
calculations also affect {cmd:irf_plot()}, so these should be kept 
...
responses should be plotted. If "all" is specified, {cmd:irf_plot()} will 
...
{pstd}
{cmd:irf_plot()} can be called in Stata using function 
{bf:{help var_nr_irf_plot:var_nr_irf_plot}}.
