...
{p2colset 1 16 18 2}{...}
{p2col:{bf:[VAR-NR] fevd_funct()} {hline 2}}Calculate forecast error variance decomposition of SVAR
{p_end}
{p2colreset}{...}
...
{cmd:fevd_funct(}{it:struct var_struct scalar VAR, struct opt_struct scalar opts}{cmd:)}
...
{p 4 4 2}
{cmd:fevd_funct(}{it:VAR,opts}{cmd:)} returns an associative array containing 
forecast error variance decompositions for the structual vector autoregression 
with elements contained in structure {it:VAR} using identification and other 
options specified in structure {it:opts}. If {it:VAR} and/or {it:opts} are 
empty or missing crucial elements, {cmd:fevd_funct()} will fail.

{pstd}
Options for identification and estimation are set to defaults when 
{help vnrs_var_nr:var_nr} is run. They are adjusted using 
...
{pstd}
{cmd:fevd_funct()} can be called in Stata using function 
{bf:{help var_nr_fevd:var_nr_fevd}}.
