*! brrtab.ado version 1.1  Nicholas Winter  15Nov2001
*
*

program define brrtab
...
$dbug di "brrcalc `vars' `wt' if `doit', type(`type') $S_VYopt b(`b') v(`V') brrw($S_BRRW) `opts' dof($S_VYdof)"
	brrcalc `vars' `wt' if `doit', type(`type') $S_VYopt /*
	*/ b(`b') v(`V') brrw($S_BRRW) `opts' dof($S_VYdof)
...
$dbug di "brrcalc `vt' `wt' if `doit', type(total) $S_VYopt v(`Vt') brrw($S_BRRW) `topts'"
		brrcalc `vt' `wt' if `doit', type(total) $S_VYopt /*
		*/ v(`Vt') brrw($S_BRRW) `topts'
	}
...
$dbug di "brrcalc `vs' `wt' if `doit', type(ratio) $S_VYopt b(`bs') v(`Vs') brrw($S_BRRW*)"
		brrcalc `vs' `wt' if `doit', type(ratio) $S_VYopt /*
		*/ b(`bs') v(`Vs') brrw($S_BRRW*)
	}
...
program define ValueLab
	...
	while `i' <= `dim' {
		...
		if `"`xlab'"'==`"`x'"' {
			...
			local xlab = trim(substr(`"`xlab'"',1,8))
...
/* Parse. */
	...
	syntax varlist(min=2 max=2 numeric) [pweight/] [if] [in] /*
	...
	*/    Level(int $S_level) FORmat(string) VERTical BRRWeight(string) dof(integer -1) /*
	*/    fay(string) ]
...
/* Get weights. */
	...
	unab junk : `brrweig'
	local nbrrw : word count `junk'
	global S_nbrrw=`nbrrw'

	global S_VYfay `fay'
	global S_VYopt "$S_VYopt fay(`fay')"
	global S_BRRW "`brrweights'"
	global S_VYbrr "`brrweights'"
	global S_VYnr  "`nbrrw'"
	global S_VYdof `dof'
...
program define Display
	...
	*/        COUnt CELl ROW COLumn SE CI DEFF DEFT OBS /*
	*/        noLABel noMARGinals PERcent PROPortion /*
	*/        Level(int $S_level) FORmat(string) VERTical ]
...
/* Display header. */
	...
	di
	di "{txt}Cross-tabulation with BRR-based standard errors"
	di
...
/* Preserve, then make data set for -tabdisp-. */
	...
	/* Make variable labels and value labels for stub variables. */
		...
		if "`label'"=="" {
			...
			if "`e(rowlab)'"!="" { MakeLab `e(rowvar)' `Row' }
			if "`e(collab)'"!="" { MakeLab `e(colvar)' `Col' }
		}

	/* Handle display of missing values if necessary. */

		if `Row'[1,e(r)]==. {
			FixMiss `e(rowvar)' `Row'[1,e(r)-1] /*
			*/ `ncat' `Nr1' `Nr2'
		}
		if `Col'[1,e(c)]==. {
			FixMiss `e(colvar)' `Col'[1,e(c)-1] /*
			*/ `ncat' `Nc1' `Nc2'
		}
	...
	*/
	...
	tabdisp `e(rowvar)' `e(colvar)', cell(`vars') `total' format(`format')
