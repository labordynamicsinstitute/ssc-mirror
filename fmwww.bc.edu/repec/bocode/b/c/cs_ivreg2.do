* ivreg2 cert script 1.0.21 CFB 14aug2009
...
g wt = 1/age
ivreg lw expr tenure rns smsa _I* (iq s = med kww mrt age) [aw=wt], robust
savedresults save ivaw e()
ivreg2 lw expr tenure rns smsa _I* (iq s = med kww mrt age) [aw=wt], small robust
savedresults comp ivaw e(), include(macros: instd depvar scalar: rmse matrix: b V) tol(1e-7) verbose
...
savedresults compare ivhacaw1 e(), include(macros: depvar scalar: N matrix: b V) tol(1e-6) verbose
...
savedresults compare ivhacaw2 e(), include(macros: depvar scalar: N matrix: b V) tol(1e-6) verbose
}

* test partial() with TS ops 
use http://fmwww.bc.edu/ec-p/data/wooldridge/phillips.dta, clear
tsset year,yearly
rcof "noi ivreg2 cinf (unem=L(1/2).unem) L.year, partial(L.year)" == 101
