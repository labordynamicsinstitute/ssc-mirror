...
{* 12May2026}{...}
{hi:help bumpline}{...}
{right:{browse "https://github.com/asjadnaqvi/stata-bumpline":bumpline v1.41 (GitHub)}}
...
{cmd:bumpline} {it:y x} {ifin} {weight}, {cmd:by}(varname)
		{cmd:[} {cmd:top}({it:num}) {cmdab:sel:ect(any|last)} {cmd:smooth}({it:num}) {cmd:palette}({it:str}) {cmd:colorby}({it:var}) {cmd:labcond}({it:str}) {cmd:offset}({it:num}) {cmd:xlabel}({it:str})
...
{p2coldent : {opt xlabel(str)}}Custom x-axis labels passed to {it:twoway}. If not specified, labels default to observed x-axis values and apply value labels when available (equivalent to {opt xlabel(values, valuelabel)}).{p_end}
...
{marker othercat}{dlgtab:Other categories}
...
{marker otherlines}{dlgtab:Other lines}
...
{marker othermarkers}{dlgtab:Other markers}
...
{marker otherlabels}{dlgtab:Other labels}
