...
{p2col:{cmd:babibplot} {hline 2} Plots two graph types which are rooted in Bland-Altman plots using journal and paper percentiles}
...
[{opt in}]
[{opt if}]
{opt , plot(scatter|bland)}

{marker overview}
{title:Overview}

{pstd}
{cmd:babibplot} plots paper and journal percentiles as two graph types which are both rooted in Bland-Altman plots (Bland and Altman, 1986). Bland-Altman plots are a standard instrument in the validation of clinical measurement. The graphs can be used in research evaluation to visualize the publication performance of single researchers. The graph types have been introduced by Bornmann and Haunschild (2017) who explain them in detail.{p_end}
...
{phang}Bland, J. M., & Altman, D. G. (1986). Statistical methods for assessing agreement between two methods of clinical measurement. {it:Lancet}, 1(8476), 307-310{p_end}
