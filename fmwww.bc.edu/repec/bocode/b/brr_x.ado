*! version 1.1  15nov2001
*
* TO DO:
*   make sure meff, etc all work correctly


program define brr_x
	version 6
...
$dbug di "/* Main call to _svy. */"
$dbug di "brrcalc $S_VYvl `wt' if `doit', type($S_VYcmd) $S_VYopt $S_VYbrr b(`b') v(`V') vsrs($S_VYdeff) `onopt'"
	brrcalc $S_VYvl `wt' if `doit', type($S_VYcmd) $S_VYopt $S_VYbrr b(`b') v(`V') vsrs($S_VYdeff) `onopt'
...
$dbug di "brrcalc $S_VYvl if `doit', type(`cmd') $S_VYsopt $S_VYbrr vsrs($S_VYmeff) `srssub'"
	brrcalc $S_VYvl if `doit', type(`cmd') $S_VYsopt $S_VYbrr vsrs($S_VYmeff) `srssub'
...
$dbug di "brrcalc `doit' `wt' if `doit', type(total) $S_VYopt $S_VYbrr vsrs(*)"
	brrcalc `doit' `wt' if `doit', type(total) $S_VYopt $S_VYbrr vsrs(*)
...
$dbug di "/* Main call to _svy. */"
$dbug di "cap brrcalc `vl' `wt' if `doit', type($S_VYcmd) $S_VYopt $S_VYbrr b(`b1') v(`V1') vsrs(`Vdeff1') `onopt'"
		cap brrcalc `vl' `wt' if `doit', type($S_VYcmd) $S_VYopt $S_VYbrr /*
...
$dbug di "/* Call for meff. */"
$dbug di "cap brrcalc `vl' if `doit', type(`meffcmd') vsrs(`Vmeff1') $S_VYbrr $S_VYsopt `srssub'"
		cap brrcalc `vl' if `doit', type(`meffcmd') vsrs(`Vmeff1') $S_VYbrr /*
		*/	$S_VYsopt `srssub'
...
/* Undocumented information for display. */
	...
	est local brr_wspec "$S_VYbsp"
	est local N_reps   $S_VYnrep

	est local cmd      svybrr$S_VYcmd
...
/* Parse. */
	...
	*/ Level(int $S_level) BRRWeights(string) CI DEFF DEFT MEFF MEFT OBS SIZE dof(int -1) fay(string) ]
...
/* Get weights. */
	...
	if "`brrweig'"=="" {				/* Abbreviated because this program is v.6.0 */
		local brrweig : char _dta[brrwspec]
		if "`brrweig'"=="" {
			di as error "Must specify BRR Weights with BRRWeight() option for first BRR command"
			error 198
		}
	}
	char define _dta[brrwspec] "`brrweig'"

	global S_VYbrr "brrw(`brrweig')"
	global S_VYbsp "`brrweig'"
	unab brrspec : `brrweig'
	global S_VYnrep : word count `brrspec'
	...
	global S_VYfay `fay'
	global S_VYdof `dof'
	global S_VYbrr "$S_VYbrr dof(`dof') fay(`fay')"
