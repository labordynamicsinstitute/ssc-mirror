{smcl}
{* *! version 1.4.0 09dec2020}{...}
{findalias asfradohelp}{...}
{title:batcher}

{phang}
{bf:batcher} {hline 2} Simple task paralleliser for Stata dofiles (Windows only)
...
{synopt:{opt tr:ackdelay(integer)}}seconds between start of dofiles and {it:completion tracking}{p_end}
{synopt:{opt be:tweendelay(integer)}}seconds between start of {it:each} dofile{p_end}
{synopt:{opt up:datedelay(integer)}}seconds between {it:updates} of the tracker{p_end}
{synopt:{opt max:parallel(integer)}}maximum concurrent instances{p_end}
...
{synopt:{opt notimestamp}}do not print timestamp on iteration start{p_end}
...
{syntab:Deprecated}
{synopt:{opt sl:eepduration(integer)}}seconds between start of dofiles and completion tracking{p_end}
{synoptline}
{p2colreset}{...}
...
{pstd}
{cmd:batcher} is a simple way to parallelise tasks in Stata (Windows only at the moment){p_end}
...
{phang}
{opt tr:ackdelay(string)} Personal preference. By default batcher waits 60 seconds to start the tracker, but on slow PCs this might cause issues, while for short programs this might be too long. The sleep duration is defined in seconds. Old name: {opt sl:eepduration(string)}.

{phang}
{opt be:tweendelay(string)} By default batcher waits 10 seconds before starting the next dofile, but on slower PCs or with many iterations this might cause issues. Then you should increase this delay. Defined in seconds.

{phang}
{opt up:datedelay(string)} By default the tracker updates every 30 seconds, which might not work for your setting. Defined in seconds.

{phang}
{opt max:parallel(string)} Limits the number of concurrent instances. If your number of iterations I exceeds maxparallel M, then batcher will run M instances and start the remaining I-M consecutively as instances finish. If you are using Stata-MP, then maxparallel is automatically set to the number of (logical) processors you have. You can overwrite this behaviour by setting maxparallel to 0. The underlying idea is that running more instances than you have processors generally causes instability and speed reductions.

{phang}
{opt notimestamp} Stop the program from printing timestamps as it starts instances.
...
{break} {cmd:. global examplePath = subinstr("`r(fn)'", "batcher.ado", "batcher_exampleDofile.do", .)}{p_end}
