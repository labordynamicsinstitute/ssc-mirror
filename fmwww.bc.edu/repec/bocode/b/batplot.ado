...
17Oct19  v1.14 Sorting out options on the xlabel
17Jul20 v1.15 Altered xlabel() to be the twoway default and I nolonger control it
23Mar21 v1.16 Need to allow rarea_options so that rarea() doesn't overwrite yline() option!
*/
...
opt[rarea() specifies options for the rarea part of the final plot.]
opt[star specifies other twoway options, for example titles and labels.]
...
/* Allow use on earlier versions of stata that have not been fully tested */
  ...
  syntax varlist (min=2 max=2) [if] [in] [,NOtrend INFO VALabel(varname) MOPTIONS(string asis) SHADING(numlist min=2 max=2) SCatter(string asis) NOGraph DP(int 2) RAREA(string asis) *]
  local gopt "`options'"
...
/* Find min and max to do shading */
  local i 1
  foreach var of varlist `varlist' {
    local var`i++' "`var'"
  }
  tempvar temp
  qui gen `temp'= (`var1'+`var2')/2
  qui su `temp'
  local shade "shade(`r(min)' `r(max)')"

/* NOW do the trend version */
  if "`valabel'"~="" local add "val(`valabel')"
  _calctrend `varlist', `notrend' `info' `add' `mopt' sc(`scatter') `glab' `shade' `nograph' dp(`dp') `gopt' rarea(`rarea')
...
prog def _calctrend,rclass
  syntax [varlist] [,NOTREND INFO VALabel(varname) MOPTIONS(string asis) SCatter(string asis) RAREA(string asis) shade(numlist) NOGraph DP(int 2) *]
  local xopt "`options'"
  ...
  if "`notrend'"~="" {
    ...
    if "`nograph'"=="" twoway (rarea `uy' `ly' `av', bc(gs13) sort `rarea') (scatter `diff' `av' `fopt', m(o) `scatteropt' `scatter' `moptions') (line `my' `av',lp(dash) sort  ) , ////
 legend(off) ytitle(`ytit') xtitle(`xtit') /*xlabel(`xmin' `xmax')*/ `xopt'
    ...
    if "`nograph'"==""  {
      twoway (rarea `uy' `ly' `av', bc(gs13) sort `rarea')(scatter `diff' `av' `fopt', `scatteropt' `scatter') /*
*/(line `y' `av', lp(dash) sort) , legend(off) ytitle(`ytit') xtitle(`xtit') `xopt'
    }
