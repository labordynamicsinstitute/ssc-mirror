*! version 1.2  15Nov2001
program define brr_disp  /* display results */
	version 6
...
program define Header /* display header */
	...
	*if "`psu'"    == "" { local psu    "<observations>" }
	...
	local cmd=substr("`e(cmd)'",4,.)
	di _n in gr "Survey `cmd' estimation, BRR variance method" _n
	...
	di in gr "Analysis weight:" _col(23) `"`exp'"'
		   ...
		   in ye %9.0f `e(N)' ;
	di in gr "Replicate weights:" _col(23) `"`e(brr_wspec)'"' 
		in gr _col(48)  "Population size" _col(68) "="
		   in ye %10.0g `e(N_pop)';
...
/* If here, we have subpops. */
	...
	if "`e(cmd)'"=="svymean" { local name "Mean" }
	else if "`e(cmd)'"=="svytotal" { local name "Total" }
	else if "`e(cmd)'"=="svyratio" { local name "Ratio" }
...
program define Body
	...
	`1' if `b'[1,`i'] < 1e7 {
	`1'	local body : di _s(2) %9.0g `b'[1,`i'] /*
	`1'	*/		_s(3) %9.0g sqrt(`V'[1,`i'])
	`1' }
	`1' else {
	`1'	local body : di _s(1) %10.0g `b'[1,`i'] /*
	`1'	*/		_s(3) %9.0g sqrt(`V'[1,`i'])
	`1' }
	`1' local s 3

	`2' local x : di _s(3) %9.0g `b'[1,`i']-`t'*sqrt(`V'[1,`i']) /*
	*/		 _s(3) %9.0g `b'[1,`i']+`t'*sqrt(`V'[1,`i'])
	`2' local body "`body'`x'"

	`3' local x : di _s(3) %9.0g cond(`Deff'[1,`i']!=0,`Deff'[1,`i'],.)
	`3' local body "`body'`x'"

	`4' local x : di _s(`s') %9.0g cond(`Deft'[1,`i']!=0,`Deft'[1,`i'],.)
	`4' local body "`body'`x'"
	`4' local s 3

	`5' local x : di _s(`s') %9.0g cond(`Meft'[1,`i']!=0,`Meft'[1,`i']^2,.)
	`5' local body "`body'`x'"
	`5' local s 3

	`6' local x : di _s(`s') %9.0g cond(`Meft'[1,`i']!=0,`Meft'[1,`i'],.)
	`6' local body "`body'`x'"
	`6' local s 3

	`7' local x : di _s(`s') %9.0g `Obs'[1,`i']
	`7' local body "`body'`x'"
	`7' local s 3

	`8' if `Size'[1,`i'] < 1e7 {
	`8'	local x : di _s(`s') %9.0g `Size'[1,`i']
	`8' }
	`8' else {
	`8'	local s = `s' - 1
	`8'	local x : di _s(`s') %10.0g `Size'[1,`i']
	`8' }
	`8' local body "`body'`x'"
