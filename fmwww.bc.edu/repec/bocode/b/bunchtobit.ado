				...
				*rescales predicted pdf to account for truncation
				...
				if missing("`nopic'") { 
					...
					*graph empirical and predicted distributions
					...
					twoway 
					(
						bar `den_y_i' `gridy' if `gridy' >= `xscalemin' & `gridy' <= `xscalemax', bstyle(histogram) barwidth(`binwidth')  title("", ring(1) pos(11) size(medlarge) color(black))
					)
					(
					...
					title("Bunching - Tobit", color(black))
...
// 				local ymin = r(min)
				...
				** define a gap between label
				...
				ytitle("Elasticity estimates and" "95 pct. confidence intervals", axis(1) size(vlarge)) 
				graphregion(margin(right) style(none) color(gs16)) 
				bgcolor(white)
