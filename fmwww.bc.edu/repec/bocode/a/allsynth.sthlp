...
{opt pval:ues}({it:string}) {opt bcor:rect}({it:string}) {opt gapfig:ure}({it:string}) {opt trans:form}({it:string}) {opt stacked}({it:string}) ]
...
{cmd:(2)} automated estimation of RMSPE-ranked {it:p}-values and placebo-variance {it:p}-values and 95% confidence intervals 
({browse "https://www.aeaweb.org/articles?id=10.1257/aer.20190159":Arkhangelsky et al. 2021}) based  on in-space placebo tests 
(for "classic" and "bias-corrected" specifications) in which treatment status is randomly permuted across untreated units 
...
(sometimes exactly) one of {cmd:A} or {cmd:B} must be specified.
...
Informative warnings are returned if the specified options need to be changed. Please thoroughly consult these instructions and examples if you experience difficulties.

{p 4 8 2}
{cmd:pvalues}({cmd:rmspe|variance)} automates estimation of in-space placebo gaps across the donor pool units, for the purpose of calculating {it:p}-values.
...
{p 8 8 2}Specifying the {cmd:pvalues()} option will greatly extend the run-time of {cmd:allsynth}, and so it is assumed that the results should be saved. Accordingly, the {cmd:keep}({it:file}) 
option must be specified if {cmd:pvalues()} is specified. The variables {it:RMSPE}, {it:RMSPE_rank}, and {it:p} (and their bias-corrected equivalents, if {cmd:bcorrect()} is also specified) will be 
merged into {it:file} if {cmd:pvalues}(rmspe) is specified. The variables {it:_Se}, {it:_Tstat}, {it:_Pval}, {it:LB_95}, and {it:UB_95} (and their bias-corrected equivalents, if {cmd:bcorrect()} 
is also specified) will be merged into {it:file} if {cmd:pvalues}(variance) is specified. In both cases the variable {it:S} will be saved as the number of sampled placebo averages.{p_end}
...
{it:filename_att.dta}. If the {cmd:allsynth} option {cmd:pvalues()} is specified, then the estimated average gaps, placebo gaps, and the {it:p}-values (and associated values) will be saved in {it:filename_att_distn.dta}.
...
population of possibilities. The default setting is 100. Note: if the {cmd:allsynth} option {cmd:pvalues}(variance) is specified, {cmd:sampleavgs()}
will be ignored and {cmd:allsynth} will automatically sample 1000 placebo average gaps.

{p 10 10 2}(8) {cmd:figure}({cmd:classic}|{cmd:bcorrect} [ {cmd:placebos ci lineback}{cmd:, save}({it:file}) [{cmd:, replace} ]) {it:twoway_options} ]) 
can be used to automatically generate plots of the trajectories of the estimated average gaps, average bias-corrected gaps, the 95% confidence intervals around either,
the set of placebo average gaps, and the set of average bias-corrected placebo gaps. Placebo gaps and/or 95% confidence intervals may {it:only} be plotted
...
{p 4 8 2}
Example 1 - Use {cmd:allsynth} exactly as you would use {cmd:synth} to reconstruct the estimate from the {cmd:synth} help file:{p_end}
...
Estimate the bias from inexact matching using elastic net regression:{p_end}
...
{phang}{stata allsynth cigsale beer(1984(1)1988) lnincome retprice age15to24 cigsale(1988) cigsale(1980) cigsale(1975), trunit(3) trperiod(1989) gapfig(classic placebos lineback) pvalues(rmspe) keep(smokingresults) rep}
...
{p 8 8 2}{stata local preds "beer(1984(1)1988) lnincome retprice age15to24 cigsale(1988) cigsale(1980) cigsale(1975)"}{p_end}

{p 4 8 2}
Now estimate, display, and save the average treatment effects (gaps) and placebo gaps:{p_end}
{phang}{stata allsynth cigsale `preds', trunit(3) trperiod(1989) gapfig(classic placebos lineback) pval(rmspe) trans(cigsale retprice, demean) keep(smokingresults) rep}

{p 8 8 2}
This example reproduces the estimation in Example 7 but with cigsale and retprice demeaned as {cmd:transform(cigsale retprice, demean)} is specified, and plotting the classic results.{p_end}
...
{p 4 8 2}
Estimate, display, and save the bias-corrected average treatment effects (gaps) and placebo gaps:{p_end}
{phang}{stata allsynth cigsale `preds', trunit(3) trperiod(1989) bcor(merge) gapfig(bcorrect placebos, title(Ex10) save(ex10, replace)) pval(rmspe) keep(smokingresults) rep}
...
{p 8 8 2}{stata local depvar_preds "emps_n10(1990) emps_n10(1991) emps_n10(1992) emps_n10(1993) emps_n10(1990(1)1994) emps_n44(1990(1)1994) pe_salary_n10(1990(1)1994) pe_salary_n44(1990(1)1994)"}{p_end}

{p 4 8 2}
Define the second local macro of predictor variables:{p_end}
{p 8 8 2}{stata local depvar_preds "`depvar_preds' emps_shr_n44(1990(1)1994) pop_t(1990(1)1994) unr(1990(1)1994) emr(1990(1)1994) lfp(1990(1)1994) prop_white(1990(1)1994)"}{p_end}

{p 4 8 2}
Define a local macro of the predictor variables we'd like to be normalized to their final pre-treatment values:{p_end}
{p 8 8 2}{stata local prednorm "pe_salary_n10 pe_salary_n44 emps_n10 emps_n44 emps_shr_n44 pop_t "}{p_end}
...
{p 4 4 2}
Next, define a macro with all non-{cmd:stacked()} specifications of {cmd:allsynth} to allow a focus on {cmd:stacked()}:{p_end}
{p 8 8 2}{stata local allsynth_specifications "`depvar_preds', transform(emps_n10 `prednorm', normalize) bcorrect(merge) keep(allsynth_walmart/employment, replace)"}{p_end}

{p 4 4 2}
And define a macro with the {cmd:donorcond()} and {cmd:donorif()} specifications of the {cmd:stacked()} option:{p_end}
...
{p 8 8 2}{stata local figure_options "figure(bcorrect placebos ci, save(allsynth_walmart/ex13, replace) yscale(range(-10 10)) ylab(-10(2)10) xtitle(Event year) xlabel(-5(1)5, tlcolor(black))))"}{p_end}

{p 4 4 2}
Next, define a macro with all the remaining elements of the {cmd:stacked()} option:{p_end}
{p 8 8 2}{stata local stacked_option "stacked(trunits(supercenter) trperiods(super_year), clear avgweights(pop90) balanced sampleavgs(1000) `donor_restrict' `figure_options')"}{p_end}
...
{p 4 8 2}
Arkhangelsky, D., Athey, S., Hirschberg, D. A., Imbens, G.W., and S. Wager, 2021. {it:American Economic Review}, 111(12): 4088-4118.
...
{p 8 8 2}
{cmd:Acknowledgement}: In addition to directly utilizing the {cmd:synth} package, the code for {cmd:allsynth} draws unapologetically from Jens Hainmueller's code from the original {cmd:synth} package.
