...
/*Update to incorporate inclusion of a column displaying the number of participants and number of events, if number of events is unavailable please use the aedots command instead which only includes columns of number of participants 
*/

*start   with dataset with a variable for number of events in each arm  and totals in each arm
cap prog drop aedots2
cap postclose ae_dot 
program define aedots2, rclass 
version 15.0
...
syntax varlist, n1(varname) n2(varname) events1(varname) events2(varname) tot1(varname) tot2(varname)  ///
   [  RISKDiff(real 0)    LOGoff(integer 0) ///
	 ...
	 leftlabsize(real 1) leftlabang(real 0) leftlabel(string) ///
	  rightxline(numlist)  rightxlinepat(string) rightxlinecol(string) rightdcolor(string) rightdotcol(string) rightdotsat(integer 60) ///
...
****Version 3 addition****
	events1 - number of events in trt group 
...
*************VERSION 2 UPDATED ERROR CHECKS*********************************************
*nummargin can only take values 0 or 1
if `nummargin' >1  | `nummargin'<0 {  
	display as error "nummargin can only take values 0 or 1"
	exit 7
}

if `nummargin'==1 {	
	if `margin'==0 | `event1pos'==0 | `event2pos'==0 | "`marginpos'"==""  {
	display as error "if event numbers to be displayed margin, event1pos, event2pos and marginpos options required"
	exit 7
}
}

if  `nummargin'==1 & "`marginpos'"!="" {
	if "`marginpos'"!="l" & "`marginpos'"!="r"  {
	display as error "option `marginpos' not allowed, can only take options l or r"
	exit 7
	}
	}	
...
local by "`1'"
	...
	if substr("`:type `by''" ,1,3) == "str" {
			...
			*local var with number of distinct AEs
			...
			postfile  ae_doti r1 eventn1 n_events1 N1 r2 eventn2 n_events2 N2 perc1 perc2 risk_diff seRD lowerRD upperRD n_events p_val log_p_val str20 event relrisk logRR stderrRR loglowerCIRR logupperCIRR lowerCIRR upperCIRR using `ae_doti_1'.dta, replace
		...
		**********RISK DIFFERENCE & P-VALUE******************
			...
			post ae_doti (`r1') (`n1') (`events1') (`tot1') (`r2') (`n2') (`events2') (`tot2') (`perc1') (`perc2') (`risk_diff') (`seRD') (`lowerRD') (`upperRD') (`events') (`p_val') (`log_p') (`e_name') (`rel_risk') (`logRR') (`seRR') (`log_lowerRR') (`log_upperRR') (`lowerRR') (`upperRR')  
			...
			postfile  ae_doti r1 eventn1 n_events1 N1 r2 eventn2 n_events2 N2 perc1 perc2 risk_diff seRD lowerRD upperRD n_events p_val log_p_val str20 event  eventnum relrisk logRR stderrRR loglowerCIRR logupperCIRR  lowerCIRR upperCIRR   using `ae_doti_1'.dta, replace
			...
			forvalues i=1(1)`count' {
				...
				*getting the string label for the event				
				...
				post ae_doti (`r1') (`n1') (`events1') (`tot1') (`r2') (`n2') (`events2') (`tot2') (`perc1') (`perc2') (`risk_diff') (`seRD') (`lowerRD') (`upperRD') (`events') (`p_val') (`log_p') ("`e_name'") (`e_name_num')   (`rel_risk') (`logRR') (`seRR') (`log_lowerRR') (`log_upperRR') (`lowerRR') (`upperRR') 
...
*Displaying log(relrisk)
if "`rightxline'" == "" & `riskdiff'==0 & `logoff'==0 {
		local xnum = 0
		}
		
*Rel risk with x-axis logged
...
*Plot of log(relrisk)	
if `legendrightyn' ==1 & "`legendright1'"=="" &  `riskdiff'==0 & `logoff'==0 {
	local legenddefscat1 = "log10(Relative risk)"
	}

if `legendleftyn' ==1 & "`legendright1'"!="" &  `riskdiff'==0 & `logoff'==0 {
	local legenddefscat1 = "`legendright1'"
	}
	
*Plot of relrisk	
if `legendrightyn' ==1 & "`legendright1'"=="" &  `riskdiff'==0 & `logoff'==1 {
	local legenddefscat1 = "Relative risk"
	}

if `legendleftyn' ==1 & "`legendright1'"!="" &  `riskdiff'==0 & `logoff'==1 {
	local legenddefscat1 = "`legendright1'"
	}
...
if `legendrightyn' ==1 & "`legendright1'"==""  & "`legendright2'"==""  &  `riskdiff'==0 & `logoff'==0 {
	local legenddefscat1 = "log10(Relative risk)"
	local legenddefscat2 = "95% CI"
	}
	
if `legendrightyn' ==1 & "`legendright1'"==""  & "`legendright2'"==""  &  `riskdiff'==0 & `logoff'==1 {
	local legenddefscat1 = "Relative risk"
	local legenddefscat2 = "95% CI"
	}
...
******VERSION 2 ADDITION***************		
****Adding an option to dispay the rel ris on the x-axis instead of the log(relrisk)	
