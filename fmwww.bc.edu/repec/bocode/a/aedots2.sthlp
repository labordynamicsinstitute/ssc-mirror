...
{p2colset 5 14 16 2}{...}
{p2col :{cmd:aedots2} {hline 2} Dot plot for summary level adverse event data with data table containing number of participants and number of events}{p_end}
{p2colreset}{...}
...
{p 8 13 2}
{cmdab:aedots2} {varname} {cmd:,} {opth n1(varname)} {opth n2(varname)} {opth event1(varname)} {opth event2(varname)}  {opth tot1(varname)} {opth tot2(varname)} [{it:options}]

{phang}
{bf:aedots2} requires summary data in long format with one row per event, where {varname} indicates the variable that contains the event name/identifier.
...
{p2coldent:* {opt event1(varname)}}variable indicating the number of events of the type specified in {varname} in the first treatment group (must be numeric){p_end}
{p2coldent:* {opt event2(varname)}}variable indicating the number of events of the type specified in {varname} in the second treatment group (must be numeric){p_end}
...
{synopt:{opt log:off(#)}} specify whether log relative risk or relative risk plotted; {cmd:logoff(0)} plots the log relative risk and {cmd:logoff(1)} plots the relative risk, default is {cmd:logoff(0)}{p_end}
...
{synopt:{opt rightxlabel(numlist)}}allows the user to specify the x-axis value labels on the plot on the right{p_end}
...
{synopt:{opt nummargin(#)}}add a margin of empty space to one side of the plot on the right; default is {cmd:nummargin(0)}. {cmd:nummargin(1)} is required to add space for inclusion of a data table of event frequencies.{p_end}
{synopt:{opt margin(#)}}set the percentage increase of space to be added when {cmd:nummargin(1)}; default is {cmd:margin(0)}. See {it:{help marginstyle}} for further details.{p_end}
...
{synoptline}
{p2colreset}{...}
{pstd}*   {cmd:n1}, {cmd:n2}, {cmd:event1}, {cmd:event2}, {cmd:tot1} and {cmd:tot2} are
required.{p_end}
...
{phang}
{cmd:aedots2} creates a dot plot for summary level AE data from a two-arm clinical trial. The dot plot produces two graphs plotted adjacent to each other. 
The first displays the incidence of each event by treatment group. 
The second displays either the relative risk with corresponding 95% CI or the risk difference with corresponding 95% CI. 

{phang}
An optional data table containing the number of participants with each event and the number of events for each treatment group can also be included. 

{phang}
In the event that information on the number of events is unavailable a data table that only contains columns of the number of participants can be produced using the {cmd:aedots} command. 
...
{phang}
{opt event1(varname)} variable indicating the number of events of type specified in {varname} in the first treatment group.  
{cmd:event1(varname)} is required and must be a numeric variable.

{phang}
{opt event2(varname)} variable indicating the number of events of type specified in {varname} in the second treatment group.  
{cmd:event2(varname)} is required and must be a numeric variable.
...
{phang}
{opt logoff(#)} specifies whether the relative risk and corresponding confidence
interval is plotted on the log scale or not;
{cmd:logoff(0)} plots the log relative risk and {cmd:logoff(1)} plots the relative risk.
{cmd:logoff(#)} can only take values 0 or 1. The default is {cmd:logoff(0)}.
...
{phang}
{opt rightxlabel(numlist)} specifies the x-axis values to label on the plot on the right of the overall plot.
If not specified default values automatically generated, apart from when {cmd:logoff(1)} the defaults are specified as 0.5, 1 and 2, but the user can still specify own list to override this.
...
{phang}
{opt nummargin(#)} adds a margin of empty space to one side of the plot on the right; default is {cmd:nummargin(0)}.
{cmd:nummargin(1)} is required to add space to the plot for inclusion of a data table of event frequencies.
{cmd:nummargin(0)} indicates no data table to be added to the plot.

{phang}
{opt marginpos(string)} indicates where to add the margin of empty space for inclusion of the data table of event frequencies.
Position is relative to the plot on the right such that {cmd:marginpos(l)} indicates that it will be placed to the left and
{cmd:marginpos(r)} indicates that it will be placed to the right.

{phang}
{opt margin(string)} sets the percentage increase of space to be added when {cmd:nummargin(1)} to accommodate the inclusion of the data table.
See {it:{help marginstyle}} for further details.

{phang}
{opt event1pos(#)} specifies x-axis position to place the column of event frequencies for the first treatment group. Note whether 
working on log scale or not when specifying this option.

{phang}
{opt event2pos(#)} specifies x-axis position to place the column of event frequencies for the second treatment group. Note whether 
...
{phang2}{help aedots2##general_remarks:General remarks}{p_end}
...
{pstd}
(10) In the event that information on the number of events is unavailable a data table that only contains columns of the number of participants can be produced using the {cmd:aedots} command. 
...
{phang2}{cmd:. use example_dot_summ2.dta}{p_end}

{pstd}
Dot plot {p_end}
{phang2}{cmd:. aedots2 ae_pt, n1(n1) n2(n2) tot1(total1) tot2(total2) events1(N1) events2(N2)}{p_end}
...
{phang2}{cmd:. use example_dot_summ2.dta, clear}{p_end}
{phang2}{cmd:. aedots2 ae_pt, n1(n1) n2(n2) tot1(total1) tot2(total2) events1(N1) events2(N2) leftsymb1(triangle) legendleft1(Placebo) legendleft2(Intervention)}{p_end}

{pstd}
Dot plot using the {cmd: riskdiff} option to plot risk difference instead of relative risk{p_end}
{phang2}{cmd:. use example_dot_summ2.dta, clear}{p_end}
{phang2}{cmd:. aedots2 ae_pt, n1(n1) n2(n2) tot1(total1) tot2(total2) events1(N1) events2(N2) riskdiff(1)}{p_end}

{pstd}
Dot plot using the {cmd:leftlabsize} option to change the size of the event labels{p_end}
{phang2}{cmd:. use example_dot_summ2.dta, clear}{p_end}
{phang2}{cmd:. aedots2 ae_pt,  n1(n1) n2(n2) tot1(total1) tot2(total2) events1(N1) events2(N2) leftlabsize(0.8)}{p_end}

{pstd}
Dot plot using the {cmd:rightxline} option to change the position of the vertical line on the plot on the right{p_end}
{phang2}{cmd:. use example_dot_summ2.dta, clear}{p_end}
{phang2}{cmd:. aedots2 ae_pt, n1(n1) n2(n2) tot1(total1) tot2(total2) events1(N1) events2(N2) rightxline(1.5)}{p_end}

{pstd}
Dot plot using the {cmd:leftplot} option to change the left plot x-axis label{p_end}
{phang2}{cmd:. use example_dot_summ2.dta, clear}{p_end}
{phang2}{cmd:. aedots2 ae_pt, n1(n1) n2(n2) tot1(total1) tot2(total2) events1(N1) events2(N2) leftxtitle(%)}{p_end}
...
{phang2}{cmd:. use example_dot_summ2.dta, clear}{p_end}
{phang2}{cmd:. aedots2 ae_pt, n1(n1) n2(n2) tot1(total1) tot2(total2) events1(N1) events2(N2) brightmargin(2) trightmargin(2)}{p_end}

{pstd}
Dot plot using the {cmd:legendleftyn} option and the {cmd:legendrightyn} option to remove legends from both plots{p_end}
{phang2}{cmd:. use example_dot_summ2.dta, clear}{p_end}
{phang2}{cmd:. aedots2 ae_pt, n1(n1) n2(n2) tot1(total1) tot2(total2) events1(N1) events2(N2) legendleftyn(0)  legendrightyn(0)}{p_end}

{pstd}
Saving the final graph with filename {it:dot_example}{cmd:.gph}{p_end} 
{phang2}{cmd:. use example_dot_summ2.dta, clear}{p_end}
{phang2}{cmd:. aedots2 ae_pt, n1(n1) n2(n2) tot1(total1) tot2(total2) events1(N1) events2(N2) graphsave(dot_example)}{p_end}

{pstd}
Dot plot using the {cmd: logoff} option to plot relative risks instead of the log relative risks{p_end} 
{phang2}{cmd:. use example_dot_summ2.dta, clear}{p_end}
{phang2}{cmd:. aedots2 ae_pt, n1(n1) n2(n2) tot1(total1) tot2(total2) events1(N1) events2(N2) logoff(1)}{p_end}

{pstd}
Dot plot using the {cmd: logoff} option to plot relative risks instead of the log relative risks and labelling the x-axis using {cmd: rightxlabel}{p_end} 
{phang2}{cmd:. use example_dot_summ2.dta, clear}{p_end}
{phang2}{cmd:. aedots2 ae_pt, n1(n1) n2(n2) tot1(total1) tot2(total2) events1(N1) events2(N2) logoff(1) rightxlabel(0.01 0.03 0.1 0.5 0 1 5 10 20 50 150)}{p_end}
...
{phang2}{cmd:. use example_dot_summ2.dta, clear}{p_end}
{phang2}{cmd:. aedots2 ae_pt, n1(n1) n2(n2) tot1(total1) tot2(total2) events1(N1) events2(N2) nummargin(1) margin(50) event1pos(8) event2pos(12) marginpos(r)}{p_end}

{pstd}
Dot plot using the options to include a data table of event frequencies on the far right of the plot and adjusting column label heights to prevent overlap {p_end} 
{phang2}{cmd:. use example_dot_summ2.dta, clear}{p_end}
{phang2}{cmd:. aedots2 ae_pt, n1(n1) n2(n2) tot1(total1) tot2(total2) events1(N1) events2(N2) nummargin(1) margin(50) event1pos(8) event2pos(12) marginpos(r) event1posy(-1) event2posy(0.2)}{p_end}

{pstd}
Dot plot using the {cmd: nummargin}, {cmd: margin}, {cmd: marginpos}, {cmd: event1pos}, {cmd: event2pos}, {cmd: event1posy} and {cmd: event2posy} options to include a data table in the centre of the plot and adjusting position of the column titles{p_end} 
{phang2}{cmd:. use example_dot_summ2.dta, clear}{p_end}
{phang2}{cmd:. aedots2 ae_pt, n1(n1) n2(n2) tot1(total1) tot2(total2) events1(N1) events2(N2) nummargin(1) margin(50)  event1pos(-10) event2pos(-7) marginpos(l) event1posy(-1) event2posy(0.2)}{p_end}
...
{marker R2020}{...}
{phang}
Cornelius, V., Cro, S. & Phillips, R. 2020. Advantages of visualisations to evaluate and communicate adverse event information in RCTs. 
