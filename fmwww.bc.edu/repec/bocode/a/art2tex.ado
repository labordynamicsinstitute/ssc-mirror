...
program define art2tex
    ...
    if "`language'" == "chinese" {
        local sections "引言 文献综述 理论分析 研究设计 实证检验 结论建议"
    }
    else {
        local sections "Introduction LiteratureReview TheoreticalAnalysis ResearchDesign EmpiricalAnalysis Conclusion"
    }
    ...
    if "`language'" == "chinese" {
        ...
        di as text "3. 以BibTeX格式添加参考文献到references.bib"
        di as text "4. 使用以下顺序编译: xelatex -> bibtex -> xelatex -> xelatex"
        ...
        di as text "3. Add references to references.bib in BibTeX format"
        di as text "4. Compile using: xelatex -> bibtex -> xelatex -> xelatex"
