...
{p 6 16 2}
{cmd:aaniv} [{vars}] {ifin} 
[{cmd:,} {cmd:deltase} {cmd:ivreg2} {cmdab:ivo:ptions}{cmd:(}{it:options for IV regression}{cmd:)}
{it:{help sureg##options for SUR}}]
...
{p}The following example offers a comparison of standard IV/2SLS (using {help ivregress}) and the unbiased IV implemented in {help aaniv}.{p_end}
...
{p 6 16 2}{stata "qui ivregress 2sls lw `x' (educ=nearc4), noid " : ivregress 2sls lw `x' (educ=nearc4), noid }{p_end}
...
by necessity incorrect and dramatically understate the true variance. Nevertheless, simulations show that inference is quite good in practice, using either approximation.

{phang}
{bf:ivreg2}; by default, the standard IV/2SLS standard error is computed using {help ivregress}. 
To use the user-written {help ivreg2} instead, specify the option {cmd:ivreg2}.

{phang}
{bf:ivoptions}; any options to pass to {help ivregress}, or it the option {cmd:ivreg2} is also specified, to {help ivreg2}.
