...
{phang}
{bf:attregtest} {hline 2} implements the regression-based attrition tests proposed in {help attregtest##GHO2022: Ghanem et al. (2022)}.
...
only be used if the goal is to conduct the attrition tests pooling all the follow-up waves.{p_end}
...
{pstd}
{cmd:attregtest} implements the two regression-based attrition tests proposed in {help attregtest##GHO2022: Ghanem et al. (2022)}. The first test is based on the 
...
of the identifying assumption of internal validity for the study population (IV-P). These tests can be implemented for completely 
or cluster randomized experiments as well as experiments with stratified randomization.{p_end}
...
{pstd} 
If the experiment is stratified, the option {opt stratavar(varname)} must be specified. In this case, the mean testable restrictions of the
...
implications of the mean testable restrictions:{p_end}
...
the regression-based test using strata fixed effects only and two sets of p-values. The first set of p-values corresponds to the
mean testable restrictions (i.e., fully saturated). The second set, on the other hand, 
corresponds to the null hypotheses that test implications of these restrictions (i.e., strata fixed effects only).{p_end}


{pstd} 
For more details on the regression-based tests, see Section B in {help attregtest##GHO2022: Ghanem et al. (2022)}.{p_end} 
...
{marker GHO2022}{...}
{phang}
Ghanem, Dalia, Hirshleifer, Sarojini, Ortiz-Becerra, Karen (2022): Testing Attrition Bias in Field Experiments. CEGA Working Paper Series No. WPS-113. Center for Effective Global Action. University of California, Berkeley. 
{p_end}
