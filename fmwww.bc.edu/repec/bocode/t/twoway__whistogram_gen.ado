*! 1.1.0 NJC 28mar2024 
*! 1.0.0 NJC 19mar2024 
*! twoway__histogram_gen version 1.1.16  07nov2022
// generate heights and bin centers for a histogram
program twoway__whistogram_gen, rclass sortpreserve
        version 8.0

        syntax varname(numeric) [aw iw] [if] [in]  ///
        [,                                      ///
                DISCrete                        ///
                BINs(numlist max=1 >0 integer)  /// number of bins
        ...
        if `"`discrete'"' != "" & `"`bins'"' != "" {
                di as error "options discrete and bins() may not be combined"
                exit 198
        }
        ...
        if `"`bins'"' != "" & `"`width'"' != "" {
                di as err "options bins() and width() may not be combined"
                exit 198
        }
        ...
        // note: frequency not allowed with aweights 
		if "`weight'" == "aweight" & "`frequency'" != "" {
			di as err "frequency option not allowed with aweights"
			exit 198 
		}
        ...
        if "`weight'" == "iweight" scalar `nobs' = r(sum_w)     
		else scalar `nobs' = r(N)
