*! tc_adlbo.ado -- ADL-BO (Li & Lee 2010)
program define tc_adlbo, rclass
    version 14
    ...
    mata: tc_run_adlbo("`depvar'", "`indepvars'", "`touse'", `maxlag', `trim', `ngrid', "`case'", "`stat'", "`thr'", "`cv'")

    di
    di as text "{hline 72}"
    di as result "  ADL-BO test (Li & Lee 2010)"
    di as text "{hline 72}"
    di as text "  sup-Wald statistic       : " as result %10.4f scalar(`stat')
