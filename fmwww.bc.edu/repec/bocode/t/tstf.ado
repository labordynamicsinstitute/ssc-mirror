*! v.1.0.2 28oct21 N.Orsini fixing changes occurred in some R packages
...
if _rc != 0 {
	...
	qui gen snames = names
	qui replace snames = "_cons" if names == "intercept"
	...
	*/
	...
	qui forv i = 1/`r(N)' {
		...
		else local eqnames "`eqnames' ARIMA"
		mat `b'[1,`i'] = `=beta[`i']'
...
// Display results 
		...
		if ("`tabulate'" != "") | ("`grdata'" != "") | ("`greffect'" != "")  {
				...
				*/
				...
				if "`smooth'" != "" |  "`decay'" != "" {
					mat `E' = `b'[1, "ARIMA:delta".."ARIMA:omega"]
					mat `VE' = `V'["ARIMA:delta".."ARIMA:omega", "ARIMA:delta".."ARIMA:omega"]
				}

				if "`step'" != "" |  "`pulse'" != "" {
					mat `E' = `b'[1, "ARIMA:omega"]
					mat `VE' = `V'["ARIMA:omega", "ARIMA:omega"]
				}
...
tstf lograte if inrange(time, 625, 691), smooth int(676) arima(1,1,1) ///
sarima(1,0,0,12) tabulate grdata eform ///
ytitle("Calling rates per 100,000 smokers") ///
xlabel(625(6)691, angle(45)) xmtick(625(1)691) name(figure2a, replace)
