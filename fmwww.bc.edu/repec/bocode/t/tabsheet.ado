...
Changelog:
v1.0.1:
	Changed all instances of "svy linearized" to "svy"
		(this fixes a bug that would have occurred for variance estimates if users intended to use replicate methods [e.g., bootstrap, BRR, jackknife, SDR])
*/
...
foreach var of varlist `varlist' {
	...
	foreach spvar of varlist `subpops' {
		...
		if(`countsum'==0) {
			...
			if("`overse'"=="") {
				qui: svy: proportion `var' if(`touse'), subpop(`spvar')
				...
				qui: svy: proportion `var' if(`touse'), over(`spvar')
				matrix `tempmat_b' = e(b)
...
foreach var of varlist `varlist' {
	...
	foreach spvar of varlist `subpops' {
		...
		if(`countObs'==0) {
			...
			if("`overse'"=="") {
				qui: svy: mean `vp`var'' if(`touse'), subpop(`spvar')
				...
				qui: svy: mean `vp`var'' if(`touse'), over(`spvar')
				matrix `tempmat_e' = e(b)
