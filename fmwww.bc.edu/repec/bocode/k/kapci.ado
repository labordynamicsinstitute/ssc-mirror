...
program define kapci, rclass
    ...
    if `npos'==2 { // two raters
        ...
        * calculate ci for kappa, following Donner & Eliasziw (1992)
        ...
        local lb=sqrt((1/9)*(`y3')^2)*(cos((`theta'+2*_pi)/3)+sqrt(3)*sin((`theta'+2*_pi)/3))-`y3'/3
		local ub=2*sqrt((1/9)*(`y3')^2-(1/3)*`y2')*(cos((`theta'+5*_pi)/3))-`y3'/3
        * output results
        if "`wilson'"!="" {
