{smcl}
{* *! Version 1.0.4 January 31 2023 }{...}

{viewerjumpto "Syntax" "winratiotest##syntax"}{...}
{viewerjumpto "Description" "winratiotest##description"}{...}
{viewerjumpto "Options" "winratiotest##options"}{...}
{viewerjumpto "Examples" "winratiotest##examples"}{...}
{viewerjumpto "Saved results" "winratiotest##savedresults"}{...}
{viewerjumpto "References" "winratiotest##references"}{...}
{viewerjumpto "Authors" "winratiotest##authors"}{...}


{cmd: help winratiotest}
{hline}

{title:Title}

{phang}
{bf:winratiotest} {hline 2} calculates the unmatched Win Ratio for prioritised outcomes.

{marker syntax}{...}
{title:Syntax}

{phang}
{cmdab:winratio:test} {it:idvar} {it:trtvar} {cmd:,}
...
{pstd}
{cmd:winratiotest} The win ratio was introduced in 2012 by Pocock {it:et al} {cmd:[1]} as a novel approach to the analysis of composite endpoints in randomised clinical trials.
...
when # is not specified the default margin is 0. Note that if a non-integer margin is used then the variables concerned must be stored using double precision. 
...
{pstd}
The following examples use the dataset {cmd: winratiotest_example.dta} which is provided along with the program files. 
...
{pstd}
{cmd: use winratiotest_example.dta}

{pstd}
{cmd: winratiotest patid trt , outcomes(dth tf fudth) outcomes(hf r4 fuhf)}
...
{pstd}
{cmd: winratiotest patid trt , outcomes(dth tf fudth) outcomes(hf r4 fuhf) outcomes(qol c >) }
...
{pstd}
{cmd: winratiotest patid trt , outcomes(dth tf fudth) outcomes(hf r4 fuhf) outcomes(qol c >0.1)}

{pstd}
Notice that there is no gap between > and 0.1. When a non integer margin is used then the variable(s) concerned must be stored using double precision. 
...
{pstd}
{cmd: winratiotest patid trt , outcomes(dth tf fudth) outcomes(hf r4 fuhf) strata(blvar) stweight(mh) }
...
{pstd}
{cmd: winratiotest patid trt , outcomes(dth tf fudth) outcomes(hf r4 fuhf) outcomes(qol_bin c >)}

{marker savedresults}{...}
{title:Saved results}

{pstd}
{cmd:winratiotest} stores the following in {cmd:r()}:
