...
local nouts=wordcount("`outcomes'")
if `nouts'>3 {
	di in r "The outcomes() options can only include 3 elements since Version 1.0.3. See help winratiotest."
	exit 198
	}

* Check elements in `outcomes' is not <3
if `nouts'<3 {
	di in r "The outcomes() option must contain 3 elements. See help winratiotest."
	exit 198
	}
...
* Check there are 3 elements in `outcomes'
local nouts=wordcount("`outcome`i''")
if `nouts'>3 {
	di in r "The outcomes() options can only include 3 elements since Version 1.0.3. See help winratiotest."
	exit 198
	}
...
forvalues i=1/`comp_n' {
		...
		if "`r(marginwarning)'"=="1" {
		di in r "When using non integer margins the variables concerned must be stored using double precision." 
		continue , break
		}		
	
		if "`r(Xcomp)'"=="1" {
		di in r "Variable X_comp is reserved for calculation of win ratio."
		continue , break
		}
	
	}
	
		if "`r(errortype)'"=="1" | "`r(errorvar)'"!="" | "`r(errorrepvar)'"!="" | "`r(errortdvar)'"=="1" | "`r(errormargin)'"=="1" | "`r(marginwarning)'"=="1" | "`r(Xcomp)'"=="1" {
		exit  198
		}		
