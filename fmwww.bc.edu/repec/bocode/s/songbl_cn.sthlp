...
{synoptset 30 }{...}
{cmd:     Stata 交流群微信：{browse "https://note.youdao.com/ynoteshare1/index.html?id=720635d3824de83e0e764a60eb34e54c&type=note":songbl_stata}}
...
{cmd:    上传Stata资源到Songbl数据库的链接：{browse "https://public.zsxq.com/groups/28855842182811.html":https://public.zsxq.com/groups/28855842182811.html}}
{p2colreset}{...}
{hline}
...
{p2colset 5 16 16 2}{...}
{p2col:{hi:songbl} {hline 2}}在 Stata 命令窗口中对微信公众号、爬虫俱乐部、连享会网站等推文的关键词检索与结果输出。{p_end}
...
{p_end}
{synopt:{cmdab:f:orum}}
帖子资源。检索 The Stata Forum 帖子资源，并输出帖子超链接。
{p_end}
{synopt:{cmdab:p:aper}}
...
{pstd}检索出现面板门槛效应 xthreg 命令的代码

{phang2}. {stata "songbl xthreg,cie"}{p_end}

{pstd}检索出现 reghdfe 命令的代码

{phang2}. {stata "songbl reghdfe,cie"}{p_end}

{pstd}呈现详细的检索结果(以 reghdfe 命令为例子).包括在哪篇论文出现、在do文档哪一行、详细内容

{phang2}. {stata "songbl reghdfe,cie save(txt)"}{p_end}
...
{phang2}. {stata "songbl did,f"}{p_end}
...
{pstd}输出 The Stata Forum 最新帖子{p_end}

{phang2}. {stata "songbl new,f"}{p_end} 
 
{pstd}输出 The Stata Forum 包含 DID 的帖子超链接{p_end}

{phang2}. {stata "songbl DID,f"}{p_end} 

{pstd}输出 The Stata Forum 同时包含 [IV DID Reg Stata ] 的帖子超链接{p_end}

{phang2}. {stata "songbl iv did reg stata,f"}{p_end} 
...
{phang2}. {stata "cd `c(sysdir_plus)'"}{p_end}
{phang2}. {stata "songbl ,dir noc"}{p_end}
