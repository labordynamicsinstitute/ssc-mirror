...
quietly {
        ...
        syntax varlist(numeric fv) [if] [in], INTervention(varlist numeric fv max=1) [, THReshold(numlist) SEPCHains DIAGnostics noIsily SAVE Cond(string) UNCond(string) *] 
        ...
        noi bayes, saving(mcmcUnc, replace) `isily' `unc_options': reg `depvar'
