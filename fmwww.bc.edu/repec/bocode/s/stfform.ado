...
syntax [varlist (numeric default=none) ] [, NSIMulation(integer 1000) noGraph nplot(integer 20) SAVing(string asis) noxb nolog novars graphnames(name) ]
version 16 
st_is 2 analysis
...
if (c(maxvar)<`nsimulation') {
	di as text "WARNING:" _n "The desired number of simulations ({bf:nsimulation}=`nsimulation') exceeds current {bf: maxvar} (`c(maxvar)')." _n "The maximum {bf:nsimulation} is {bf: maxvar}-3. Overriding graphs and {bf: saving}"	
	local saving
	local graph nograph
}
...
if (`nplot'>20) {	
	...
	foreach c of local ncalls {
		...
		local call `call' (line `pp' z, lpattern(dot ..) lcolor(gs7 ..))
	}
} 
else local call (line W1-W`nplot' z, lpattern(dot ..) lcolor(gs7 ..))

frame `df': {
	tw `call' (line M z )    , by(var, rescale yrescale note("first `nplot' simulated processes") ) ytitle("Cum. martingale residuals") xtitle("Covariate") legend(order(`=`nplot'+1' "Observed" 1 "Simulated")) 
		quie levelsof var, local(vnames)
		foreach v of local vnames {
			tw `call' (line M z ) if  var=="`v'"   , note("first `nplot' simulated processes")  ytitle("Cum. martingale residuals") xtitle("`v'") ///
