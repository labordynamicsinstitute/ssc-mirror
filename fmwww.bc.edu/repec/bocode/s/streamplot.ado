...
version 15
	...
	syntax varlist(min=2 max=2 numeric) [if] [in], by(varname) [color(string) alpha(real 80) smooth(real 6)] ///
		[ LColor(string)] [LWidth(string)] [XLABSize(real 2) labcond(string) YLABSize(real 1.4) YLABel(varname) ] ///
...
colorpalette `mycolor', n(`numcolor') nograph
	...
	, ///
		...
		xlabel(`xti', labsize(`xlabsize') labcolor(`xlabcolor') angle(vertical) glwidth(vvthin) glpattern(solid)) ///
