...
{right:also see: {help switch_oprobit postestimation}}
{hline}
...
{title:Description}
{pstd}
{cmd:switch_oprobit} estimates a model in which {cmd:treat} is a binary indicator for a treatment ({it:y_treat}) for which selection is believed 
...
{phang}{cmd:. switch_oprobit sah female married [pweight=weight], treat(medicaid female married) robust}
{smcl}
