...
{phang} {opt it:erateover(Iteration_Varlist)} this option specifies the list of variables that will be used to generate all possible combinations of regressors (i.e., all-subset regressions), taken from 0 to n at a time, where n is the number of variables in the iteration list. For example, {it:iterateover(var1 var2 var3 var4 var5)} will run estimations using different combinations of these 5 variables — that is, all combinations of 5 variables taken from 0 to 5 at a time. See {it:{help samregc##egncomb:Examples of using ncomb}} {p_end}
...
{phang} {opt it:erateover(Iten_Varlist)} and {opt grit:erateover(Groups_Iteration_Varlist)}: At least one of these options must be included. They specify the variables (and/or groups of variables) to iterate over and evaluate their effect on the coefficients of the main variables. See {it:{help samregc##egitover:Examples of using iterateover and griterateover}}.{p_end}
