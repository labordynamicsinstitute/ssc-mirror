{smcl}
{* *! version November 14, 2016 @ 18:17:52}{...}
...
{phang}
{cmd:sqstrmerge} {hline 2} Merge files using closest matches according to Levenshtein
{p_end}
...
{synopt:{opt asciiletters:only}}sets all non-ascii-letters to missing{p_end}
{synopt:{opt sound:ex}}caluculate distance on soundex transformed strings{p_end}
{syntab:merge options}
{synoptline}
{p2colreset}{...}
...
{pstd} {cmd:sqstrmerge} merges the master and the using data by
finding the closest match of the key variable in the using data. The
closest match is  defined as the match that have the smalles Levenshtein
distance. The command merges close mathches only up to the Levenshtein
distance specified in the option max(){p_end} {p_end}

{marker options}{...}
{title:Options}

{phang} {opt max(#)} do not macht observations with a Levenshtein distance larger than the number specified. This option is requried. The default is {cmd:indelcost(1)}.
...
{synopt:{opt sound:ex}}calculate distance on soundex transformed
strings. See {help soundex()} for an explanation of the
soundex-transformation{p_end}

{marker examples}{...}
{title:Example(s)}{* Be sure to change Example(s) to either Example or Examples}

{phang}{cmd:. sqstrmerge 1:1 prename, max(2)}
{p_end}
