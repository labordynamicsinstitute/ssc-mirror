...
if "`by'" != "" {
	...
	if "`bylabel'" == "bylabel" {
		...
		**# WORD TABLE OUTPUT FOR HEADER - "BY" VARIABLE LABELS
		if "`word'" == "word" {
			putdocx table tbl1(1,`c_col') = ("`variable_lab'"), colspan(`coltospan') border(top) border(bottom) halign(center)  valign(center)
			putdocx table tbl1(1,`finalcol') = (" "), halign(center) border(bottom) valign(center)
			putdocx table tbl1(1,1) = (" "), halign(center) border(bottom) valign(center)
		}
...
if `t' == 1 {
	...
	**# LOOP TO OUTPUT MEAN (SD) SUMMARIES
	...
	if "`mean'" == "mean" {
		...
		**# WORD TABLE LABELS FOR MEANS
		if "`word'" == "word" {
			if "`meanrow'" == "meanrow" {
				putdocx table tbl2(`row',1) = ("`var_lab', Mean (SD)"), halign(left) colspan(2) bold
			}
			else {
	...
	**# LOOP TO OUTPUT MEDIAN (Q1, Q3) SUMMARIES
	...
	if "`median'" == "median" {
		...
		**# WORD LABEL OUTPUT - MEDIANS
		if "`word'" == "word" {
			if "`medianrow'" == "medianrow" {
				putdocx table tbl2(`row',1) = ("`var_lab', Median (Q1, Q3)"), halign(left) colspan(2) bold
			}
			else {
				putdocx table tbl2(`row',1) = ("`var_lab'"), halign(left) colspan(2) bold
				local ++row
	...
	**# WORD LABEL OUTPUT - RANGE
	if "`word'" == "word" {
		if "`rangerow'" == "rangerow" {
			putdocx table tbl2(`row',1) = ("`var_lab', (Min, Max)"), halign(left) colspan(2) bold
		}
		else {
			putdocx table tbl2(`row',1) = ("`var_lab'"), halign(left) colspan(2) bold
			local ++row
