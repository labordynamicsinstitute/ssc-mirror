...
** PURPOSE: THIS PROGRAM ALLOWS THE USER TO CREATE NICELY-FORMATTED SUMMARY
			...
			THIS EXTENSION OF SUMMTAB ALLOWS MORE CONTROL OVER VARIABLE PLACEMENT
			WITHIN THE TABLE.
** UPDATES: 
		SEP 02, 2020 - BUG FIX SINCE PROGRAM WAS NOT WORKING IF ONLY CATEGORICAL VARIABLES ARE SPECIFIED
...
**		AUG 27, 2024 - UPDATES CONSISTENT WITH SUMMTAB UPDATES; SEE CODE FILE FOR SUMMTAB FOR 
**					   LIST OF UPDATES; UPDATING VERSION NUMBER TO BE CONSISTENT WITH SUMMTAB
**		OCT 24, 2024 - UPDATES CONSISTENT WITH SUMMTAB UPDATES; SEE CODE FILE FOR SUMMTAB FOR 
**					   LIST OF UPDATES; UPDATING VERSION NUMBER TO BE CONSISTENT WITH SUMMTAB
** -----------------------------------------------------------------------------
...
** John Gallis is not responsible for the results of use of these programs.
*/
...
/* |||||||||||||||||||||||||||||||||||||||||||||||||||| */
#delimit ;
syntax [if] [in], [by(varname) bylabel bymiss total vars(varlist) type(numlist) meanrow medianrow rangerow catrow mean median range pnonmiss pmiss rowperc catmisstype(string) pval 
...
**# ERROR MESSAGES
/* |||||||||||||||||||||||||||||||||||||||||||||||||||| */	
local typect: word count `type'
local varct: word count `vars'
if "`varct'" != "`typect'" {
...
if "`rangerow'" == "rangerow" & ("`median'" == "median" | "`mean'" == "mean" | "`pnonmiss'" == "pnonmiss") {
	di as error "If using 'range' option, only 'range' can be specified" 
	exit 198
}
...
if "`by'" != "" {
	...
	if "`bylabel'" == "bylabel" {
		...
		**# WORD TABLE OUTPUT FOR HEADER - "BY" VARIABLE LABELS
		if "`word'" == "word" {
			putdocx table tbl(2,`c_col') = ("`variable_lab'"), colspan(`coltospan') border(top) border(bottom) halign(center)  valign(center)
			putdocx table tbl(2,`finalcol') = (" "), halign(center) border(bottom) valign(center)
	...
	foreach l of local `by'_g {
		...
		**# WORD TABLE OUTPUT FOR HEADER - "BY" LEVEL LABELS
		if "`word'" == "word" {
			if "`bylabel'" == "bylabel" {
				putdocx table tbl(3,`c_col') = ("`var_lab'"), halign(center)  colspan(1) valign(center) 
			}
			else {
				putdocx table tbl(2,`c_col') = ("`var_lab'"), halign(center)  colspan(1) valign(center) 
			}
		}
		...
		**# WORD TABLE OUTPUT FOR HEADER - NUMBER OF OBSERVATIONS BY LEVEL
		if "`word'" == "word" {
			if "`bylabel'" == "bylabel" {
				putdocx table tbl(4,`c_col') = ("(N = `obs_')"), halign(center)  colspan(1) valign(center) 
			}
			else {
				putdocx table tbl(3,`c_col') = ("(N = `obs_')"), halign(center)  colspan(1) valign(center) 
			}
		}
		local ++c_col
	...
	**# IF A "TOTAL" COLUMN IS SPECIFIED
	if "`total'" == "total" {
		...
		**# WORD TABLE OUTPUT FOR HEADER - NUMBER OF OBSERVATIONS TOTAL
		if "`word'" == "word" {
			if "`bylabel'" == "bylabel" {
				putdocx table tbl(3,`c_col') = ("Total"), halign(center)  colspan(1) valign(center) 
				putdocx table tbl(4,`c_col') = ("(N = `obs_all')"), halign(center)  colspan(1) valign(center) 
			}
			else {
				putdocx table tbl(2,`c_col') = ("Total"), halign(center)  colspan(1) valign(center) 
				putdocx table tbl(3,`c_col') = ("(N = `obs_all')"), halign(center)  colspan(1) valign(center) 
			}
		}
	...
	**# IF P-VALUE IS SPECIFIED
	if "`pval'" == "pval" {
		...
		**# WORD TABLE OUTPUT FOR HEADER - P-VALUE COLUMN
		if "`word'" == "word" {
			if "`bylabel'" == "bylabel" {
				putdocx table tbl(3,`c_col') = (" "), halign(right)  
				putdocx table tbl(4,`c_col') = ("p-value"), halign(center) 
			}
			else {
				
				putdocx table tbl(2,`c_col') = (" "), halign(right) 
				putdocx table tbl(3,`c_col') = ("p-value"), halign(center) 
			}
		}
...
/* |||||||||||||||||||||||||||||||||||||||||||||||||||| */
	...
	if "`by'" != "" {
		...
		foreach i of local over_group{
			...
			foreach c of local c_l {
				...
				if `cur_col'==1{
					**# WORD OUTPUT - CATEGORICAL VARIABLE LABEL (BY OPTION)
					...
					**# EXCEL OUTPUT - CATEGORICAL VARIABLE LABEL (BY OPTION)
					if "`excel'" == "excel" {
						if "`bylabel'" == "bylabel" {
							local s=`r'-1
						}
				...
				**# WORD OUTPUT - CATEGORICAL VARIABLE FREQUENCY (BY OPTION)
				if "`word'" == "word" {
					putdocx table tbl(`r',`d') = ("`freqperc'"), halign(center)
				}
				
				**# EXCEL OUTPUT - CATEGORICAL VARIABLE FREQUENCY (BY OPTION)
				if "`excel'" == "excel" {
					if "`bylabel'" == "bylabel" {
						local s=`r'-1
					}
				...
				if "`catmisstype'" == "none" | "`catmisstype'" == "missperc" | ("`catmisstype'" == "missnoperc" & `categories' == 1)  {
					...
					*set frequency to missing if it's a column of zeroes
					...
					if "`rowperc'" == "" {
						...
						if "`totalncat'"=="totalncat" {
							mata: st_matrix("N", rowsum(st_matrix("M1")))
							local freq_n = N[1,1]
						}
						mata: st_matrix("M1", (st_matrix("M1")  :/ rowsum(st_matrix("M1"))))
					}
	...
	**# NO BY OPTION SPECIFIED - CATEGORICAL VARIABLE OUTPUT
	...
	di as error "Invalid value of 'type' option; enter only 1 (continuous) or 2 (categorical)"
	exit 198
}
local ++w
local ++x
