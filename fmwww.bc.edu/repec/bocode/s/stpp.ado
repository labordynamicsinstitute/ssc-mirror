...
************
  ...
  if "`standstrata'" != "" {
    ...
    if "`crudeprob'`allcause'" != "" {
      ...
      di as error "You could use the indweights() option as an alternative"
      exit 198
    }
...
*******************
  ...
  if "`using2'" != "" {
    ...
    if `r(N)'==0 {
      di as error "popmort2() file has no observations after restrictions - check syntax."
  ...
  if "`list'" != ""  {
  	quietly {
  	  matrix colnames PP=`by' time PP Nrisk PP_lci PP_uci	
  	  return matrix PP=PP
  	  if "`allcause'"  != "" {
  	  	matrix colnames AC=`by' time Nrisk AC AC_lci AC_uci	
  	    return matrix AC=AC
  	  }
	    if "`crudeprob'" != "" {
	      matrix colnames CP_can=`by' time Nrisk CP_can CP_can_lci CP_can_uci	
	      return matrix CP_can=CP_can
	  	  if wordcount("`CP_newvarnames'")>1 {
	  	    matrix colnames CP_oth=`by' time Nrisk CP_oth CP_oth_lci CP_oth_uci	
...
// maximum potential time.	
  if "`standstrata'" != "" {
    ...
    qui bysort `by' `standstrata': egen double `tmax_by_standstrata' = max(_t) if `touse'
    qui bysort `by': egen double `tmax_by' = min(`tmax_by_standstrata') if `touse'
...
// label variables	
  if "`popmort2'" == "" {
    local rstext = cond("`ederer2'"=="","Pohar Perme","Ederer II")
    label variable `RS_newvarname' "`rstext'"
    label variable `RS_newvarname'_lci "`rstext' lci"
    label variable `RS_newvarname'_uci "`rstext' uci"
    if "`allcause'" != "" {
      label variable `AC_newvarname' "All cause "
      label variable `AC_newvarname'_lci "All cause lci"
      label variable `AC_newvarname'_uci "All cause uci"
    }
    if "`crudeprob'" != "" {
    	tokenize "`CP_newvarnames'"
      label variable `1'     "Crude probability cancer "
      label variable `1'_lci "Crude probability cancer lci"
      label variable `1'_uci "Crude probability cancer uci"
      if "`2'" != "" {
        label variable `2'     "Crude probability other "
        label variable `2'_lci "Crude probability other lci"
        label variable `2'_uci "Crude probability other uci"
      }
    }
  }
  else {
    local rstext Sasieni and Brentnall
    label variable `RS_newvarname' "`rstext'"
    label variable `RS_newvarname'_lci "`rstext' lci"
    label variable `RS_newvarname'_uci "`rstext' uci"
    if "`allcause'" != "" {
      label variable `AC_newvarname' "Reference Adjusted All cause "
      label variable `AC_newvarname'_lci "Reference Adjusted  All cause lci"
      label variable `AC_newvarname'_uci "Reference Adjusted  All cause uci"
    }
    if "`crudeprob'" != "" {
    	tokenize "`CP_newvarnames'"
      label variable `1'     "Reference Adjusted Crude probability cancer "
      label variable `1'_lci "Reference Adjusted Crude probability cancer lci"
      label variable `1'_uci "Reference Adjusted Crude probability cancer uci"
      if "`2'" != "" {
        label variable `2'     "Reference Adjusted Crude probability other "
        label variable `2'_lci "Reference Adjusted Crude probability other lci"
        label variable `2'_uci "Reference Adjusted Crude probability other uci"
      }
    }
  }
...
// estimates
  transmorphic matrix   lambda_e_t,            // contribution to PP at each time
                        ...
                        Nrisk_s_t              // Number at risk at each time point
...
// Results for list
  transmorphic matrix   RS_list_matrix,        // RS results at list values
                        ...
                        Nrisk_list,            // Number at risk at list values
                        Ndeaths_list,          // Number at risk at list values
...
// Results for frame
  transmorphic matrix   RS_frame_results,       // RS results for frame
                        ...
                        Ndeaths_frame           // Number of deaths store in frame
}
...
// various options  
  ...
  S.adddeaths      = st_local("adddeaths") != ""
...
// contribution to PP at each time point
  ...
  S.Nrisk_s_t        = asarray_create("real",S.Nbyvars:+1)
...
void function PP_Gen_estimates(struct stpp_info scalar   S)
{
  real matrix byselect, byselect_ind, lambda_e_tmp, lambda_e_tmp_var,
              lambda_all_tmp, lambda_all_tmp_var, 
              ...
              died_tj, atrisk_index, exprates_index,
              wt_atrisk, N_wt, Y_wt, v1, v2,
              died_tj_select, Ndied_tj,
              tmpattyear, tmpselect, tmp
...
{  
  real matrix  exprates, rateindex, pmotherselect, tmppm 
  ...
  for(p=1;p<=S.Npmoth_levels[1,pmnumber];p++) {
    pmotherselect = rowsum(asarray(S.pmothervars_by,pmnumber)[S.atrisk_all_index,] :== 
                    asarray(S.pmoth_levels,pmnumber)[p,]):==S.Npmother[pmnumber] 
    ...
    tmppm = asarray(pm,asarray(S.pmoth_levels,pmnumber)[p,])
    for(b=ymax;b>=ymin;b--) {
       rateindex = selectindex((tmpattyear_pmo:==b))
       if(cols(rateindex)) tmpexprates[rateindex] = tmppm[tmpattage[rateindex] , b]
     } 
...
{
  real matrix         RS_tmplist, AC_tmplist, 
                      CP_can_tmplist, CP_oth_tmplist, 
                      tindex, tminindex, tmp,
                      RS_tmplist_var, AC_tmplist_var, 
                      CP_can_tmplist_var, CP_oth_tmplist_var,
                      Ndeaths_tmplist

  real scalar         i, k, Natrisk_time0, Ndeaths_time_tj, tj, Natrisk_time_tj
...
// create output list
  ...
  S.Nrisk_list       = asarray_create("real",1)
  S.Ndeaths_list     = asarray_create("real",1)
...
// variance is stored, so can be used later for contrasts
  for(k=1;k<=S.Nbylevels;k++) {
    ...
    AC_tmplist         = J(S.Nlist,5,.)
    AC_tmplist_var     = J(S.Nlist,1,.)
    CP_can_tmplist     = J(S.Nlist,5,.)
    CP_can_tmplist_var = J(S.Nlist,1,.)
    CP_oth_tmplist     = J(S.Nlist,5,.)
    CP_oth_tmplist_var = J(S.Nlist,1,.)
    Ndeaths_tmplist    = J(S.Nlist,1,.)
    ...
    for(i=1;i<=S.Nlist;i++) {
      ...
      if(S.adddeaths) {
        if(i==1) Ndeaths_time_tj = PP_Ndeaths_time_tj(S,k,0,S.list[i])
        else     Ndeaths_time_tj = PP_Ndeaths_time_tj(S,k,S.list[i-1],S.list[i])
        Ndeaths_tmplist[i] = Ndeaths_time_tj
      }  
      ...
      if(S.list[i]<=asarray(S.maxt_k,k) & S.list[i]>=asarray(S.mint_k,k)) {
        ...
        if(S.hasdeathprob) RS_tmplist[i,] = (S.list[i],Natrisk_time0,0,0,0)
        else RS_tmplist[i,] = (S.list[i],Natrisk_time0,1,1,1)
        RS_tmplist_var[i] = 0
        if(S.hasallcause)  {
          AC_tmplist_var[i] = 0
          if(S.hasdeathprob) AC_tmplist[i,] = (S.list[i],Natrisk_time0,0,0,0)    
          else AC_tmplist[i,] = (S.list[i],Natrisk_time0,1,1,1)    
        }
        if(S.hascrudeprob) CP_can_tmplist[i,] = (S.list[i],Natrisk_time0,0,0,0)  
        if(S.CP_calcother) CP_oth_tmplist[i,] = (S.list[i],Natrisk_time0,0,0,0)        
      }
      else {
    ...
    if(S.hascontrast) {
      if(S.contrasttype=="difference") {
        asarray(S.RS_list_var,k,((asarray(S.RS_list_matrix,k)[,3]):^2):*RS_tmplist_var)
        if(S.hasallcause) {
          asarray(S.AC_list_var,k,((asarray(S.AC_list_matrix,k)[,3]):^2):*AC_tmplist_var)
        }
        if(S.hascrudeprob)  {
          asarray(S.CP_can_list_var,k,((asarray(S.CP_can_list_matrix,k)[,3]):^2):*CP_can_tmplist_var)          
        }
        if(S.CP_calcother)  {
          asarray(S.CP_oth_list_var,k,((asarray(S.CP_oth_list_matrix,k)[,3]):^2):*CP_oth_tmplist_var)          
        }        
      }
    } 
    if(S.adddeaths) {
      asarray(S.Ndeaths_list,k,Ndeaths_tmplist)
    }
  }
...
void function PP_calc_list_contrast(struct stpp_info scalar S)
{
  ...
  for(k=1;k<=S.Nbylevels;k++) {
    ...
    if(k==tmprefindex) {
      ...
      diff     = asarray(S.RS_list_matrix ,k)[,3] :- asarray(S.RS_list_matrix ,tmprefindex)[,3]
      ...
      if(S.hasallcause) {
        diff     = asarray(S.AC_list_matrix ,k)[,3] :- asarray(S.AC_list_matrix ,tmprefindex)[,3]
      ...
      if(S.hascrudeprob) {
        diff     = asarray(S.CP_can_list_matrix ,k)[,3] :- asarray(S.CP_can_list_matrix ,tmprefindex)[,3]
      ...
      if(S.CP_calcother) {
        diff     = asarray(S.CP_oth_list_matrix ,k)[,3] :- asarray(S.CP_oth_list_matrix ,tmprefindex)[,3]
...
void function PP_calc_return_matrices(struct stpp_info scalar S)
{
  ...
  tempPP = J(0,S.Nbyvars+5,.)
  if(S.hasallcause) tempAC = J(0,S.Nbyvars+5,.)
  if(S.hascrudeprob) tempCP_can = J(0,S.Nbyvars+5,.)
  if(S.CP_calcother) tempCP_oth = J(0,S.Nbyvars+5,.)       
  if(S.hascontrast) {
  ...
  if(S.adddeaths) temp_Ndeaths = J(0,1,.)
...
// temppPP_con etc do the same for contrasts for frames
  for(k=1;k<=S.Nbylevels;k++) {
    ...
    if(S.adddeaths) {
      temp_Ndeaths = temp_Ndeaths \ asarray(S.Ndeaths_list,k)
    }
  }
  ...
  if(S.adddeaths) S.Ndeaths_frame = temp_Ndeaths
...
void function PP_print_list_results(struct stpp_info scalar S)
{
  ...
  for(k=1;k<=S.Nbylevels;k++) {
    ...
    printf("{txt}{space   2}Time{space   5}{c |}{space 2}Natrisk{space 5}" + methodshort+"{space 5}(%s{txt}%% CI)" ,strofreal(S.level*100))
    if(S.hascontrast) printf("{space 9} {c |}{space 3}Difference (%s{txt}%% CI)",strofreal(S.level*100))
    printf("\n")
    printf("{hline 11}{c +}{hline 39}")		
    if(S.hascontrast) printf("{c +}{hline 30}")
    printf("\n")
    
    for(i=1;i<=S.Nlist;i++) {
      printf("{res}%6.3g{space 5}{txt}{c |}{space 1}{res}%8.0fc{res}{space 5}%5.3f (%5.3f to %5.3f)",
      asarray(*outtype,k)[i,1],
      asarray(*outtype,k)[i,2],
      asarray(*outtype,k)[i,3],
      asarray(*outtype,k)[i,4],
      asarray(*outtype,k)[i,5])
      if(S.hascontrast) {
        if(sum(k:==S.bylevel_isref)) {
          printf("{space 3}{txt}{c |}{space 10}-")
        }
        else {
          printf("{space 3}{txt}{c |}{space 3}{res}%6.3f (%6.3f to %6.3f)",
    ...
    printf("{txt}{hline 11}{c +}{hline 39}")
    if(S.hascontrast) printf("{c +}{hline 30}")
    printf("\n\n")  
  }
}
...
void function PP_write_frame_results(struct stpp_info scalar S)
{
  ...
  if(S.adddeaths) {
    newvars = "Ndeaths"
    (void) st_addvar("double", newvars)   
    st_store(.,newvars,.,S.Ndeaths_frame)
  }
...
{
  real matrix byselect, byselect_ind  
  
  byselect     = rowsum(S.by:==S.bylevels[k,]):==S.Nbyvars 
  byselect_ind = selectindex(byselect)
  
  return(sum(S.t0[byselect_ind]:==0))
}

// calculate number at risk at time tj (for by group k)
real function PP_Natrisk_time_tj(struct stpp_info scalar S, real scalar k, real scalar tj)
{
  real matrix byselect, byselect_ind  
...
// calculate number of deaths in interval for frame
real function PP_Ndeaths_time_tj(struct stpp_info scalar S, real scalar k, real scalar tstart, real scalar tend)
{
  real matrix byselect, byselect_ind  
  ...
  return(sum(S.d[byselect_ind] :&  ///
             S.t[byselect_ind]:>tstart :& S.t[byselect_ind]:<=tend)) 
}
