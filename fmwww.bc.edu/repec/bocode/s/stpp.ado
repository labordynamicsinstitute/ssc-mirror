...
*******************
  ...
  tempvar tmplink
  quietly {
    frlink m:1 _t _d `by', frame(stpp_tmpdataframe) generate(`tmplink')
    frget *, from(`tmplink') 
...
// various options  
  ...
  S.level          = st_numscalar("c(level)")/100  
  ...
  if(S.hasstandstrata) {
    ...
    for(k=1;k<=S.Nbylevels;k++) {
      ...
      if(S.hascrudeprob) {
        ...
        tmpmat_v = quadrunningsum((allcause_lag :*(1:-tmp_CP_t_s)):^2:*asarray(S.lambda_all_t_var,(1,S.bylevels[k,])))
...
// List results
  ...
  if(S.haslist) {
    ...
    for(k=1;k<=S.Nbylevels;k++) {
      ...
      printf("{txt}Time{space 5}{c |}   PP (95%% CI) \n")
...
void function PP_Write_new_variables(struct stpp_info scalar   S) 
{
  ...
  for(k=1;k<=S.Nbylevels;k++) {
    ...
    if(S.Nbylevels>1) data_k = data_k, J(S.Nunique_t_k[k],1,S.bylevels[k,])  // by vars
