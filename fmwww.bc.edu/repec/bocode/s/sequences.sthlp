...
{p2colset 5 17 18 2}{...}
{p2col:{hi:sequences} {hline 2}} Generates versatile numerical sequences {p_end}
{p2colreset}{...}
...
{pstd}
{opt sequences} is a versatile alternative to official Stata's {helpb egen seq()} for generating numerical sequences. In contrast to {helpb egen seq()}, 
{opt sequences} can generate sequences when there are currently no observations in the data (_N == 0), and {opt sequences} accepts non-integer values for 
{cmd:from()}, {cmd:to()}, and {cmd:by()} options, thereby generating non-integer sequences. Like {helpb egen seq()}, {opt sequences} can generate 
...
{pmore2}{bf:{stata "sequences test, from(-30) to(30) by(0.5)": . sequences test, from(-30) to(30) by(0.5)}} {p_end}
{pmore2}{bf:{stata "sum test": . sum test}} {p_end}

{pmore} Same as above but we do not specify {cmd:by()} {p_end}
{pmore2}{bf:{stata "clear": . clear}} {p_end}
{pmore2}{bf:{stata "sequences test, from(-30) to(30)": . sequences test, from(-30) to(30)}} {p_end}
{pmore2}{bf:{stata "sum test": . sum test}} {p_end}

{pmore} Same as above but we do not specify {cmd:from()} {p_end}
{pmore2}{bf:{stata "clear": . clear}} {p_end}
{pmore2}{bf:{stata "sequences test, to(30) by(0.5)": . sequences test, to(30) by(0.5)}} {p_end}
{pmore2}{bf:{stata "sum test": . sum test}} {p_end}

{pmore} Same as above but we specify only {cmd:to()} {p_end}
{pmore2}{bf:{stata "clear": . clear}} {p_end}
{pmore2}{bf:{stata "sequences test, to(30)": . sequences test, to(30)}} {p_end}
{pmore2}{bf:{stata "sum test": . sum test}} {p_end}
...
{p 4 8 2}{cmd:sequences} is not an official Stata command. It is a free contribution
to the research community, like a paper. Please cite it as such: {p_end}

{p 4 8 2}
Linden A. (2019). SEQUENCES: Stata module for generating versatile numeric sequences.
{p_end}
