...
{* September 2026}{...}
{hline}
help for {hi:smfit}{right:Stephen P. Jenkins (September 2026)}
{hline}

{title:Fitting a Singh-Maddala distribution by ML to unit record data}

{p 8 17 2}{cmd:smfit} {it:var} [{it:weight}] [{cmd:if} {it:exp}]
	[{cmd:in} {it:range}] [{cmd:,}
	{cmdab:a:var(}{it:varlist1}{cmd:)} {cmdab:b:var(}{it:varlist2}{cmd:)} 
	{cmdab:q:var(}{it:varlist3}{cmd:)} {cmdab:abq(}{it:varlist}{cmd:)}
...
{p 4 4 2}{cmd:by} {it:...} {cmd::} may be used with {cmd:smfit}; see help
{help by}. 

{p 4 4 2}Factor variables may be used with {cmd:smfit} when specifying a parameter 
or parameters as a function of covariates; see help {help fvarlist}.
...
{p 4 4 2}
{cmd:smfit} fits by ML the 3 parameter Singh-Maddala (1976) distribution to 
sample observations on a random variable {it:var}. Unit record data 
are assumed (rather than grouped data). Otherwise known as 
...
distribution (see {help gb2fit}). For a comprehensive review of these 
and other related distributions, see Kleiber and Kotz (2003). 
For derivation of Lorenz orderings of pairs of income distributions 
in terms of their Singh-Maddala parameters, see Wifling and 
Kraemer (1993) and Kleiber (1996). Of course the Singh-Maddala 
distribution might be suitable for describing any skewed variable, 
not only income.
...
{p 4 8 2}{cmd:abq(}{it:varlist}{cmd:)} can be used instead of the previous option 
...
{p 4 8 2}{cmd:from(}{it:string}{cmd:)} specifies initial values for the 
	...
	names with the values 	(e.g., from(a:_cons=1 b:_cons=5 q:_cons = .16);
...
{p 8 8 2}If covariates are specified, the next four options are not available. 
	Use {help smpred} to generate statistics at particular values of the 
...
{p 4 8 2}{cmd:stats} displays selected distributional statistics implied by the
	Singh-Maddala parameter estimates:  quantiles, cumulative 
...
{p 4 4 2}In addition to the usual results saved after {cmd:ml}, {cmd:smfit} also
saves the following, if there are no covariates have been specified and
the relevant options used:

{p 4 4 2}{cmd:e(ba)}, {cmd:e(bb)}, and {cmd:e(bq)} are the estimated Singh-Maddala 
parameters.
...
Parameters a and q are the key distributional 'shape' parameters; b is a scale parameter.
...
{p 8 8 2}
      	L(s) = {cmd:ibeta}(1+1/a, q- 1/a, 1-(1-s)^(1/q) ).

	
{title:Examples}

{p 4 8 2}{inp:. smfit x [w=wgt] }

{p 4 8 2}{inp:. smfit }

{p 4 8 2}{inp:. smfit, stats poorfrac(100) }

{p 4 8 2}{inp:. smfit x, a(age sex) b(age sex) q(age sex) }

{p 4 8 2}{inp:. smfit x, abq(age sex) }
...
programs for distributional diagnostic plots ({help qsm}, {help psm}).
...
{p 4 8 2}Wifling, B. and W. Kraemer (1993). The Lorenz-ordering of Singh-
	Maddala income distributions. {it:Economics Letters} 43: 53-57.

{title:Also see}

{p 4 13 2}
Online: help for {help smpred}, {help psm}, {help qsm}, {help dagumfit}, {help gb2fit}, 
{help lognfit}, if installed. 
