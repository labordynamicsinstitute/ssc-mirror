...
{* 16June2012}{...}
{cmd:help sdhollister}
{hline}

{title:Title}

{p2colset 5 17 23 2}{...}
{p2col:{hi:sdhollister} {hline 2}}Calculate inter-sequence distances using Hollister's Localized OM{p_end}
{p2colreset}{...}

{title:Syntax}

{p 8 17 2}
{cmd:sdhollister} {it: varlist} {cmd:,} {it:options} [option] 
...
{pstd}{cmd:sdhollister} calculates localised Optimal Matching distances
...
{phang}{cmd:. sdhollister mon1-mon36, subsmat(scost) time(0.5) local(0.5) pwdist(dist) len(36)}{p_end}
{phang}{cmd:. matrix list dist}{p_end}

{phang}{cmd:. sdhollister mon1-mon36, subsmat(scost) time(0.5) local(0.5) pwdist(dist) len(dur)}{p_end}
{phang}{cmd:. matrix list dist}{p_end}
