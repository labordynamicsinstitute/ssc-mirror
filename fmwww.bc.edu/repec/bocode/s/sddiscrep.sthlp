...
{p2colset 5 20 22 2}{...}
{p2col:{hi:sddiscrep} {hline 2}}Calculate Studer et al's discrepancy measure{p_end}
{p2colreset}{...}

{title:Syntax}

{p 8 17 2}
{cmd:sddiscrep} {it: groupvar} , DISTmat(string) IDvar(varname) [NITer(integer 100) DCG(string)]
...
{pstd}{cmd:sddiscrep} calculates Studer et al's measure of the
...
{pstd}The pseudo-F is exactly the same as that calculated by the
Caliński-Harabasz cluster analysis stopping-rule index.
See {help calinski} if installed. {p_end}
...
{phang}{cmd:. sddiscrep sex, dist(d) id(id) dcg(dsex)}{p_end}
{phang}{cmd:. bysort sex: egen mindist = min(dsex)}{p_end}
{phang}{cmd:. gen medoid = mindist == dsex}{p_end}
