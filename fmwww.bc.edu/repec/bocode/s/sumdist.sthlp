...
{* February 2021}{...}
{hline}
help for {hi:sumdist}{right:Stephen P. Jenkins (revised February 2021)}
{hline}
...
{p 8 15 2}
{cmd:sumdist} {it:varname} [{it:weights}]  [{cmd:if} {it:exp}]
...
{p 4 4 2}
Bootstrapped standard errors for unweighted estimates can be derived using 
{help bootstrap}. Bootstrapped standard errors for weighted estimates can 
be derived using {help svy bootstrap} and Philippe Van Kerm's {cmd:rhsbsample}
on SSC. Standard errors derived using linearization methods can be calculated 
for Lorenz and generalized ordinates using {cmd:svylorenz} on SSC.
