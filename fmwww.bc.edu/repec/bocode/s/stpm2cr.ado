...
SI: Change censvalue() so that user can specify a numlist
SI: Need origin to test delayed entry models
SI: Delayed entry models still incomplete - need to correct code and likelihood
*/ 
...
version 14.1
	...
	if replay() {
		...
		if "`df'`knots'" != "" {
			`by' Estimate `0'
			ereturn local cmdline `"stpm2crinit `0'"'
		}
		else {
			if "`e(cmd)'" != "stpm2cr" {
				error 301
...
program Estimate, eclass byable(recall)
	...
	/*else {
		tempvar newcens
		gen `newcens' = .
		foreach c in "`censvalue'" {
			replace `newcens' = 0 if `events' == `c'
	}*/
	...
	foreach n in `causelist' {													
		local 0 `syntax_c`n''
		syntax [varlist(default=empty)], [/*cause(string)*/ df(int 0) scale(string) tvc(varlist fv) dftvc(string) BKnots(numlist ascending min=2 max=2) KNOTS(numlist ascending) rcsbaseoff noCONStant CURE REVerse ]
		...
		if (`nbhknots' == 0 & "`df'" == "") & "`rcsbaseoff_c`n''" == "" {
		...
		if "`tvc_c`n''" != "" {
			if "`dftvc_c`n''" == "" {
				display as error "The dftvc option is compulsory if you use the tvc option in equation `n'"
				exit 198
			}
		...
		if `nbhknots' == 0 & "`rcsbaseoff_c`n''" == "" & "`cure_c`n''" == "" { 
			...
			qui rcsgen `lnt' if `touse', knots(`knots') gen(_rcs_c`n'_) dgen(_d_rcs_c`n'_) if2(`_d`n'')  `orthog' `reverse_c`n''
			local rknots_c`n' `r(knots)'
			tokenize `knots'
			local df : word count `knots'
			local df = `df' - 1
			//di `df'
			local df_c`n' : word count `knots'
		}
			...
			foreach tvcvar in  `tvc_c`n'' {
				...
				if "`rcsbaseoff_c`n''" == "" & "`cure_c`n''" == ""{
					if `nbhknots' == 0 {
						...
						qui rcsgen `lnt' if `touse', knots(`tvc_`tvcvar'_knots_c`n'') gen(_rcs_`tvcvar'_c`n'_) dgen(_d_rcs_`tvcvar'_c`n'_) if2(`_d`n'') `orthog' `reverse_c`n''
						local rknotstvc_`tvcvar'_c`n' `r(knots)'
					}
				}
					...
					if `nbhknots' == 0 {
						...
						qui rcsgen `lnt' if `touse', df(``tvc_`tvcvar'_df_c`n''_temp') gen(`rcs_`tvcvar'_`n'_') dgen(`d_rcs_`tvcvar'_`n'_') if2(`_d`n'') bknots(`bknots')  `orthog' `reverse_c`n''
						local rknotstvc_`tvcvar'_c`n' `r(knots)'
...
//di "delayed entry `n': `conslist'"
	...
	*/
		...
		capture ml model `mlmethod' stpm2cr_ml_`scale'() /// 
			`mleqfit' ///
			if `touse' ///
			`wt', ///
			`mlopts' ///
