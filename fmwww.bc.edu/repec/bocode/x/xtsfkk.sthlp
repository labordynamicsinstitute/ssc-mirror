...
{synopt :{cmdab:save(}{it:{help filename:filename}}{cmd:)}}save the current status of the estimation while running{p_end}
{synopt :{cmdab:load(}{it:{help filename:filename}}{cmd:)}}load the estimation from a previously saved file to continue
from where the estimation was{p_end}
...
{synopt :{cmd:beep[({it:#})]}}beep when command completes{p_end}
...
{phang}
{cmd:save({it:{help filename:filename}})} saves the current status of the estimation to the hard drive
in every iteration while the estimation is running. Saving is especially useful if the user thinks 
that intentional breaks may be needed or unintentional interruptions may happen while the estimation is running.
{cmd:save} option allows stopping the estimation temporarily to release memory for other tasks, and then continue
from where the estimation was left by using the {cmd:load} option. Even if the computer completely shuts down for 
some external reason, as long as the {cmd:save} option is specified, the estimation can continue from where it was 
with the {cmd:load} option.

{phang}
{cmd:load({it:{help filename:filename}})} loads the estimation from a previously saved file to continue from
where the estimation was. The model specification with the {cmd:load} option needs to be the same as the specification
in the saved file.
...
{phang}
{cmd:beep[({it:#})]} produces a beep when {opt xtsfkk} reports all of the findings. {opt beep} is
useful for multitasking. If {it:#} is specified, then the option produces {it:#} number of beeps.
If {it:#} is negative, then the option acts like an alarm and produces continuous beeps until the user 
interrupts them actively.
...
{pstd}Examples of using {cmd:iterate(}{it:#}{cmd:)}, {cmd:initial(}{it:matname}{cmd:)}, 
{cmd:efficiency(}{it:effvar}{cmd:)}, {cmd:beep(}{it:#}{cmd:)} and {opt test} options{p_end}
{phang2}{cmd:. {stata "xtsfkk y x1 z1, cost u(z2) en(z1 z2) i(iv1 iv2) iter(6) timer beep(3)"}}{p_end}
{phang2}{cmd:. {stata "matrix EST = e(b)"}}{p_end}
{phang2}{cmd:. {stata "xtsfkk y x1 z1, cost u(z2) en(z1 z2) i(iv1 iv2) init(EST) eff(costef) test beep(-1)"}}{p_end}

{pmore}Note that in the examples above, {cmd:beep(3)} option results in 3 beeps once the output is displayed
and {cmd:beep(-1)} option results in continuous beeps that would not stop unless the user actively stops them. 
Beep option is useful for multitasking so that the user would not need to view the Stata results
window until it starts beeping.

{pstd}Examples of using {opt compare} and {opt nicely} options{p_end}
{phang2}{cmd:. {stata "xtsfkk y x1 z1, cost u(z2) en(z1 z2) i(iv1 iv2) delve iter(15) compare beep"}}{p_end}
...
{pstd}Examples of using {opt difficult}, and {cmd:technique(}{it:algorithm_spec}{cmd:)}{p_end}
...
{phang2}{cmd:. {stata "xtsfkk y x1 x2 x3 z1 i.year, u(z2) en(z1 z2) i(iv1 iv2) nicely compare diff"}}{p_end}
...
{pmore}Note that in the example above, the estimation is intentially limited to 5 iterations. {cmd: save} option
saves the status of estimation in each iteration, and when the estimation stops, it records the status from
the last iteration. In the example below, the estimation loads the saved file and continues from where it was left. Note
that the model specifications are exactly the same. These two examples demostrate the usage of {cmd:save} and {cmd:load}
options. 

{phang2}{cmd:. {stata "xtsfkk loans cdep hotm gta roe big, u(hhi) en(hhi) i(rep) header compare nicely timer beep(3) load(banks.est)"}}{p_end}

{pmore}For details about the datasets and results, see Karakaplan (2020). 
