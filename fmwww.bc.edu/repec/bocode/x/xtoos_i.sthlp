...
{p}{cmd:xtoos_i} reports the specified model's forecasting performance, both in absolute terms (RMSE) and also relative to an alternative model by means of an U-Theil ratio, which is the ratio
between the RMSE of the specified model and the RMSE of the alternative model. By default, the alternative model is a "naive" prediction in which the mean of all in-sample individuals 
at every period {it:t} is used as a prediction for the excluded ones.  The procedure also allows to use an AR1 model as the alternative model for the comparison.{p_end}

{p}It also reports several in-sample and out-of-sample statistics of both the specified and the Alternative model. In the summary table, the in-sample statistics are labeled as "_in"
and the out-of-sample are labeled as "_oos". The procedure reports the in-sample R2 and the out-of-sample pseudo-R2 of both the specified model and of the alternative model, as well as the RMSE of those 
same cases, plus the U-theil ratio.{p_end}
...
{synopthdr:Options}
{synoptline}
{synopt:{opt *o:us()}}It is always required. Specifies the size {it:n} of each random subsample (i.e. the number of individuals) that is excluded from each (in)sample estimation, and for which the prediction performance is evaluated.{p_end}
...
{synopt:{opt *k:smpl()}}It is always required. Specifies the size {it:k} of each partition or (non-random) subsample, i.e. the number of individuals in each partition, and for which the prediction performance is evaluated. 
