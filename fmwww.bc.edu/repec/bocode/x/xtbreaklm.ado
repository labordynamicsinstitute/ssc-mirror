*! xtbreaklm 1.0.0  23jul2026
*! Panel Lagrange-multiplier unit-root tests allowing for structural breaks
*! Enders-Lee flexible Fourier LM (smooth breaks) and Lee-Tieslau two-break LM
*! Author: Dr Merwan Roudane  (merwanroudane920@gmail.com)
*! https://github.com/merwanroudane
program xtbreaklm, rclass
    version 14.0
    syntax varname [if] [in] , [ TYPE(string) NHARM(integer 1) ///
        Model(string) Pmax(integer 8) IC(integer 3) ]

    // ---- resolve type ------------------------------------------------
    if ("`type'"=="") local type "fourier"
    local type = strlower("`type'")
    if !inlist("`type'","fourier","sharp") {
        di as error "type() must be {bf:fourier} (Enders-Lee) or {bf:sharp} (Lee-Tieslau)"
        exit 198
    }
    if ("`model'"=="") local model "trend"
    local model = strlower("`model'")
    if !inlist("`model'","constant","trend") {
        di as error "model() must be constant or trend"
        exit 198
    }
    if ("`type'"=="fourier") {
        if `nharm'<0 | `nharm'>3 {
            di as error "nharm() must be between 0 and 3 for the Fourier LM test"
            exit 198
        }
    }
    if `ic'<1 | `ic'>3 {
    ...
    if "`r(balanced)'"!="strongly balanced" {
        di as error "xtbreaklm requires a strongly balanced panel"
        exit 451
    }
    tempvar tag
    ...
    di ""
    if ("`type'"=="fourier") {
        di as txt "Enders-Lee panel Fourier LM unit-root test" ///
            _col(52) "Number of panels" _col(70) "= " as res %5.0f `N'
        di as txt "H0: unit root (all panels)" ///
            _col(52) "Time periods" _col(70) "= " as res %5.0f `T'
        di as txt "Ha: at least one panel is (trend-)stationary" ///
            _col(52) "Cumulative freq." _col(70) "= " as res %5.0f `nharm'
    }
    else {
        di as txt "Lee-Tieslau panel two-break LM unit-root test" ///
            _col(52) "Number of panels" _col(70) "= " as res %5.0f `N'
        di as txt "H0: unit root (all panels)" ///
            _col(52) "Time periods" _col(70) "= " as res %5.0f `T'
        di as txt "Ha: at least one panel is trend-stationary (2 breaks)"
    }

    // ---- dispatch to Mata (fills matrices, does not touch r()) ------
    tempname OUT STAT
    mata: _xbl_run("`varlist'","`touse'", `N', `T', "`type'", ///
        `nharm', "`model'", `pmax', `ic', `tmin', "`OUT'", "`STAT'")

    // ---- returns (all posted here, after Mata) ----------------------
    if ("`type'"=="fourier") {
        return scalar P        = `STAT'[1,1]
        return scalar P_p      = `STAT'[1,2]
        return scalar Pm       = `STAT'[1,3]
        return scalar Pm_p     = `STAT'[1,4]
        return scalar Choi     = `STAT'[1,5]
        return scalar Choi_p   = `STAT'[1,6]
        return scalar nreject  = `STAT'[1,7]
    }
    else {
        return scalar PanelLM   = `STAT'[1,1]
        return scalar PanelLM_p = `STAT'[1,2]
    }
    return matrix units = `OUT'
    return local model  "`model'"
    return local type   "`type'"
    return local cmd    "xtbreaklm"
    return scalar N = `N'
    return scalar T = `T'
end
...
// ---------- helpers ---------------------------------------------------
real colvector _xbl_lagn(real colvector x, real scalar n)
{
    real scalar r
    r = rows(x)
...
real colvector _xbl_diff(real colvector x, real scalar k)
{
    if (k==0) return(x)
    return((J(k,1,0) \ (x[(k+1)..rows(x)] - _xbl_lagn(x,k)[(k+1)..rows(x)])))
}
// OLS via the moment matrix, returning coefficient standard errors
void _xbl_ols(real colvector y, real matrix X, real colvector b,
...
real scalar _xbl_getlag(real scalar ic, real scalar pmax,
    real colvector aicp, real colvector sicp, real colvector tstatp)
{
    real scalar p, j
    if (ic==1) return(_xbl_argmin(aicp))
    if (ic==2) return(_xbl_argmin(sicp))
    j = pmax + 1
    while (j >= 1) {
...
real scalar _xbl_argmin(real colvector v)
{
    real scalar i, im, mn
    mn = v[1]
    im = 1
...
// ---------- 221-percentile response-surface machinery ---------------
real colvector _xbl_pgrid()
{
    real colvector p
    p = (0.0001 \ 0.0002 \ 0.0005) \ ((1::9)/1000) \ (0.005:*(2::198)) \ ((991::999)/1000) \ (0.9995 \ 0.9998 \ 0.9999)
    return(p)
}
real matrix _xbl_rsblock(real scalar m)
{
    if (m==0) return(_xbl_rs0())
    if (m==1) return(_xbl_rs1())
    if (m==2) return(_xbl_rs2())
    return(_xbl_rs3())
}
real scalar _xbl_pval(real scalar test, real scalar T, real scalar m, real scalar plags)
{
    real matrix params, Xw
    real colvector p, reg, cv, yv, bw, idx
    real scalar it, pt, mn, mx, bands, lower, lo, hi, pval
    params = _xbl_rsblock(m)
    it = 1/T
    pt = plags/T
...
// ---------- Enders-Lee flexible Fourier LM on one series ------------
// returns (tau, plags)
real rowvector _xbl_fourierlm(real colvector y, real scalar pmax,
    real scalar m_max, real scalar ic)
{
    real scalar t, k, p, tau, laglm, psi
    real colvector dt, dy, st, ds, ls, b0psi, b, se, e, dep, s1, aicp, sicp, tstatp, taup, zb
    real matrix sink, cosk, z, dsink, dcosk, dz, dsinp, dcosp, X, ds_lags, dc
    real scalar j
    t = rows(y)
    dt = (1::t)
    sink = J(t,0,0)
    ...
    k = 1
    while (k <= m_max) {
        sink = sink , sin(2*pi()*k*(1::t)/t)
        cosk = cosk , cos(2*pi()*k*(1::t)/t)
        k = k + 1
    }
    z = dt , sink , cosk
    dy = _xbl_diff(y,1)[2..t]
    dsink = _xbl_diff2(sink,1)
    dcosk = _xbl_diff2(cosk,1)
    dz = J(rows(dy),1,1) , dsink , dcosk
    b0psi = _xbl_solve(dy, dz)
    psi = y[1] - (dz[1,.]*b0psi)[1,1]
    zb  = z*b0psi
    st  = (y :- psi) - zb
    ds = _xbl_diff(st,1)[2..rows(st)]
    ls = _xbl_lagn(st,1)[2..rows(st)]
    taup = J(pmax+1,1,0)
    aicp = J(pmax+1,1,0)
    sicp = J(pmax+1,1,0)
    tstatp = J(pmax+1,1,0)
    ...
    while (p <= pmax) {
        ...
        s1  = ls[(p+1)..rows(ls)]
        dc  = J(rows(s1),1,1)
        dsinp = dsink[(p+1)..rows(dsink),.]
        dcosp = dcosk[(p+1)..rows(dcosk),.]
        if (p==0) X = s1 , dc , dsinp , dcosp
        else {
            ds_lags = J(rows(ds),p,0)
            j = 1
            while (j <= p) {
                ds_lags[.,j] = _xbl_lagn(ds,j)
                j = j + 1
            }
            ds_lags = ds_lags[(p+1)..rows(ds_lags),.]
            X = s1 , dc , dsinp , dcosp , ds_lags
        }
        _xbl_ols(dep, X, b, se, e)
        taup[p+1] = b[1]/se[1]
    ...
    laglm = _xbl_getlag(ic, pmax, aicp, sicp, tstatp)
    tau = taup[laglm]
    return((tau, laglm-1))
}
// differencing helpers for matrices (drop first differenced row)
real matrix _xbl_diff2(real matrix X, real scalar k)
{
    real scalar t
    t = rows(X)
    return(X[(k+1)..t,.] - X[1..(t-k),.])
}
real colvector _xbl_solve(real colvector y, real matrix X)
{
    return(invsym(cross(X,X))*cross(X,y))
}

// ---------- Lee-Tieslau two-break (trend) LM on one series ----------
// returns (LM, tb1, tb2, plags).  model fixed to trend+level (LS 2004)
real rowvector _xbl_lt2breaks(real colvector y, real scalar pmax, real scalar ic)
{
    real scalar t, tb1, tb2, T1, T2, LMmin, tb1min, tb2min, p, stat, j, s0
    real colvector dt, du1, du2, dt1, dt2, d1, d2, dy, b0, st, s2v
    real colvector dep, ls, ds1, dylm, taup, aicp, sicp, tstatp, b, se, e
    real matrix z, dz, X, lmat, ds
    t = rows(y)
    T1 = round(0.15*t)
    T2 = round(0.85*t)
    ...
    while (tb1 <= T2) {
        ...
        while (tb2 <= T2) {
            ...
            d2  = (J(tb2,1,0) \ 1 \ J(t-tb2-1,1,0))
            z   = dt , du1 , dt1 , du2 , dt2 , d1 , d2
            dy  = _xbl_diff(y,1)[2..t]
            dz  = _xbl_diff2(z,1)
            b0  = _xbl_solve(dy, dz)
            s0 = y[1] - (z[1,.]*b0)[1,1]
            st  = (y :- s0) - z*b0
            // Lee-Strazicich transformation of the LM detrended series
            s2v = J(t,1,0)
            s2v[1..tb1] = st[1..tb1] :/ (tb1/t)
            s2v[(tb1+1)..tb2] = st[(tb1+1)..tb2] :/ ((tb2-tb1)/t)
            s2v[(tb2+1)..t] = st[(tb2+1)..t] :/ ((t-tb2)/t)
            dylm = _xbl_diff(s2v,1)[2..t]
            lmat = J(rows(dylm),pmax,0)
            j = 1
            while (j <= pmax) {
                lmat[.,j] = _xbl_lagn(dylm,j)
                j = j + 1
            }
            taup = J(pmax+1,1,0)
            aicp = J(pmax+1,1,0)
            ...
            p = 0
            while (p <= pmax) {
                dep = _xbl_diff(y,1)[(p+2)..t]
                ls  = _xbl_lagn(s2v,1)[(p+2)..t]
                dz  = _xbl_diff2(z,1)
                dz  = dz[(p+1)..rows(dz),.]
            ...
            p = _xbl_getlag(ic, pmax, aicp, sicp, tstatp)
            stat = taup[p]
            if (stat < LMmin) {
                tb1min = tb1
                tb2min = tb2
    ...
    return((LMmin, tb1min, tb2min, p-1))
}

// Lee-Tieslau moment table: two trend breaks (crit_tr rows 5-7)
// columns: mean/var for p = 0..8, chosen row by T
real rowvector _xbl_ltmoment(real scalar T, real scalar plag)
{
    real matrix M
    real scalar row, optk
    M = (-3.22, 0.37, -3.27, 0.35, -3.26, 0.32, -3.30, 0.33, -3.27, 0.35, -3.28, 0.40, -3.21, 0.45, -3.18, 0.53, -3.08, 0.59 \
         -3.19, 0.34, -3.21, 0.33, -3.21, 0.31, -3.23, 0.30, -3.23, 0.30, -3.24, 0.30, -3.23, 0.29, -3.24, 0.30, -3.22, 0.31 \
         -3.17, 0.33, -3.18, 0.32, -3.18, 0.32, -3.19, 0.31, -3.19, 0.30, -3.20, 0.30, -3.20, 0.29, -3.21, 0.29, -3.20, 0.28)
    if (T <= 75) row = 1
    else if (T <= 150) row = 2
    else row = 3
    optk = plag
    if (optk > 8) optk = 8
    return((M[row, 1+2*optk], M[row, 2+2*optk]))
}

// ---------- driver ---------------------------------------------------
void _xbl_run(string scalar vname, string scalar touse, real scalar N,
    real scalar T, string scalar type, real scalar nharm,
    string scalar model, real scalar pmax, real scalar ic, real scalar tmin,
    string scalar outname, string scalar statname)
{
    real matrix Y, out, mom
    real colvector yi, res, tau, pval, lags, br1, br2, cv10, cv5, cv1
    real scalar i, P, Pm, Choi, nrej, meanLM, meanp, varp, panelLM, panelp
    real rowvector mr
    Y = _xbl_getpanel(vname, touse, N, T)
    if (type=="fourier") {
        tau  = J(N,1,0)
        pval = J(N,1,0)
        lags = J(N,1,0)
        ...
        cv1  = J(N,1,0)
        for (i=1; i<=N; i++) {
            res = _xbl_fourierlm(Y[.,i], pmax, nharm, ic)'
            tau[i]  = res[1]
            lags[i] = res[2]
            pval[i] = _xbl_pval(res[1], T, nharm, res[2])
            // report the response-surface critical values at 10/5/1%
            cv10[i] = _xbl_cv(T, nharm, res[2], 31)
            cv5[i]  = _xbl_cv(T, nharm, res[2], 21)
            cv1[i]  = _xbl_cv(T, nharm, res[2], 13)
        }
        P    = -2*sum(ln(pval))
        Pm   = (P - 2*N)/sqrt(4*N)
        Choi = sum(invnormal(pval))/sqrt(N)
        nrej = sum(pval :< 0.05)
        out = (1::N), tau, pval, lags, cv10, cv5, cv1
        _xbl_print_unit(out, ("id","LM-tau","p-value","lags","cv10%","cv5%","cv1%"))
        printf("{txt}{hline 78}\n")
        printf("{txt}Fisher P (chi2, %g df){col 40}{res}%9.3f{txt}   p = {res}%6.3f\n", 2*N, P, chi2tail(2*N,P))
        printf("{txt}Pm (standardized){col 40}{res}%9.3f{txt}   p = {res}%6.3f\n", Pm, 1-normal(Pm))
        printf("{txt}Choi Z (inverse-normal){col 40}{res}%9.3f{txt}   p = {res}%6.3f\n", Choi, normal(Choi))
        printf("{txt}# rejected at 5%%{col 40}{res}%9.0f\n", nrej)
        st_matrix(statname, (P, chi2tail(2*N,P), Pm, 1-normal(Pm), Choi, normal(Choi), nrej))
        st_matrix(outname, out)
        st_matrixcolstripe(outname, (J(7,1,""), ("id","LMtau","pval","lags","cv10","cv5","cv1")'))
    }
    else {
        tau  = J(N,1,0)
        br1  = J(N,1,0)
        ...
        varp  = 0
        for (i=1; i<=N; i++) {
            res = _xbl_lt2breaks(Y[.,i], pmax, ic)'
            tau[i]  = res[1]
            br1[i]  = res[2]
            br2[i]  = res[3]
            lags[i] = res[4]
            mr = _xbl_ltmoment(T, res[4])
            meanp = meanp + mr[1]
            varp  = varp  + mr[2]
        }
        meanp = meanp/N
