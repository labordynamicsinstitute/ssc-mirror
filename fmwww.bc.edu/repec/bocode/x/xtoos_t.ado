...
if "`met'"=="xtabond" | "`met'"=="xtdpdsys" {
	if "`if'"!="" qui `met' `yvar' `rhsvars' if `timevar'<=`indate' & `if' `in', lags(`lags')
	else qui `met' `yvar' `rhsvars' if `timevar'<=`indate' `in', lags(`lags')
}
else {
	if "`if'"!="" qui reg `yvar' `lyvar' `rhsvars' if `timevar'<=`indate' & `if' `in'
	else qui reg `yvar' `lyvar' `rhsvars' if `timevar'<=`indate' `in'
}
tempvar sample
...
forvalues k=1/`ousd' {	
	...
	************************************************************
	...
	if "`met'"=="" {
		if "`dum'"=="dum" {
			if "`if'"!="" qui xtreg `yvar' `lyvar' `rhsvars' i.`panvar' if `if' & `timevar'<`indate'+`k' & `sample'==1 `in', `options'
			else qui xtreg `yvar' `lyvar' `rhsvars' i.`panvar' if `timevar'<`indate'+`k' & `sample'==1 `in', `options'
		}
		else {
			if "`if'"!="" qui xtreg `yvar' `lyvar' `rhsvars' if `if' & `timevar'<`indate'+`k' & `sample'==1 `in', `fe' `options'
			else qui xtreg `yvar' `lyvar' `rhsvars' if `timevar'<`indate'+`k' & `sample'==1 `in', `fe' `options'
		}
	}
	else if "`dum'"=="dum" {
		if "`if'"!="" qui `met' `yvar' `lyvar' `rhsvars' i.`panvar' if `if' & `timevar'<`indate'+`k' & `sample'==1 `in',`options'
		else qui `met' `yvar' `lyvar' `rhsvars' i.`panvar' if `timevar'<`indate'+`k' & `sample'==1 `in', `options'
	}	
	else if ("`met'"=="xtabond" | "`met'"=="xtdpdsys") {
		if "`if'"!="" qui `met' `yvar' `rhsvars' if `if' & `timevar'<`indate'+`k' & `sample'==1 `in', `fe' `options' lags(`lags')
		else qui `met' `yvar' `rhsvars' if `timevar'<`indate'+`k' & `sample'==1 `in', `fe' `options' lags(`lags')
	}	
	else {
		if "`if'"!="" qui `met' `yvar' `lyvar' `rhsvars' if `if' & `timevar'<`indate'+`k' & `sample'==1 `in', `fe' `options' 
		else qui `met' `yvar' `lyvar' `rhsvars' if `timevar'<`indate'+`k' & `sample'==1 `in', `fe' `options' 
	}	
	...
	if "`evalopt'"=="" qui sum `res2' if `samplein'==0 & `sample'==1 & `timevar'>=`indate'+`k' `in'
	else qui sum `res2' if `samplein'==0 & `sample'==1 & `evalopt'==1 & `timevar'>=`indate'+`k' `in'
	...
	forvalues t=1/`j' {
		if "`evalopt'"=="" qui sum `res2' if `samplein'==0 & `sample'==1 & `timevar'==`indate'+`t'+`k'-1 `in'
		else qui sum `res2' if `samplein'==0 & `sample'==1 & `evalopt'==1 & `timevar'==`indate'+`t'+`k'-1 `in'
	...
	*************************************************************************************
	qui gen `y'=`yvar'
	qui replace `y'=. if `timevar'>=`indate'+`k' `in'

	if "`mcomp'"=="" {
		qui replace `y'=l.`y' if `timevar'>=`indate'+`k' `in'
	}
	else if ("`mcomp'"=="xtabond" | "`mcomp'"=="xtdpdsys") {
		if "`if'"!="" qui `mcomp' `y' if `if' & `timevar'<`indate'+`k' & `samplein'==1 `in'
		else qui `mcomp' `y' if `timevar'<`indate'+`k' & `samplein'==1 `in'
		qui predict `y_ar1'
	}
	else {
		if "`if'"!="" qui `mcomp' `y' L.`y' if `if' & `timevar'<`indate'+`k' & `samplein'==1 `in'
		else qui `mcomp' `y' L.`y' if `timevar'<`indate'+`k' & `samplein'==1 `in'
		qui predict `y_ar1'
	}
	...
	if "`mcomp'"!="" {
		forvalues t = `indate'(1)`cdate' {
			qui replace `y_ar1'=_b[L.`y']*L.`y_ar1'+_b[_cons] if `timevar'==`t'+1 & `y_ar1'==. `in'
		}
	}	
