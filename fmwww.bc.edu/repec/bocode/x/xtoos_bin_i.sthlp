...
{p}The procedure evaluates the prediction performance based on the area under the receiver operator characteristic (ROC) statistic evaluated in both the training sample (labeled as _in) and the out-of-sample (labeld as _oos).
The procedure reports both the estimated ROC and also its standard error (SE).{p_end}
...
{cmd:xtoos_bin_i} {depvar} [{indepvars}] [{it:if}], {opt *o:us(integer)} [{opt r:smpl(integer)}] {opt *k:smpl(integer)}
	[{opt mpr:ob(string)}] 	[{opt ev:alopt(varname)}] [{opt m:et}] [{opt fe}]  
	[{it:model_options}]
...
{synopt:{opt *k:smpl()}}It is always required. Specifies the size {it:k} of each partition or (non-random) subsample, i.e. the number of individuals in each partition, and for which the prediction performance is evaluated. The size {it:k} automatically determines the number of subsamples. If this parameter is defined as zero, the procedure will not perform any partition.{p_end}
