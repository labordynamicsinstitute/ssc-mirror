...
{synoptline}
{syntab:Required}
{synopt :{cmdab:cl:uster(}{it:varname})}Specify the name of the variable indicating the clusters{p_end}

{syntab:Optional}
{synopt :{cmdab:f:amily(}{it:string})}Specifies the distribution family; default is binomial{p_end}
{synopt :{cmdab:l:ink(}{it:string)}}Specifies the link function; the default depends on the specification of {cmd:family}.  The 
default for Gaussian, binomial, and Poisson are {ul:i}dentity, {ul:logi}t, and log, respectively.  See table below for 
combinations of family and link allowed.{p_end}
{synopt :{cmdab:std:err(}{it:string})}Standard error to compute; default is Kauermann-Carroll (kc).  See table below for full list of bias-corrected standard errors available.{p_end}
{synopt :{cmdab:stat:istic(}{it:string)}}Specifies the test. Specifying "statistic(t)" requests the Wald t-test (the default).  Alternatively, the user may specify "statistic(z)" to report 
the Wald z-test instead of the Wald t-test.{p_end}
{synopt :{cmdab:dfm:ethod(}{it:string)}}Specifies the computation of the degrees of freedom for the t-test. More information provided in the explanations seciton below.{p_end}
{synopt :{cmdab:dfs:pec(}{it:integer)}}Only specified for "dfmethod(specify)". More information provided in the explanations section below.{p_end}
...
{pstd}
{hi: Updates as of May 23, 2023}: {cmd:xtgeebcv} now allows for additional degrees of freedom adjustments for the t-statistic, with the default being the number of clusters minus the number of cluster-level 
...
{cmd:If you are using Stata version 15, please install the original Stata Journal version (package sj0599)}, which still works on version 15, but does not allow for factor variables and their interactions.
...
{pstd}
{hi:Original program description:} {cmd:xtgeebcv} computes bias-corrected standard errors for generalized estimating equations (GEEs), specifically 
...
estimates from mixed models) will almost certainly be biased. Given that CRTs are often performed in resource-limited
...
been around for many years, such methods have not yet been implemented in Stata until now.
...
{phang}
The following options are available for {cmd:family} and {cmd:link}. Note that one can also specify {ul:nor}mal in place of {ul:g}aussian.

{center:Family     Link    }
{center:{hline 20}}
{center:{ul:b}inomial   {ul:logi}t   }
{center:{ul:b}inomial   log     }
{center:{ul:b}inomial   {ul:i}dentity}
{center:{ul:p}oisson    log     }
{center:{ul:p}oisson    {ul:i}dentity}
{center:{ul:g}aussian   {ul:i}dentity}
{center:{hline 20}}
...
We would also like to thank Stephen Nash of Karolinska Institutet for suggesting some improvements to the program.
