...
** UPDATES: Sep 10, 2019 - For easier coding and postestimation, require user to 
			...
			Aug 17, 2020 - This version only works on Stata 16.1 or later; use Stata Journal version for functionality with version 15.
...
program define xtgeebcv, eclass
	...
	*newvarlist only contains non-zero parameters
	...
	/* \\\\\ OFFSET VARIABLE (IF USED) \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ */
	if "`e(offset)'" == "" {
		tempname _offsetvar_
	    gen `_offsetvar_' = 0
		local offsetvar = "`_offsetvar_'"
	}
	else if substr("`e(offset)'",1,3)=="ln(" {
	    local offsetvar1 = subinstr("`e(offset)'","ln(","",1)
		local offsetvar = subinstr("`offsetvar1'",")","",3)
		tempname _offsetvar_
		gen `_offsetvar_' = ln(`offsetvar')
		local offsetvar = "`_offsetvar_'"		
	}
	else if substr("`e(offset)'",1,3)!="ln(" {
		local offsetvar = "`e(offset)'"
	    capture drop _offsetvar_
		tempname _offsetvar_
		gen `_offsetvar_' = `offsetvar'
		local offsetvar = "`_offsetvar_'"		
	}
	...
	* because of error(?) in Mata, have to pass the unchanged Beta matrix to Mata
	...
	/* \\\\\\\ ADD OFFSET TO THE END OF NEWVARLIST \\\\\\\\\\\\\\\\\\\\\\\ */
	qui local newvarlist : list newvarlist | offsetvar
	
	/* \\\\\\ SORT THE CLUSTERS THEN CREATE A NEW CLUSTER ID VARIABLE WHICH IS \\\\\\\\\\\\
	\\\\\\\\\ SEQUENTIAL \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ */
	sort `cluster'
	tempname _newclustid
...
// mbn BIAS-CORRECTED  STANDARD ERROR ///////
	...
	//April 5, 2021 - Trace updated to match Morel et al. (2003) (see equation 5)
	phi2 = rowmax((1,trace((nstar-1)/(nstar-p)*(K/(K-1))*naive*correction)/p))
