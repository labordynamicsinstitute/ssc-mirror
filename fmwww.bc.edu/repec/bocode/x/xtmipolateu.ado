...
program def xtmipolateu, rclass
	...
	*/
	...
	if "`generate'" != "" {
		di as err "option generate() not allowed; output overwrites in place"
		exit 198
	}
	...
	cap which mipolate
	if _rc {
		di as err "mipolate not found; install from SSC"
		exit 499
	}
	cap which summarizeby
	if _rc {
		di as err "summarizeby not found; install from SSC"
		exit 499
	}
	...
	// return results                                                           
	egen `gid' = group(`i'), missing
	qui levelsof `gid'
	return scalar N_groups = r(r)
	drop `gid'
