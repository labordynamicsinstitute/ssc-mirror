...
syntax [varlist(numeric)] [if] [in] [fweight] [,  Quantile(real .5) NOTest mss(string) WLSiter(integer 1) Cluster(string) Epsilon(real 1e-7) Silverman]           
marksample touse 
markout `touse'  `cluster', s 
tempname  _e _ae _ep50 _hsi _dens _rhs _y _gv _id _g _w _f _f2 _check _fw A D Vm phi sphi sphi2 numt ct tt _oldsort one _b _V oldest
gettoken _y _rhs: varlist
if ("`_rhs'")~=("") unab _rhs :  `_rhs'
...
sort `_id'  
matrix opaccum `A' = `_rhss' `one' if `touse', group(`_id') opvar(`_w') noconstant 
sort `_gv'  
matrix opaccum `D' = `_rhss' `one' if `touse', group(`_gv') opvar(`_dens') noconstant 
