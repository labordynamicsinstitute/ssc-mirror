*! 1.1.0 Ariel Linden 24 September 2021 // fixed lfit to skip obs with only one observation
										// added r(table)
										// moved "id" from varlist to option 
*! 1.0.1 Ariel Linden 14 August 2021 	// switched from N to e(df_r) for computing CIs 
...
program rmcorr, rclass byable(recall)
	...
	version 11
	syntax varlist(min=2 max=2 numeric) [if] [in] , Id(varlist min=1 max=1 numeric) [Level(real `c(level)') FIGure FIGure2(str asis) ]
		...
		mat E = `n'
		mat X = A\B\C\D\E
		mat rownames X = rho ll ul pvalue n
		mata : X = st_matrix("X")
		...
		if `"`figure'`figure2'"' != ""{
			...
			// gen "count" to allow lfit to fit regress lines for IDs with > 1 observation
			tempvar count
			bys `id': gen `count' = _N
		
			forval x = 1/`:word count `ids''{
				local color  `=runiformint(0,255)' `=runiformint(0,255)' `=runiformint(0,255)'
				local graph `graph'scatter `var1' `var2' if `id'==`:word `x' of `ids'', mcolor("`color'") || lfit `var1' `var2' if `id'==`:word `x' of `ids'' & `count' > 1, lcolor("`color'") ||
			}
			qui twoway `graph', ytitle(`var1') legend(off) ytitle("`ydesc'") xtitle("`xdesc'") `figure2'
		}	
