...
{pstd}
{cmd:srtbayes} performs {cmd:Effect Size} (ES) calculation of educational trials under the assumption of independent errors among pupils under a Bayesian framework; 
this can also be used with schools as fixed effects. This analysis produces ES estimates for both conditional and unconditional model specifications.
...
{phang}
{opt *} Additional Bayesian arguments to be passed to the command such as {cmd:mcmcsize(#) burnin(#) rseed(#) nchains(#)} and custom priors. 
Stata defaults of Bayesian regression models apply; see {opt bayes} prefix ({manhelp bayes BAYES}).
...
{phang}
{opt save} Saves simulation output in two datasets {cmd:(mcmcCond.dta, mcmcUnc.dta)} for the conditional and unconditional models respectively.
