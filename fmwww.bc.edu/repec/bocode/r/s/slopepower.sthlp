...
{pstd}
Load example observational data with no healthy controls (slpower1.dta, available as an ancillary file): 
{p_end}

{pmore}{bf:{stata "use slpower1, clear":. use slpower1, clear}}{p_end}

{pstd} 
Use {cmd: slopepower} to give the sample size for an RCT with annual visits over two years, assuming no dropouts, with 80% power to detect a treatment effect that will eliminate one-third of the slope in the outcome sdmt. The default value of 5% Type I error rate is used.{p_end}

{pmore}{bf:{stata "slopepower sdmt, schedule(1 2) subject(id) time(visit) obs nocontrol effectiveness(0.33)": . slopepower sdmt, schedule(1 2) subject(id) time(visit) obs nocontrol effectiveness(0.33)}} {p_end}

{pstd} 
Extend the trial to 5 years, with no additional interim visits, and assuming that 10% of participants will be lost to follow-up between the visit at year two and the final visit.{p_end}

{pmore}{bf:{stata "slopepower sdmt, schedule(1 2 5) subject(id) time(visit) obs nocontrol eff(0.33) dropouts(0 0 0.1)": . slopepower sdmt, schedule(1 2 5) subject(id) time(visit) obs nocontrol eff(0.33) dropouts(0 0 0.1)}} {p_end}

{pstd} 
Schedule visits every six months in a two year trial, by using the {opt scale} option.{p_end}

{pmore}{bf:{stata "slopepower sdmt, schedule(1 2 3 4) scale(0.5) subject(id) time(visit) obs nocontrol effectiveness(0.33)": . slopepower sdmt, schedule(1 2 3 4) scale(0.5) subject(id) time(visit) obs nocontrol effectiveness(0.33)}} {p_end}


{pstd}
Load example observational data with a healthy control group (slpower2.dta, available as an ancillary file): 
{p_end}

{pmore}{bf:{stata "use slpower2, clear":. use slpower2, clear}}{p_end}

{pstd} 
Use {cmd: slopepower} to give the sample size for an RCT with annual visits over two years, assuming no dropouts, with 80% power to detect a treatment effect that will eliminate one-third of the slope. The {opt scale} option is used to tell Stata that the time variable is a date (recorded in days). {p_end}

{pmore}{bf:{stata "slopepower sdmt, schedule(1 2) scale(365) subject(id) time(vdate) obs case(case) effectiveness(0.33)": . slopepower sdmt, schedule(1 2) scale(365) subject(id) time(vdate) obs case(case) effectiveness(0.33)}} {p_end}

{pstd} 
Calculate the power for a 200 person trial, assuming a dropout rate of 5% per year of those who start the trial. {p_end}

{pmore}{bf:{stata "slopepower sdmt, sched(1 2) scale(365) subject(id) time(vdate) obs case(case) eff(0.33) n(200) drop(0.05 0.05)": . slopepower sdmt, sched(1 2) scale(365) subject(id) time(vdate) obs case(case) eff(0.33) n(200) drop(0.05 0.05)}} {p_end}


{pstd}
Load example RCT data with treated and untreated groups (slpower3.dta, available as an ancillary file): 
{p_end}

{pmore}{bf:{stata "use slpower3, clear":. use slpower3, clear}}{p_end}

{pstd} 
Use {cmd: slopepower} to give the sample size for a three-year RCT with an interim visit at two years, with 80% power to detect the same treatment effect as observed in the previous RCT. Assume a dropout rate of 10% per year. {p_end}

{pmore}{bf:{stata "slopepower sdmt , schedule(2 3) subject(id) time(visit) rct treat(treat) usetrt dropout(0.2 0.1)": . slopepower sdmt , schedule(2 3) subject(id) time(visit) rct treat(treat) usetrt dropout(0.2 0.1)}} {p_end}
