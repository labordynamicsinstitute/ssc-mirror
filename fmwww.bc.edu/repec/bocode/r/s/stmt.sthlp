...
of freedom.Additional timescale interactions can be added using |
in the following way: {cmd: timeint(t1:t2 2:4 | t1:t3 2:2)}.
...
{pmore}
{cmd:timeintknots(2 5 : 50 60 70 | 2 3 5 : 2)}

{pmore}
Note that knots should be specified on the untransformed
time scale.
...
{pmore}
{cmd:timeintbknots(0 7 : 25 95 | 0 7 : 1 12)}

{pmore}
Note that both the {cmd:timeint()} options and {cmd:timeintknots()} options can be used with the
{cmd:timeintbknots()} option and that knots in the {cmd:timeintbknots()} should be specified
on the untransformed time scale.
...
{phang}
Therese M-L Andersson, Karolinska Institutet, Stockholm, Sweden.

{phang}
Michael Crowther, Karolinska Institutet, Stockholm, Sweden.
