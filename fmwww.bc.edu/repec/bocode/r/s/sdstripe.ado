program define sdstripe
version 10
   syntax varlist (min=1) , GENerate(string) [ SYMbols(string) XT XTSPellsep(string) XTDursep(string) OFFset(real 0)]
   if "`symbols'"=="" {
   ...
   foreach a in `varlist' {
     qui replace `generate' = `generate' + substr("`symbols'",`a'+`offset',1) if !missing(`a') & `a'+`offset'>0
     qui replace `generate' = `generate' + "." if missing(`a') | (`a'+`offset')<=0
     }
   }
   else {
