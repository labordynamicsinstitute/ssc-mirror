...
{phang} {cmd:out_sample_x}{cmd:(}{it:filename}{cmd:)} requests to provide a dataset as {it:filename} containing the testing observations over which estimating predictions. This dataset contains only predictors.

{phang} {cmd:out_sample_y}{cmd:(}{it:filename}{cmd:)} requests to provide a dataset as {it:filename} containing the outcome of the testing observations over which estimating predictions. This dataset must contain only the outcome. When the outcome is unknown, this can be inserted as a variable with all missing values.     
...
{phang} {cmd:Elastic-net} (option "elasticnet"):

{phang} -> {it:e(OPT_ALPHA)}: scalar. It is the so-called shrinkage parameter, that is, a constant that multiplies the penalty term. 
A value of this parameter equal to zero corresponds to ordinary least squares. 

{phang} -> {it:e(OPT_L1_RATIO)}: scalar. It is the elastic-net mixing parameter, varying between 
zero (Lasso regression) and one (Ridge regression). Intermediate values of this 
parameter weigh differently the Lasso and Ridge penalization terms.
...
{phang} -> When running {cmd:r_ml_stata}, one has to prepare in advance three datasets: 
...
{phang} *** {bf:RUNNING A REGRESSION TREE} ***

{phang} * {bf:Download these {browse "https://www.dropbox.com/sh/5ficl2jfsm8efk1/AAAmLu5zjmbSJx7g-gq5mduja?dl=0":datasets}, by putting them in the same directory:}

{phang} * Dataset 1: "r_data" -> the complete dataset

{phang} * Dataset 2: "r_data_x" -> the test dataset containing "only" the features X 

{phang} * Dataset 3: "r_data_y" -> the test dataset containing "only" the outcome y  

{phang} * {bf:Load the complete dataset}

{phang} . use "r_data" , clear

{phang} * {bf:Run the "r_ml_stata" command with model option "tree"}

{phang} #delimit ;
       
{phang} . r_ml_stata y x1 x2 x3 x4 , mlmodel("tree") in_prediction("in_pred") cross_validation("CV") out_sample_x("r_data_x") out_sample_y("r_data_y") out_prediction("out_pred") seed(10) n_folds(10) save_graph_cv("graph_cv");
