...
*TODO make eksposure optional
*TODO Handle mixed models
	...
	foreach outcome in `outcomes' {
		foreach exposure in `exposures' {
			...
			foreach adj in `adjustments' {
				...
				*mata _row = _row, `"`_cmd' `outcome' `exposure' `adj' `s(if)' `s(in)' `_options'"'
				*mata _row = _row, `"`_cmd' `outcome' `exposure' `adj' `_rest'"'
			}
