...
**!          number of which can be set by global macro INSHELL_ESCTAB_SIZE, ranging from
...
mata : inshell_parse_syntax2()  // parse the syntax through Mata to avoid parsing in Stata
...
if ("`parser'" == "diagnostics") {
	if (lower(c(os)) != "windows") {
		...
		display as error ///
			_n "{p 2 1} >>> Sorry, but {bf:inshell} does not currently have a diagnostics mode for {bf:Microsoft Windows}. Stay tuned."
		exit 1
	}
}
...
if ("`parser'" == "cd") {
	if (`"`2'"' == "-") {
		display as error ///
			"{p 2 1} >>> previous directory switching is not supported by this {bf:cd} wrapper" _n
		exit 1
	}
...
if ("`parser'" == "command") {
	...
	// check that the alternative horizontal tab (ASCII character 9) size is properly set
	capture assert inrange(${INSHELL_ESCTAB_SIZE}, 1, 8)
	if (!_rc) {
		capture confirm integer number ${INSHELL_ESCTAB_SIZE}
		if (_rc) global INSHELL_ESCTAB_SIZE = 4
	}
	else global INSHELL_ESCTAB_SIZE = 4
	...
	if (lower(c(os)) != "windows") {
		...
		if ((c(mode) == "batch") & (c(stata_version) < 17)) {
			display as error ///
				"{p 2 1} >>> {bf:inshell} will not function in batch mode on {bf:Microsoft Windows}. " ///
				"This is a limitation of any version of Stata prior to the October 2022 update of Stata 17, including versions 16 and prior. You must update to Stata version 17 or newer to use shell commands in batch mode."
			exit 990
		}
	...
	if (!_rc) {                                        // if there's no problem with that...
		tempname Q       // create a temporary name for the matrix from which to return macros
		if (!missing("$INSHELL_ENABLE_ALLCHARDISP")) local p s
		mata : M = inshell_process_file3("`stdout'")             // process the stdout in Mata
		mata : for (i=1; i<=rows(M); i++) `p'printf("%s\n", M[i]) // print the matrix via Mata
		mata : `Q' = select(M , M :!= "")   // create a matrix containing only non-blank lines
	...
	if (!_rc) {                                        // if there's no problem with that...
		tempname R       // create a temporary name for the matrix from which to return macros
		mata : N = inshell_process_file3("`stderr'")                       // process the file
		mata : st_local("displayrows", strofreal(rows(N)))      // get the number of rows of N
		if (`displayrows' > 0) {
			mata : `R' = select(N , N :!= "") // create a matrix containing only non-blank lines
			...
			return local stderr = subinstr(ustrtrim(`"`macval(errormessage)'"'),  char(10), char(32), .)
		}
