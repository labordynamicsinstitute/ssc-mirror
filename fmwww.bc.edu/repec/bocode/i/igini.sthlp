...
{* 4 May 2018}{...}
{hline}
help for {hi:iGini}{right:Tim F. Liao (May 2018)}
{hline}
...
{title:Syntax}
	...
	{cmd:iGini} {it:varname} [{it:weights}] [{cmd:if} {it:exp}] [{cmd:in} {it:range}]
...
{p 4 4 2} 
{cmd:iGini} computes a refined Gini inequality index decomposition by decomposing the overall index into individual contributions to the Gini and further into the between-group and within-group components for each individual case. In addition to the main outcome variable, the by-group is typically required. Sampling weight is optional, and can be included as an {cmd:aweight}. The program {cmd:iGini} generates a new dataset of the same size as the input data, containing six variable: an id variable bearing the case number ordered in the same sequence as the original data, a group variable that records the group membership number, the iGini individual contributions to the Gini index, the iGini between-group component, the iGini within-group component, and a quantile variable reflecting the membership in either the lower or the upper half of the outcome variable distribution within each level of the group variable. The last quantile variable can be useful when used jointly with iGini components.

{title:Techical Details}
{phang}

{p 4 4 2} 
{cmd:iGini} computes a refined Gini inequality index. The relation between an individual iGini {it:g}_{it:i} component and the overall Gini idex {it:G} is
...
{p 4 4 2} 
{cmd:iGini} is computation-intensive. To surpass the upper limit of the number of cases in Stata, the ado uses Stata's Mata, and can handle as many cases as your system allows. Thus, computation time can be long if you have more than ten thousand cases. To speed up, you may consider using {cmd: Stata MP} or the ado {cmd:parallel}.

{title:Options}
{phang}

There is only one "{it:option}" implemented in the current version of {cmd:iGini}. It is required to run {cmd:iGini}.
...
{p 4 8 2}{cmd:. iGini y, by(Sex)}

{p 4 8 2}{cmd:. iGini y [aw = weight], by(Race)}

{p 4 8 2}{cmd:. iGini inc [aw = weight], byg(SexRace)}

{phang}
{it: For computing iGini only without group-based decomposition}:

{p 4 8 2}{cmd:. gen one = 1}

{p 4 8 2}{cmd:. iGini outcome [aw = weight], by(one)}
...
{p 4 8 2}{cmd:. iGini mpg, by(origin)}
...
{p 4 4 2}The author acknowledges the 2017-2018 fellowship support by the Center for Advanced Studies in the Behaviorial Sciences. Comments and suggestions will be welcome for debugging and updating {cmd:iGini}.
