...
#delimit ;
capture program drop imwsub;
program define imwsub, sortpreserve;
version 9.2;
...
tempvar wvar ;
qui gen `wvar' = 0 ;
qui replace `wvar' = -`bexp'*(`p2'[1]/`p1'[1]-1)                                                         if `class'==1; 
if `nn1' > 1 {;
forvalues i = 2/`nn1' {;
local j=`i'-1;
 qui replace `wvar' = `basev`i'' - (`bexp'-`se`j'')*(`p2'[`i']/`p1'[`i']-1)                             if (`class'==`i') ;
};
};
if `nn1'>=1  qui replace  `wvar' = `basev`nn'' - (`bexp'-`se`nn1'')*(`p2'[`nn']/`p1'[`nn'] - 1)           if (`class' == `nn' )  ;

if `bun1'==1 qui replace `wvar' = `wvar'/ `hsize' ;



cap drop __imwsub;
qui gen  __imwsub=`wvar';
end;
