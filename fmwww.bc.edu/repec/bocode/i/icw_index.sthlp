...
{pstd}
 ...
 {pstd}
 Beware that if varlist contains lots of missing values (or rather if each observation has a specific pattern of missing values) the code can take a long time as the number of potential combinaison can be exponential.  
 For instance,  an index composed of 10 variables has a maximum number of combinations equals to 1023. Typically when your varlist is composed of more than 10 variables it could be useful to verify if your missing values are equally spread out (if every observation has the same number of missing values) or if each observation have a unique missing pattern. In the latter case, the code will take a long time. A warning will be displayed if more than 1000 matrix needs to be inverted.   
...
{cmd:. icw_index math economics and biology, gen(GPA)}
