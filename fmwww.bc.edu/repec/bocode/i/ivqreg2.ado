...
conv_nrtol(real 1e-6) tracelevel(string) quickd from(string) MUsigma  ///
 winitial(string) ONEstep TWOstep]
...
di
di
if ("`winitial'"=="") local winitial="identity"
...
instruments(main 2: `instruments')  winitial(`winitial')  from(`from') `quickd' ///
technique(`technique') tracelevel(`tracelevel') conv_maxiter(`conv_maxiter') ///
conv_ptol(`conv_ptol') conv_vtol(`conv_vtol')  conv_nrtol(`conv_nrtol')  `onestep' `twostep'  ///
...
instruments(main 2: `instruments')   winitial(`winitial')  from(`from') `quickd' ///
technique(`technique') tracelevel(`tracelevel') conv_maxiter(`conv_maxiter') ///
conv_ptol(`conv_ptol') conv_vtol(`conv_vtol') `onestep' `twostep'  ///
deriv(main/xb =                           -1/( {xg: }+{g0})   ) ///                           
deriv(main/b0 =                           -1/( {xg: }+{g0})   ) ///
deriv(2/xb = -sign(`_y' - {xb: }-{b0})/( {xg: }+{g0})   ) ///
deriv(2/b0 = -sign(`_y' - {xb: }-{b0})/( {xg: }+{g0})   ) ///
...
di as text "                              MM-QR regression results"
di
di as txt "Number of obs = "  _continue
di as result   `enne'
di as txt "GMM criterion Q(b) = "  _continue
di as result e(Q)
if (e(J_df)>0) {
di as txt "J-test = "  _continue
di as result e(J) _continue 
di as txt ", df = " _continue
di as result e(J_df) _continue
di as txt ", p-value " _continue
di as result chi2tail(e(J_df),e(J))
}
if ("`ls'"!="") {
mat `G'=`b'
mat `O'=`V_location'
ereturn post `G' `O', depname("`_y'")
...
ereturn post `bpost' `V', obs(`enne') depname("`_y'")
...
ereturn local cmd ivqreg2
ereturn local cmdline ivqreg2  `0'
