...
quiet{
    * here I identify the observations with available info to speed up the code. 
tempvar  `generate'_mi
egen ``generate'_mi'=rowmiss(`varlist')
local c= wordcount("`varlist'")
replace ``generate'_mi'=(``generate'_mi'==`c')

*** establishing the pattern of missing values: for each observation, I create a tempvar mi_pattern that indicates which variables are non-missing
...
if `error'>1000 { 
	...
	noisily dis  as result "Beware the command needs to inverse `error' matrices: it will take time."
}
local c=1
