*! version 1.3.2   Leo Ahrens   leo@ahrensmail.de
...
// handle sudan special case 
local sudanspecial = 0
if strpos("`gen'","iso3n") {
	count if iso3n==729 `__keepif'
	if r(N)>0 local sudanspecial = 1
}
...
if strpos("`gen'","iso3n") & "`nolabel'"=="" {
	...
	label define __isolabels__ 832 "Jersey" 833 "Isle of Man" 834 "Tanzania" 840 "United States" 850 "US Virgin Islands" 854 "Burkina Faso" 858 "Uruguay" 860 "Uzbekistan" 862 "Venezuela" 876 "Wallis and Futuna Islands" 882 "Samoa" 887 "Yemen" 894 "Zambia" 891 "Serbia and Montenegro" 810 "Soviet Union" 278 "German Democratic Republic", add
...
if `sudanspecial'==1 {
	...
	if !_rc {
		count if year<2011 
		if r(N)>0 {
			replace iso3n = 736 if year<2011 & iso3n==729
			noisily di as error "The iso3n code of (North) Sudan changed from 736 to 729 in 2011. Please be aware that Sudan has more than one iso3n code in your data."
		}
		
	}
	else {
		noisily di as error "All observations from (North) Sudan are given the iso3n code '729'. This code officially changed from 736 to 729 in 2011. This may not be reflected in your data. Please adjust manually if required."
	}
}
