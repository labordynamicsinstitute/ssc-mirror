...
{p2colset 5 22 23 2}{...}
{p2col :{cmd:ppml_fe_bias} {hline 2}} Bias corrections for Poisson Pseudo-Maximum Likelihood (PPML) estimation of gravity models with two-way and three-way fixed effects.{p_end}
{p2colreset}{...}
...
{pstd}
{cmd:ppml_fe_bias}  implements analytical bias corrections described in {browse "https://arxiv.org/abs/1909.01327":Weidner & Zylkin (2021)} for PPML "gravity"
regressions with two-way and three-way fixed effects, as are commonly used with international trade data and other types
of spatial flows. As shown by Weidner & Zylkin (2021), when the time dimension is fixed, the point estimates produced by
...
and the cluster-robust variance estimator itself has a downward bias that is also of order 1/N. For two-way PPML, only the standard errors are biased.
{p_end}
...
{p 6 6 2} •  "Two-way" and "three-way" gravity models are very popular for the estimation of international data and other types of spatial flows. 
The two-way model features origin-time and destination-time fixed effects. The three-way model then adds an origin-destination fixed effect.{break}
 •  Though PPML estimates of the three-way model are consistent, they suffer from an asymptotic bias due to the slow convergence of the origin-time and destination-time fixed effects. This bias causes confidence intervals to be incorrectly centered even in moderately large samples.{break}
 ...
 •  Two-way PPML gives estimates that are asymptotically unbiased but still suffers from downward-biased standard errors in much the same way as the three-way model.{break}
 •  This command implements analytical bias corrections that address each of these issues. These corrections are based on Taylor expansions described in Weidner & Zylkin (2021).{p_end}

{pstd}PPML estimation of gravity models with two-way and three-way fixed effects is very popular in the study of international trade
and other similar applications that involve bilateral flows (such as urban commuting or interregional migration).
The two-way gravity model may be written as{p_end}
...
{pstd}Weidner and Zylkin (2021)'s analysis identifies several econometric issues that arise with the three-way model, but their method for correcting the
...
asymptotic bias of two-way nonlinear models. The three-way gravity model is a more complicated model
than the ones studied in Fernández-Val & Weidner (2016), but Weidner & Zylkin (2021) nonetheless demonstrate
...
{pstd}Weidner and Zylkin (2021) provide a more detailed derivation of the bias in b based on a second-order Taylor series expansion
...
b-coefficients obtained using two-way PPML do not suffer from any asymptotic bias, however,
...
Zylkin (2021), the estimated standard error is also found to be biased: the bias-corrected standard error is about
...
0.109). While these results were obtained for a moderate number of countries (I=J=65), it is important to note that Weidner & Zylkin (2021)’s results imply that the magnitudes of
...
{pstd}• Other methods for correcting standard errors in this setting: Pfaffermayr (2021) {p_end}
...
{phang}
Pfaffermayr, M. (2021), “Confidence intervals for the trade cost parameters of cross-section gravity models,” 
Economics Letters.{p_end}
...
{phang}
Weidner, M. & Zylkin, T. (2021), “Bias and Consistency in Three-Way Gravity Models”, arXiv preprint
