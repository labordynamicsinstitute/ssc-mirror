...
*/
capture program drop prevent10i
program prevent10i, rclass
	version 13.0
	syntax ,			  				///
		...
		[INClude]
				...
				tempname cvd10i ascvd10i hf10i
				...
				********************				
				...
				scalar `cvd10i' = exp(`cvd10i') / (1 + exp(`cvd10i'))
				scalar `cvd10i' = round(`cvd10i' * 100, 0.01) 	
				...
				********************
				...
				scalar `ascvd10i' = exp(`ascvd10i') / (1 + exp(`ascvd10i'))
				scalar `ascvd10i' = round(`ascvd10i' * 100, 0.01) 				
				...
				********************
				...
				scalar `hf10i' = exp(`hf10i') / (1 + exp(`hf10i'))
				scalar `hf10i' = round(`hf10i' * 100, 0.01)
			
                // display results
                di 
                di as txt "   Estimated 10-Year ASCVD Risk: " as result %6.2f `ascvd10i' "%"
                di as txt "   Estimated 10-Year CVD Risk:   " as result %6.2f `cvd10i' "%"				
                di as txt "   Estimated 10-Year HF Risk:    " as result %6.2f `hf10i' "%"		
				
				if "`include'" != "" {
					di
					if inrange(`age',30,79) & inrange(`chol',130,320) & inrange(`hdl',20,100) & inrange(`sbp',90,200) ///
				...
				// saved values
				return scalar hf10i = `hf10i'
				return scalar cvd10i = `cvd10i'			
				return scalar ascvd10i = `ascvd10i'
