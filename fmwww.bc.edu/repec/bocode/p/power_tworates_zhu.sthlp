...
{p2coldent:* {opth overdisp:ersion(numlist)}}negative binomial overdispersion parameter (>0 for overdispersion); default is {cmd:overdispersion(0)}{p_end}
...
{synopt :{opt overdispersion}}negative binomial overdispersion parameter{p_end}
...
{phang}
{opth overdispersion(numlist)} specifies what Stata calls the negative binomial "overdispersion parameter",  
alpha in {manhelp nbreg R: nbreg}, but what Zhu and Lakkis (2014) call the "dispersion parameter", k. 
For a group where there is equal follow-up or exposure per subject, and  
#Events has Variance = r*(1 + k*r) where r is the expected number of events per subject.
...
{pstd}
For ideas on what value of overdispersion to assume, (i) use {manhelp nbreg R: nbreg} on raw data, (ii) read the references.
...
that the common [over]dispersion parameter k is known. Before a sample size formula for such a test becomes
...
{pstd}
    Extending the above, by varying the amount of overdispersion{p_end}
{phang2}{sf:. }{stata "power tworates_zhu, r1(3) irr(0.7) overdispersion(0 (0.4) 1.2)"}
...
{pstd}
    ...
    5% significance level, 80% power, N1=N2, and overdispersion k=0.4 {p_end}
{phang2}{sf:. }{stata "power tworates_zhu, r1(0.8) irr(0.85) duration(0.75 1.5) overdispersion(0.4)"}

{pstd}
    Compute sample sizes for a range of IRRs and powers, graphing the results{p_end}
{phang2}{sf:. }{stata "power tworates_zhu, r1(0.8) irr(0.5 (0.05) 0.85) duration(0.75) overdispersion(0.4) power(0.8 0.9) graph(y(N) x(IRR)) table"}

{pstd}
    Reproduce Zhu and Lakkis (2014) Table I (Methods 2 & 3). 
	(Note: sort order is different, but Ns are the same){p_end}
{phang2}{sf:. }{stata "power tworates_zhu, r1(0.8 1.0 1.2 1.4) irr(0.85 1.15) duration(0.75) overdispersion(0.4 0.7 1.0 1.5) power(0.8) varmethod(2 3) table(overdispersion IRR r1 N1 varmethod)"}
...
{pstd}
    Extending the above, by varying the amount of overdispersion{p_end}
{phang2}{sf:. }{stata "power tworates_zhu, r1(3) irr(0.7) n1(49) overdispersion(0 (0.4) 1.2)"}
        
{pstd}
    Compute the power required to detect an IRR of 0.85 given a control-group rate of 0.8 per subject per year,  
    average follow-up of 0.75 years for scenario 1 (and 1.5 years for scenario 2), 
    5% significance level, N1=N2=1311, and overdispersion k=0.4 {p_end}
{phang2}{sf:. }{stata "power tworates_zhu, r1(0.8) irr(0.85) n1(1311) duration(0.75 1.5) overdispersion(0.4)"}

{pstd}
    Compute power for a range of IRRs, graphing and tabling the results{p_end}
{phang2}{sf:. }{stata "power tworates_zhu, r1(0.8) irr(0.75 (0.05) 0.9) n1(1311) duration(0.75) overdispersion(0.4) graph(y(power) x(IRR) ylab(0 (0.1) 1)) table"}
...
{synopt:{cmd: r(r1)}}control-group rate (per unit of time){p_end}
{synopt:{cmd: r(r2)}}experimental-group rate (per unit of time){p_end}
...
{synopt:{cmd: r(overdispersion)}}negative binomial overdispersion parameter{p_end}
