...
version 12.1
#delimit ;
syntax varlist(min=2 max=2) [if] [in], [MAXBW(numlist max=1) MINBW(real 0) C(real 0) PZRANGE(numlist min=2 max=2) 
...
MCUSTOM9(string) MCUSTOM10(string) nodisplay(integer 0) LEVEL(string)] ;
#delimit cr
...
if "`maxbw'" == "" {
	local rot = max(abs(`pz1'-`c'),abs(`pz2'-`c'))/3
	di as error "option maxbw() required. 
	di as text "A rule-of-thumb you might want to consider is one-third of the distance from the threshold to the furthest boundary of the placebo zone, which would be " as result %5.3f `rot'
		exit 198
}
...
if "`level'" == "" {
	local level = 95  // default 95% level of confidence intervals
}
local alpha = 0.5*(100-`level')/100
capture reg `1' `2' if `2' > r(max), level(`level')  //this is just to check whether level is correctly specified, there will be no observations
if _rc == 198 {
	di as err "Option level incorrectly specified. Must be between 10 to 99.99 and have no more than two decimal palces."
	exit 198
}
...
if (`pz2' - `maxbw')-(`c' + `Lbw') <= 0 & (`c' - `Rbw') <= (`pz1' + `maxbw') {
	di as error "maxbw is too large - there will be no placebo thresholds."
	exit 198
}

if ("`: word 1 of `vce''"  == "bootstrap" | "`: word 1 of `vce''"  == "jackknife") & ("`weight'" != "" | "`kernel'" == "triangular" | ("`collapse'" == "`weight'" & "`collapse'" != "")) {
...
forvalues i = `lendob'(-`pzstepsize')`lstart' {
	local num_p = `num_p'+1  //manually count the possible number of placebo thresholds - in cases where placebo zone is symmetric and pzstepnum is an odd number, can differ from maxiter
}
...
if `num_p' != `maxiter' {
	local maxiter = `num_p'
	di as text _newline "to ensure even step sizes, the number of placebo steps has been set to `maxiter'"
}
...
*ESS and alt-p - first as contiguous (placebo zone is all on one side of the threshold)
if `LHS'==0 | `RHS'==0 {
	local B = 1
	...
	local t_crit_alt = abs(invt(`alt_t_df',`alpha'))
}

if `LHS'==1 & `RHS'==1 {  //now the case where there is placebo zone on both sides of the threshold
	qui count if ___5 ==1
	local bn = r(N)
	local bn_minus1 = `bn'-1
	local B = 1
...
local t_crit = abs(invt(e(df_r),`alpha'))  //for constructing conventional confidence intervals  
local stat = "t"
...
if `nodisplay' == 0 {  //nodisplay is an undocumented option that suppresses the main results table. Invoked in our companion program pzms_sim
	...
	di as text "Effective sample size of placebo estimates from optimal model: " as result _col(25) %5.3f `ess' 
	...
	di as text _col(53) "vce:" as result "{ralign 24:`e(vcetype)'}"
	di as text "{hline 19}{c TT}{hline 60}"
	di as text "{ralign 18:Inference method}"  _col(19) " {c |} " _col(24) "Coef."  _col(33) `"Std. Err."'  _col(46) "`stat'"    _col(52) "P>|`stat'|"   _col(61) `"[`level'% Conf. Interval]"'
...
capture ereturn scalar pz_altt = `t_crit_alt'
capture ereturn scalar pz_altdf = `alt_t_df'
