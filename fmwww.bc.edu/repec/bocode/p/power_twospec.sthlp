...
{p2colset 5 22 23 2}{...}
{p2col:{hi:power twospec} {hline 2}} Power and sample-size analysis for specificity between two diagnostic tests in two independent samples  {p_end}
{p2colreset}{...}
...
{p 8 43 2}
{opt power twospec} {it:spec1} {it:spec2}
...
{pstd}
{opt power twospec} computes sample size or power for independent two-sample specificity of a diagnostic test,
...
{pstd}
{opt power twospec} implements the naive prevalence inflation method introduced by Obuchowski and Zhou (2002). 
...
{phang2}{cmd:. power twospec 0.70 0.90, power(0.90) prev(0.10)}

{pstd}
    Computing sample size for a range of prevalence rates {p_end}
{phang2}{cmd:. power twospec 0.70 0.90, power(0.90) prev(0.10(.10).90)}
...
{phang2}{cmd:. power twospec (0.60(0.05).80) 0.90, power(0.90) alpha(0.01 0.05) prev(0.10) frac(0.33) graph}
...
{phang2}{cmd:. power twospec 0.70 0.90, n(50)}

{pstd}
    Same as above, but change the prevalence to 0.20 and use a one-sided test {p_end}
{phang2}{cmd:. power twospec 0.70 0.90, n(50) prev(0.20) onesided }

{pstd}
    Same as above, but apply a range prevalence rates {p_end}
{phang2}{cmd:. power twospec 0.70 0.90, n(50) prev(0.20 0.50 0.70) onesided }

{pstd}
	Compute powers for a range of null sensitivities and total sample sizes, 
	graphing the results{p_end}
{phang2}{cmd:. power twospec (0.60(.10)0.80) 0.90, n(5(5)80) onesided graph}


{title:Stored results}

{pstd}
{cmd:power twospec} stores the following in {cmd:r()}:
...
{synopt:{cmd:r(method)}}{cmd:twospec}{p_end}
...
{marker citation}{title:Citation of {cmd:power twospec}}

{p 4 8 2}{cmd:power twospec} is not an official Stata command. It is a free contribution
to the research community, like a paper. Please cite it as such: {p_end}

{p 4 8 2}
Linden, A. 2022. POWER TWOSPEC: Stata module to compute power and sample size for specificity between two diagnostic tests in two independent samples
...
{helpb power onespec} (if installed), {helpb power twosens} (if installed), {helpb power pairsens} (if installed), 
