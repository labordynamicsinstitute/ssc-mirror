...
local r_obs = (`coef'/`sd')/sqrt((`coef'/`sd')^2 +`N'-`Ncov'-3)
 ...
 local nr_con = -1 * `r_con'
  ...
  dis "------------------"
   ...
   dis "------------------"
  dis "Impact Threshold for an Omitted Confounding Variable"
 dis ""
  if abs((`coef' - `nu')/`sd') >= abs(`criticalt')  {
  
  if `itcv'>0 {
  dis "An omitted variable would have to be correlated at `r_con' with the outcome and at `r_con' with the predictor" _newline "of interest (conditioning on observed covariates) to invalidate an inference." _newline "Correspondingly the minimum impact to invalidate an inference for a"_newline "null hypothesis of `nu' effect is `r_con' x `r_con'=`impact_'"
}
else {
  dis "An omitted variable would have to be correlated at `r_con' with the outcome and at `nr_con' with the predictor" _newline "of interest (conditioning on observed covariates. Signs are interchangeable) to invalidate an inference." _newline "Correspondingly the minimum impact to invalidate an inference for a"_newline "null hypothesis of `nu' effect is `r_con' x `nr_con'=`impact_'"
  ...
  if `itcv'>0 {
  dis "An omitted variable would have to be correlated at `r_con' with the outcome and at `r_con' with the predictor" _newline "of interest (conditioning on observed covariates) to sustain an inference." _newline "Correspondingly the impact of an omitted variable (as defined in Frank 2000) is" _newline "`r_con' x `r_con'=`impact_' to sustain an inference."
}
else {
  dis "An omitted variable would have to be correlated at `r_con' with the outcome and at `nr_con' with the predictor" _newline "of interest (conditioning on observed covariates. Signs are interchangeable) to sustain an inference." _newline "Correspondingly the impact of an omitted variable (as defined in Frank 2000) is" _newline "`r_con' x `nr_con'=`impact_' to sustain an inference."
...
local threshold = `criticalt' * `sd'
  ...
  local recase = round(`N' * `bias'/100,1)
    ...
    dis "------------------"
  dis "% Bias Necessary to Invalidate/Sustain the Inference" 
  dis ""
 ...
 local recase = round(`N' * `bias'/100,1)
  ...
  dis ""
  dis "Note:"
  dis "For non-linear models, the impact threshold for an omitted confounding variable should not be used."
  dis "The % bias calculation is based on the original coefficient, compare" _newline "with the use of average marginal effects as in the [konfound] command."
