...
{p2colset 5 23 24 2}{...}
{p2col:{hi:power pairsens} {hline 2}} Power and sample-size analysis for sensitivity between two diagnostic tests in a paired-sample   {p_end}
{p2colreset}{...}
...
{p 8 43 2}
{opt power pairsens} {it:sens1} {it:sens2}
...
{pstd}
{opt power pairsens} computes sample size or power for sensitivity between two diagnostic tests in paired data,
...
{pstd}
{opt power pairsens} computes sample size according to Miettinen (1968), then implements the crude prevalence inflation method 
...
{phang2}{cmd:. power pairsens 0.70 0.90} 

{pstd}
    Same as above, using a power of 90% and a prevalence of 0.10 {p_end}
{phang2}{cmd:. power pairsens 0.70 0.90, power(0.90) prev(0.10)}

{pstd}
    Computing sample size for a range of prevalence rates {p_end}
{phang2}{cmd:. power pairsens 0.70 0.90, power(0.90) prev(0.10(.10).90)}
...
{phang2}{cmd:. power pairsens (0.60(0.05).80) 0.90, power(0.90) alpha(0.01 0.05) prev(0.10) graph}
...
{phang2}{cmd:. power pairsens 0.70 0.90, n(50)}

{pstd}
    Same as above, but change the prevalence to 0.20 and use a one-sided test {p_end}
{phang2}{cmd:. power pairsens 0.70 0.90, n(50) prev(0.20) onesided }

{pstd}
    Same as above, but apply a range prevalence rates {p_end}
{phang2}{cmd:. power pairsens 0.70 0.90, n(50) prev(0.20 0.50 0.70) onesided }

{pstd}
	Compute powers for a range of null sensitivities and total sample sizes, 
	graphing the results{p_end}
{phang2}{cmd:. power pairsens (0.60(.10)0.80) 0.90, n(5(5)80) onesided graph}


{title:Stored results}

{pstd}
{cmd:power pairsens} stores the following in {cmd:r()}:
...
{synopt:{cmd:r(method)}}{cmd:pairsens}{p_end}
...
{p 4 8 2}{cmd:power pairsens} is not an official Stata command. It is a free contribution
to the research community, like a paper. Please cite it as such: {p_end}

{p 4 8 2}
Linden, A. 2022. POWER PAIRSENS: Stata module to compute power and sample size for sensitivity between two diagnostic tests in a paired-sample
