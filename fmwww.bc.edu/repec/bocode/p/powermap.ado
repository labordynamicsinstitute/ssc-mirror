...
program define powermap, rclass
version 14.2
...
quietly{
	...
	* Check installed packages
	cap which heatplot
	if _rc {
		di as err "-heatplot- package must be installed first. (Click {stata ssc install heatplot, replace:here} to install)"
		exit 499
	}
	
	cap which colorpalette
	if _rc {
		di as err "-palettes- package must be installed first. (Click {stata ssc install palettes, replace:here} to install)"
		exit 499
	}
	...
	*******************************************
	...
	if (`n'!=-1 & `power'==-1) {
		...
		* Loop over all the pre-post combinations
		forval pre_r_pc=1/`rounds' { 
			forval post_r_pc=1/`rounds' { 
				...
				* Replace variables for heat plots
		...
		* Define scalars and locals for r()
		return local method = "`method'"
		return scalar N = `n'
		return scalar mde = `mde'
		return scalar rho = `rho'
		return scalar ptreat = `ptreat'
		...
		* Save graph (if -savegraph- option is specified)
		if "`savegraph'"!="" {
			graph save "`savegraph'", replace
		}
		
		* Save power calculations data (if -savedata- option was selected)
		if "`savedata'"!="" {
