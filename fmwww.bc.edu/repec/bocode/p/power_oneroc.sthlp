{smcl}
{* *! version 2.0.0 07nov2022}{...}
{title:Title}

{p2colset 5 21 22 2}{...}
{p2col:{hi:power oneroc} {hline 2}} Power analysis for a one-sample ROC analysis  {p_end}
{p2colreset}{...}
...
[{cmd:,} {opth p:ower(numlist)} {opth a:lpha(numlist)} {opth ratio(numlist)} {opt onesid:ed} {opt han:ley} {opt gr:aph}[{cmd:(}{it:{help power_optgraph##graphopts:graphopts}}{cmd:)}] ]
...
{p 8 43 2}
{opt power oneroc} {it:auc0} {it:auc1}
[{cmd:,} {opth n(numlist)} {opth n1(numlist)} {opth n0(numlist)} {opth a:lpha(numlist)} {opth ratio(numlist)} {opt onesid:ed} {opt han:ley} {opt gr:aph}[{cmd:(}{it:{help power_optgraph##graphopts:graphopts}}{cmd:)}] ]
...
or the method described by Hanley and McNeil (1982) for continuous data only, which is based on the Mann-Whitney version of the rank-sum test.
{cmd:power oneroc} is loosely based upon the code used in the SAS® macro ROCPOWER (Zepp 1995). 
...
{pstd}
	...
	variances using the Obuchowski formula; assume a 5% significance level and 80% 
...
{phang2}{cmd:. power oneroc 0.50 0.70, power(0.90) onesided hanley ratio(4)}
...
{phang2}{cmd:. power oneroc 0.50 (0.60(0.05).95), power(0.90) onesided hanley ratio(4) alpha(0.01 0.05) graph}
...
{pstd}
    ...
    detect an AUC of 0.70 given a null AUC of 0.50 and computing variances using the 
	Obuchowski method; assume a 5% significance level (the default){p_end}
...
{p 4 8 2} Online: {helpb roc}, {helpb power}, {helpb power tworoc} (if installed){p_end}
