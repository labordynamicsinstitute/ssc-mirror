...
syntax [namelist(min=1)] [if] [in] , cmd(string) [cmdops(string) cons pvindep1(string) pvindep2(string) pvindep3(string) cycle(integer 2012) over(varname numeric) cnt(string) save(string) fast round(integer 2) r2(string)]
			 ...
			 if `pv'==1 | `pvindep'==1 {
			 	if "`fast'"=="" {
					forvalues ip=1(1)5 {	
						...
						if `pv'==1 _cmdbrr pv`ip'`outcome' `indeplist', cmd("`cmd'") cmdops("`cmdops'") `cons' r2(`r2')
						else _cmdbrr `outcome' `indeplist', cmd("`cmd'") cmdops("`cmdops'") `cons'  r2(`r2')
						matrix b`ip'=r(coef)
						matrix se`ip'=r(se)
						local r2`ip'=r(r2)
						}
					...
					forvalues ip=1(1)5 {
						...
						if "`pvindep3'"!="" local indeplist="`indeplist' pv`ip'`pvindep3'"
						local indeplist="`indeplist' `varlist'"
						if `pv'==1 _cmdfast pv`ip'`outcome' `indeplist', cmd("`cmd'") cmdops("`cmdops'") `cons' r2(`r2')
						else _cmdfast `outcome' `indeplist', cmd("`cmd'") cmdops("`cmdops'") `cons' r2(`r2')
						matrix b`ip'=r(coef)
						matrix se`ip'=r(se)
						local r2`ip'=r(r2)
						}
				...
				* HERE CALCULATES MATRICES WITH MEANS AND SEs FROM FIVE PVs
				...
				if "`fast'"=="" {
					_cmdbrr `outcome' `varlist', cmd("`cmd'") cmdops("`cmdops'") `cons' r2(`r2')
					matrix b=r(coef)
					matrix se=r(se)
					local temp_r2=r(r2)
					}
				else {
					_cmdfast `outcome' `varlist', cmd("`cmd'") cmdops("`cmdops'") `cons' r2(`r2')
					matrix b=r(coef)
					matrix se=r(se)
					local temp_r2=r(r2)
					}
...
syntax [namelist(min=1)], cmd(string) [cmdops(string) cons r2(string)]
...
qui `cmd' `outcome' `varlist' [aw=w_fstuwt], `cmdops'
...
forvalues j=1(1)80 {
	qui `cmd' `outcome' `varlist' [aw=w_fstr`j'], `cmdops'
	foreach var in `varlist' `stala' {
