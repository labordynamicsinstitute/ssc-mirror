...
forvalues t_LoOP_sample = 1/`t_LoOP_max' {
	...
	preserve
		...
		local n_NEW_obs_SET = max(_N,`prepsi'+`postpsi')		
		qui set obs	`n_NEW_obs_SET' // add rows in case the panel is longer than it is wide, 
									// for populating all elements of the covariance matrix
