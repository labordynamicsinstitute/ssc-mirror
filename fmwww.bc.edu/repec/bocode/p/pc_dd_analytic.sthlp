...
(the number of cross-sectional units included in the randomization); must be positive integers; e.g. {cmd:n(}50(10)150{cmd:)}.{p_end}
{p2coldent:* {opth mde(numlist)}}list of minimum detectable effect sizes, in levels ({it:not} in percents); e.g. {cmd:mde(}10 20 30{cmd:)}.{p_end}
{synopt :{opth pow:er(numlist)}}list of desired statistical power of experiment; must be between 0 and 1; default is {opt pow:er}{cmd:(0.80)}.{p_end}

{syntab:Experimental design parameters {help pc_dd_analytic##opt_params:[+]}}
{synopt :{opth p(numlist)}}list of treatment ratios (the proportion of cross-sectional units to receive treatment); must be between 0 and 1, e.g. {cmd:p(}0.5 0.75{cmd:)}; default is {cmd:p(0.5)}.{p_end}
{synopt :{opth pre(numlist)}}list of number of pre-treatment periods; must be positive integer(s), e.g. {cmd:pre(}2(2)10{cmd:)}; default is {cmd:pre(1)}.{p_end}
...
{synopt :{opth ar1(numlist)}}list of AR(1) coefficients, to model within-unit serial correlation of {it:idiosyncratic} residuals; must be between -1 and 1, e.g. {cmd:ar1(}0.1(0.1)0.9{cmd:)}.{p_end}

{syntab:Residual variance-covariance (estimated) {help pc_dd_analytic##opt_resid_estimated:[+]}}
{p2coldent:+ {opth dep:var(depvar)}}dependent variable of experiment, for estimating idiosyncratic residual variance and covariances; must be numeric.{p_end}
{synopt :{opt i(panelvar)}}cross-sectional unit of randomization; must be numeric; will default to stored r({it:panelvar}) if not specified.{p_end}
{synopt :{opt t(timevar)}}time period variable; must be numeric; will default to stored r({it:timevar}) if not specified.{p_end}
{synopt :{opth if(if)}}option to restrict observations over which program estimates idiosyncratic residual variance and covariances of {it:depvar}; e.g. {cmd:if(}{it:year>2000}{cmd:)}.{p_end}
{synopt :{opth in(in)}}option to restrict observations over which program estimates idiosyncratic residual variance and covariances of {it:depvar}; e.g. {cmd:in(}{it:1/1000}{cmd:)}.{p_end}
...
use with caution, see details {help pc_simulate##opt_avgcov:below}.{p_end}
{synopt :{opt ncovest(#)}}number of units used to estimate {opt avgcov(pre post cross)}; 
equivalent to stored result {it:r(n_units)} from {help pc_dd_covar}; see 
details {help pc_simulate##opt_avgcov:below}.{p_end}
{synopt :{opt trueparam:eters}}overrides correction factors for estimated
idiosyncratic variance and covariances; use with caution, see 
details {help pc_simulate##opt_avgcov:below}.{p_end}
...
{synopt: [{cmd:append}|{cmd:replace}]}{cmd:append} adds results to an existing .txt file; {cmd:replace} is required to overwrite existing .txt file (if {cmd:append} is 
not specified).{p_end}
{synoptline}
{p2colreset}{...}
{p 4 6 2}* At least 1 of {opth n(numlist)} and {opth mde(numlist)} is required. If only 1 is specified, {opth pow:er(numlist)} will default to {it:power=}0.8 if missing.{p_end}
{p 4 6 2}+ Either {opth var:iance(numlist)} or {opth dep:var(depvar)} is required, and these options are mutually exclusive.{p_end}
...
accommodates arbitrary serial correlation in the outcome variable, unlike the Stata routines {help sampsi} and {help power} that assume the 
idiosyncratic error term is i.i.d.{p_end}
 
{pstd}
{help pc_dd_analytic} operationalizes the formula developed in Equation (8) of: {p_end}
...
{pstd}
This paper demonstrates that standard power calculation tools ignore the serial correlation present in panel data, which can lead to dramatically incorrect power calculations. While 
{help pc_simulate:power calculations by simulation} are recommended, this program is appropriate in cases where pre-existing data do not exist, or are not reliable. This
program can also help to narrow the range of potential parameters values analytically, before undergoing more time-consuming simulations.
{p_end}

{pstd}
Users must provide exactly 2 of the following 3 parameters, allowing {help pc_dd_analytic} to solve for the missing parameter:{p_end}
{break}
...
{p2col 8 12 12 2: -}{opth pow:er(numlist)} statistical power (probability of correctly rejecting a false null given the true effect size {it:mde}) {p_end}

{pstd}
(If {it:power} is not specified and one of {it:n} or {it:mde} is missing, the program will assume the default {it:power}=0.80 and solve of the missing parameter.){p_end}

{pstd}
Power calculations assume a standard panel fixed effects/difference-in-differences model, with a treatment indicator {it:D_it}, unit fixed effects {it:fe_i}, time period fixed effects {it:fe_t}, and serially correlated errors {it:e_it}:{p_end}
{break}
...
{phang}
{opth pre(numlist)} governs the number of pre-treatment periods in the experiment. If not specified,  
For {opt ONESHOT} and {opt POST} models, this option must be either {cmd:pre(}0{cmd:)} or missing.

{phang}
{opth post(numlist)} governs the number of post-treatment periods in the experiment. 
...
that these are not variances of the composite error term, but rather of the {it:idiosyncratic} error term, after partialing out unit and time period fixed effects (i.e., {it:e_it} in the equation above).

{pmore}
This option cannot be combined with {opth dep:var(depvar)}.

{phang}
{opth ar1(numlist)} allows users to model serially correlated errors using a simple AR(1) process. This 
assumes that each cross-sectional unit's error terms evolve by the process:{p_end}

{phang3}
{it:e_it = ar1*e_i(t-1) + v_it},{p_end}

{phang2} where {it:v_it} is an i.i.d. white noise term.

{pmore}
While AR(1) represents a poor approximation of the complex covariance structures found in real-world panel data, 
...
{pmore}
{opth ar1(numlist)} can only be used in combination with {opth var:iance(numlist)}. However, 
{opth var:iance(numlist)} may also be combined with option {opt avgcov(pre post cross)}. See discussion {help pc_simulate##opt_avgcov:below}. 

{pmore}
If {opth var:iance(numlist)} alone is specified, then the resulting power calculations will assume i.i.d. errors with no serial correlation.
...
{pstd}
Instead of providing a (range of) residual variance(s) and AR(1) parameter(s), users can have the subprogram {help pc_dd_covar} 
estimate the residual variance and covariances of the desired outcome variable ({it:depvar}).

{pstd}
By estimating the variance-covariance structure from a pre-existing dataset (instead of assuming an AR(1) dependency structure), the power calculations will be able to incorporate the complex dependencies inherent in real data.

{phang}
{opth dep:var(depvar)} denotes the dependent variable of choice, which must be in the current dataset.
...
{phang}
{opth if(if)} and {opth in(in)} allow users to specify a subset of {it:depvar} observations over which to estimate residual variance and covariances, if desired.

{pstd}
Please see the  {help pc_dd_covar:pc_dd_covar help documentation} for details on how this program estimates variance and covariance parameters to be fed into the analytical power calculation formula.
...
{phang}
{opt avgcov(pre post cross)} allows users to manually input three average within-unit covariance terms. These terms are, in order: the 
...
because residual covariances scale with the residual variance, option {opt var:iance(#)} can only accommodate one value when combined with {opt avgcov}. (Option 
{opth dep:var(depvar)} performs the same operation under the hood, by using  {help pc_dd_covar} to calculate new covariances for each combination of parameters.)

{pmore}
If {it:pre}=1 ({it:post}=1) then the average residual covariance between pre-treatment (post-treatment) periods is vacuously defined. However, {opt avgcov(pre post cross)} still 
...
used to estimate the four values {opt var:iance(#)} and {opt avgcov(pre post cross)}. This 
...
{pmore}
One note of caution: estimating variance and covariance parameters from regression residuals will {cmd:not} replicate the {it:population parameters} governing the data-generating process of the error term. To take one simple example, 
suppose {it:e_it = 0.8*e_i{t-1} + v_it}, where {it:var(v_it)=3.6}, implying that {it:var(e_it)=10}. In a model with {it:pre=1} and {it:post=1}, the average
...
{pmore}
Importantly,
this program assumes that {opt var:iance(#)} is an {bf:estimate} of the true error variance, estimated jointly with {opt avgcov(pre post cross)} (when both 
options are specified). If {opt var:iance(#)} was not estimated using the same 
residuals as {opt avgcov(pre post cross)}, the resulting
power calculations will be incorrect.

{phang}
{opt trueparam:eters} allows users to override this assumption. If specified, the program will treat all four values {opt var:iance(#)} and {opt avgcov(pre post cross)} as {it:population} 
parameters (equivalent to {it:sigma^2_omega}, {it:psi^B}, {it:psi^A}, and {it:psi^X} in the notation of Burlig, Preonas, and Woerman (2017)).
...
{phang}
{opth out:file(filename)} specifies the name of the .txt output file that stores {cmd:pc_dd_analytic} results in the current directory. If not specified, the program will not store its results, but they will still display in the Stata console.

{pmore} Option {cmd:append} will append {cmd:pc_dd_analytic} results below previous results in an existing {it:filename}. If {it:filename} exists, then either {cmd:append} or {cmd:replace} is required.
...
{phang}
{browse "https://ei.haas.berkeley.edu/research/papers/WP277.pdf":Burlig, Fiona, Louis Preonas, and Matt Woerman (2017). "Panel Data and Experimental Design." Energy Institute at Haas Working Paper #277. University of California Berkeley.} {p_end}
