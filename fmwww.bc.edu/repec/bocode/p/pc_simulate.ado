...
syntax varname [if] [in], MODel(string) [i(varname) t(varname) p(numlist >0 <1 sort) ///
              pre(numlist >=0 integer sort) post(numlist >=0 integer sort) n(numlist >0 integer sort) ///
							...
							vce(string) COLLapse nsim(numlist max=1 >0) Weight(string) reghdfeoptions(string) ///
							BOOTstrap OUTfile(string) append replace] 
...
if "`i'"!="" {	
	capture confirm numeric variable `i' 
		local rc = _rc
		if `rc' {
			display "{err}Error: Cross-sectional unit of treatment `i' must be numeric"
			use `m_dta_before_sims', clear	
			exit `rc'
		}
}
	...
	capture tsset
		if !_rc {
			local i = r(panelvar)
			display "{text}Warning: {inp}Cross-sectional unit of treatment missing, assumed to be `i'" _n
		}
}
		...
		if "`i'"!="" {
			forvalues tEMp_id = 1/`tEMp_grpSTRAT_MAX' {
				preserve
					...
					local obs_STRAT_min = min(`obs_STRAT_min',r(sum))
				restore
			}
...
capture reghdfe, version
	local rc = _rc
	if `rc' {
		display "{err}Error: Please ssc install reghdfe"
		use `m_dta_before_sims', clear	
		exit `rc'
	}
capture assert real(substr(e(version),1,1))>=3
	local rc = _rc
	if `rc' {
		display "{err}Error: Please update reghdfe by running ssc install reghdfe, replace"
		use `m_dta_before_sims', clear	
		exit `rc'
	}
...
quietly keep `y' `i' `t' `stratify' `controls' `absorb' `abs_fvars' `cl_varlist' `wtvar'
...
if "`stratify'"=="" {	
	quietly unique `i'
	capture assert `nMAX'<=r(sum)
		local rc = _rc
		if `rc' & "`i'"!="i_fOr_OneshOt" {
			display "{err}Error: Sample size too small to simulate experiment with `nMAX' unique `i' units" _n
			display "{err}Specify option bootstrap to sample `nMAX' `i' units with replacement from existing dataset"
			use `m_dta_before_sims', clear	
			exit `rc'
		}
			...
			display "{err}Specify option bootstrap to sample `nMAX' observations with replacement from existing dataset"
			use `m_dta_before_sims', clear	
			exit `rc'
		}	
}
	...
	forvalues stratLOOP = 1/`levelsSTRAT' {
		...
		capture assert `nMAX'<=r(sum)
			...
			if `rc' & "`i'"!="i_fOr_OneshOt" {
				...
				display "{err}Specify option bootstrap to sample `nMAX' `i' units with replacement from "
				...
				display "{err}       in each stratified randomizaton cell. " _n
				display "{err}Specify option bootstrap to sample `nMAX' observations with replacement from "
				display "{err}existing dataset, for each stratified cell. "
				use `m_dta_before_sims', clear	
				exit `rc'
			}	
...
if inlist("`model'","POST")  {
	...
	if "`tstart'"!="" {
		...
		quietly replace tEMp_enoughT = 1 if `t'>=real(word("`tstart'",1))
		quietly replace tEMp_enoughT = 1 if `t'<=real(word("`tstart'",2))+`postMAX'-1
	}
	else {
		quietly gen tEMp_enoughT = 1
	}
	quietly unique `t' if tEMp_enoughT==1
	capture assert `postMAX'<=r(sum)
		local rc = _rc
		if `rc' & "`tstart'"!="" {
			display "{err}Error: Dataset contains too few time periods to simulate experiment with "
			display "            `postMAX' post-treatment periods (given tstart() option) " _n
			...
			*display "{err}by drawing additional `i' units with replacement from existing dataset"
			...
			display "{err}Error: Dataset contains too few time periods to simulate experiment with "
			display "            `postMAX' post-treatment periods " _n
			*display "{err}Use program ?????? HELP ????? to create dataset with sufficient number of units, "
			*display "{err}by drawing additional `i' units with replacement from existing dataset"
			use `m_dta_before_sims', clear	
			exit `rc'
	...
	if "`tstart'"!="" {
		quietly gen tEMp_enoughT = 0
		quietly replace tEMp_enoughT = 1 if `t'>=real(word("`tstart'",1))
		quietly replace tEMp_enoughT = 1 if `t'<=real(word("`tstart'",2))+`preMAX'+`postMAX'-1
	}
	else {
		quietly gen tEMp_enoughT = 1
	}
	quietly unique `t' if tEMp_enoughT==1
	capture assert `preMAX'+`postMAX'<=r(sum)
		local rc = _rc
		if `rc' & "`tstart'"!="" {
			display "{err}Error: Dataset contains too few time periods to simulate experiment with "
			display "            `preMAX' pre-treatment and `postMAX' post-treatment periods " 
			display "            (given tstart() option) " _n
			*display "{err}Use program ?????? HELP ????? to create dataset with sufficient number of units, "
			*display "{err}by drawing additional `i' units with replacement from existing dataset"
			use `m_dta_before_sims', clear	
			exit `rc'
			...
			display "{err}Error: Dataset contains too few time periods to simulate experiment with "
			display "            `preMAX' pre-treatment and `postMAX' post-treatment periods " _n
			*display "{err}Use program ?????? HELP ????? to create dataset with sufficient number of units, "
			*display "{err}by drawing additional `i' units with replacement from existing dataset"
			use `m_dta_before_sims', clear	
			exit `rc'
						...
						if "`bootstrap'"!="" {
							if "`stratify'"!="" {
								...
								* sample `nLOOP' units with replacement
								...
								forvalues stratLOOP = 1/`levelsSTRAT' {
									quietly unique `i' if tEMp_grpSTRAT==`stratLOOP'
									local expand_numb = max(`expand_numb',ceil(`nLOOP'/r(sum)))
								}
								expand `expand_numb'
							...
							* if drawing units WITH replacement (option bootstrap is specified)
							if "`bootstrap'"!="" {
								if "`stratify'"!="" {
									...
									* sample `nLOOP' units with replacement
									unique `i'
									local expand_numb = ceil(`nLOOP'/r(sum))
									expand `expand_numb'
									bysort `i' `t' : gen tEMp_bSAMple_id1 = _n
									egen tEMp_bSAMple_id2 = group(`i' tEMp_bSAMple_id1)
									drop `i'
...
if "`append'"=="append" {
	...
	foreach v in n p mde pre post power outcome model unit time stratify controls fixed_effects alpha onesided tstart std_errors collapse bootstrap nsim ifs ins weight reghdfe_options dataset {
	...
	capture outsheet using "`outfile'", comma names
		local rc = _rc
		if `rc' {
			display "{err}Error in writing results to file `outfile' "
			use `m_dta_before_sims', clear	
...
// reload master dataset and display final output
{
use `m_dta_before_sims', clear	
