...
{p 8 8 2}
{cmd:pathutil} {cmd:confirm} [ {opt new} {c |} {opt url} {c |} {opt abs:olute} ] 
{cmd:"}{it:{help filename:path}}{cmd:"}
...
extension/suffix. Elements are returned in {cmd:s()}. If path has no 
...
{pstd}
{cmd:pathutil} {{cmd:to}{c |}{cmd:of}} is a clone of {cmd:pathof} 
({help pathutil##ref:Barker 2014}), conceptually. It returns in {cmd:s()} 
the current working directory up to the specifed {it:directory} or the 
root of the current working directory, if {it:directory} is not specified.
...
{help pathutil##ref:Blanchette's (2011)} {cmd:confirmdir}. 
...
{pstd}
Earlier versions of {cmd:pathutil} were released under the simpler 
name {cmd:path}; typing {cmd:path} instead of {cmd:pathutil} continues 
to work until, some day, StataCorp decides to write their own {cmd:path} 
command. Thus, I recommend changing old code and typing {cmd:pathutil}. 
...
{synopt:{cmd:s(piece{it:#})}}elements of {it:path} (reverse order){p_end}
...
{pstd}
{cmd:pathutil} {{cmd:to}{c |}{cmd:of}} saves in {cmd:s()}
...
{psee}
if installed: {help confirmdir}, {help pathof},
