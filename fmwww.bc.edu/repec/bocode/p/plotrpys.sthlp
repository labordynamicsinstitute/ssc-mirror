...
{pstd}
Baum and Otero (2022) introduced an enhanced Stata implementation for testing and dating explosive episodes in time-series data. Building on the right-tail augmented Dickey–Fuller (ADF) framework, their paper presents the {cmd:radf} command. With {cmd:radf}, one can test whether a time series ever enters a phase of explosive growth rather than following a persistent but non-explosive (unit-root) pattern. With yearly counts of cited references (exported as CSV exports from CRExplorer) {cmd:radf} can identify years in which citation activity "exploded". {p_end}
