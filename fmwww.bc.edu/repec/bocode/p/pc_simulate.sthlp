...
{synopt :{opth mde(numlist)}}list of minimum detectable effect sizes for calculating statistical power, in units of {depvar} ({it:not} in percent of {depvar}); e.g. {cmd:mde(}10 20 30{cmd:)}.{p_end}

{syntab:Experimental design parameters {help pc_simulate##opt_params:[+]}}
{synopt :{opt i(panelvar)}}cross-sectional unit of randomization, required for {opt POST}, {opt DD}, and {opt ANCOVA} models; must be numeric; will default to stored r({it:panelvar}) if not specified.{p_end}
{synopt :{opt t(timevar)}}time period variable, required for {opt POST}, {opt DD}, and {opt ANCOVA} models; must be numeric; will default to stored r({it:timevar}) if not specified.{p_end}
{synopt :{opth n(numlist)}}list of sample sizes for experiment (the number of cross-sectional units included in the randomization); must be positive integer(s), e.g. {cmd:n(}50(10)80{cmd:)}; default is # of unique values of {it:panelvar}.{p_end}
...
{synopt :{opth pre(numlist)}}list of number of pre-treatment periods for {opt DD} and {opt ANCOVA} models; must be positive integer(s), e.g. {cmd:pre(}2(2)10{cmd:)}; default is {cmd:pre(1)}.{p_end}
{synopt :{opth post(numlist)}}list of number of post-treatment periods for {opt POST}, {opt DD}, and {opt ANCOVA} models; must be positive integer(s), e.g. {cmd:post(}1 2 3{cmd:)}; 
...
{synopt :{opth a:bsorb(varlist)}}fixed effects to be included in estimating equation; must be numeric; will default to {opt a:bsorb(panelvar timevar)} for {opt DD} model only.{p_end}
{synopt :{opth cont:rols(varlist)}}linear control variables to be included in estimating equation; must be numeric.{p_end}
{synopt :{opt vce}{cmd:(}{help reghdfe##opt_vce:{it:vcetype}}{cmd:)}}{it:vcetype} may be {opt un:adjusted}, {opt r:obust} or {opt cl:uster} {help fvvarlist} (see {help reghdfe##opt_vce:reghdfe} for detail); {p_end}
{synopt :}default is {opt vce(unadjusted)} for {opt ONESHOT} model, {opt DD} model with 1 pre-treatment and 1 post-treatment period, and {opt ANCOVA} model with 1 post-treatment period;{p_end}
...
{synopt: [{cmd:append}|{cmd:replace}]}{cmd:append} adds simulation results to an existing .csv file; {cmd:replace} is required to overwrite existing .csv file (if {cmd:append} is 
not specified).{p_end}


{syntab:Advanced options {help pc_simulate##opt_advanced:[+]}}
{synopt :{opt ts:tart(# [#])}}restricts the range of time periods over which to begin experiment; for example, {cmd:tstart(}4 7{cmd:)} will simulate experiments beginning in time periods {it:timevar}={4,5,6,7} with equal probability; 
...
{synopt :{opth strat:ify(varlist)}}stratifies randomization by a (list of) categorical covariate(s); 
if specified, then {opth n(numlist)} governs the number of units {it:in each stratified randomization cell}; note that the resulting power calculations still estimate a model with a {it:pooled} average treatment effect.{p_end}
{synopt :{opth absorbf:actor(fvvarlist)}}accommodates factor variable fixed effects, as allowed by {help reghdfe##absvar:reghdfe}; can be combined with the simpler {opth a:bsorb(varlist)} option.{p_end}
{synopt :{opth w:eight(weight)}}{opt fw:eight}s, {opt aw:eight}s and {opt pw:eight}s are allowed; syntax is {opt w:eight(aw=varname)}; see {help weight}.{p_end}
{synopt :{opt reghdfeopt:ions}{cmd:(}{help reghdfe##options:options}{cmd:)}}allows users to pass through additional reghdfe options (not all of which are supported); see {help reghdfe}.{p_end}	
{synoptline}
...
{pstd}
The average rejection rate across all {it:nsim} simulations is the statistical power, or the probability that the experiment would reject a true effect size of at least {it:mde}. 
...
{phang}
{opth p(numlist)} governs the proportion of units randomized into treatment. The program rounds {it:p*n} to the nearest integer, and randomly chooses this number of treated units in each iteration.
...
{phang}
{opt boot:strap} controls whether the program samples cross-sectional units
with or without replacement. The 
default is to sample {it:without} replacement (if {opt boot:strap} is
not specified), which requires {it:n} to be weakly less than the 
number of
units in the dataset.

{pmore}
If {opt boot:strap} is specified, each simulation will draw {it:n} 
cross-sectional units {it:with} replacement from the existing dataset,
and {it:n} may be larger or smaller than the number of units in
the dataset.

{pmore}
(This option replaces the program {it:pc_bootstrap_units}, which is now obsolete.)
...
in practice, simulated power will not converge unless 
{it:nsim{c 179}}500. {it:nsim{c 179}}1000 (or even {it:nsim{c 179}}5000) is recommended for more precise convergence, although this does increase runtime.
...
{pmore} Option {cmd:append} will append {cmd:pc_simulate} results below previous results in an existing {it:filename}. If {it:filename} exists, then either {cmd:append} or {cmd:replace} is required.
...
{phang}
{opt ts:tart(# [#])} restricts the range of time periods over which to begin experiment. For example, {cmd:tstart(}4 7{cmd:)} will simulate experiments beginning in time periods {it:timevar}={4,5,6,7} with equal probability.

{pmore}
If only one number is specified, the option will default to {opt ts:tart(# max(timevar))}.

{pmore}
This option indicates the first {it:pre-treatment} period in a {opt DD} or {opt ANCOVA} model, the first {it:post-treatment} period in a {opt POST} model, and the {it:only} period in a {opt ONESHOT} model.

{pmore}
Unless {opt ts:tart(# [#])} is specified, each iteration will randomly select a ({it:pre+post})-period window from the full range of {it:timevar} in the dataset. For 
example, if {it:timevar}={1,2,...,100} and {it:pre+post}=8, then each iteration will simulate an 8-period experiment beginning in a time period drawn randomly from {it:timevar}={1,2,...,93}.
...
{phang}
{opth strat:ify(varlist)} allows users to simulate stratified randomization, where randomization occurs within cells defined by {it:egen cell = group(varlist)}. For
 {opt stratify(gender age)}, each {it:gender-age} combination would represent a separate randomization cell. Each {it:panelvar} unit must be nested within a single cell.

{pmore}
Importantly, when using this option, {opt n(numlist)} governs the number of units {it:in each cell}, and exactly {it:p*n} units per cell will be assigned to treatment. (The 
dataset must contain at least {it:n} units in each randomization cell.)
