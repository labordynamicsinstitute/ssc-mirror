...
The placebo effects are then estimated using the fake treatment variables. {bf:didplacebo} uses the same method to compute the standard errors as specified by the command to estimate the DID model. 
...
The in-space placebo test implements many repetitions, say 500 repetitions. In each repetition, a fake treatment variable is generated by randomly shuffling {it:treatvarname} blockwise along the dimension of {it:panelvar}. 
...
For statistical inference, the original estimated treatment effect is then compared against the distribution of estimated placebo effects to calculate two-sided, left-sided and right-sided p-values. 
...
The mixed placebo test implements many repetitions, say 500 repetitions. 
In each repetition, a fake treatment variable is generated by randomly selecting both fake treatment units and times.
Not less than one of {bf:1}, {bf:2} and {bf:3} must be specified in {cmd:pbomix}{bf:()} to choose the version of implementation, where {bf:1} correponds to the version suitable for standard DID, {bf:2} and {bf:3} correpond to the free and restricted versions suitable for staggered DID, respectively.
...
{phang2} 
{opt pbomix}{bf:(}[{bf:2} {bf:3}]{bf:)} is used for staggered DID with staggered policy adoption, where {bf:2} and {bf:3} correponds to the free and the restricted versions, respectively. 
The free (unrestricted) version {opt pbomix}{bf:(}{bf:2}{bf:)} randomly selects a fake treatment time for each unit, without maintaining the original cohort structure (i.e., how many units in each cohort). The number of fake treatment units and the range of fake treatment periods are specified by the options {opth ranunitnum(int)} and {opt rantimescope(t_min t_max)} respectively.

{phang2} 
On the other hand, the restricted version {opt pbomix}{bf:(}{bf:3}{bf:)} maintains the cohort structure. Specifically, suppose the treatment periods in the sample consist of t_1, t_2,...,t_G, and the correponding cohorts contain N_1, N_2,...,N_G units respectively. In each repetition, we randomly partition all units in the sample into G cohorts containing N_1, N_2,...,N_G units respectively, then randomly assign G different periods from {opt rantimescope(t_min t_max)} to these cohorts as their fake treatment periods. The range of fake treatment periods specified by the options {opt rantimescope(t_min t_max)} must be greater than or equal to the number of periods in the actual treatment variable.
...
If {opt rantimescope(t_min t_max)} is not specified, {it:t_min} and {it:t_max} will default to the first period and the last period in the sample respectively. 
...
{phang}
Chen, Q., Qi, J., and Yan, G. 2023. didplacebo: Implementing placebo tests for difference-in-differences estimation. {it:Shandong University working paper}.
