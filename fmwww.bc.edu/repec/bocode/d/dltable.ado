*! version 1.0  31/31/2024
* This is dltable with 3 digits precision

cap program drop dltable
program define dltable
version 18.0
set more off
syntax [varlist] [using] [if], TREATment(varlist) CONTrol(varlist fv) [PARTialout(varlist fv) CLUSTer(varlist)  keep  quiet sd sheet(string asis) QValue(name) SELECTion(name)]
pause on
...
if mi("`keep'")==1 {
	...
	capture confirm variable `cvar'
		if !_rc {
			dis as error  "Dataset should not include the following variables: VAR LAB  N_ind N_clust A C COEF* "
			exit 100
        }
	}
...
foreach i in `varlist' {
	...
	if mi("`quiet'")==0 { 
	xi: quietly dsregress  `i' `treatment' `if' , cluster(`cluster') controls((`partialout') `control') selection(`selection')
...
quiet {
	...
	distinct `cluster' if e(sample)
	replace N_clust="`: display %7.0fc r(ndistinct)'" if _n==`j'
	...
	forval h=1/`c' {
		...
		* first row
		...
		replace COEF`h'=COEF`h'+"*" if   _n==`j' &  2*normal(-abs(_b[``h'']/_se[``h''])) <=0.1
		replace COEF`h'=COEF`h'+"*" if   _n==`j' &  2*normal(-abs(_b[``h'']/_se[``h'']))<=0.05
		replace COEF`h'=COEF`h'+"*" if   _n==`j' &  2*normal(-abs(_b[``h'']/_se[``h'']))<=0.01
		* second row (SE)
		...
		* store pvalues for qvalue calculation
		if mi("`qvalue'") ==0 {
			local p``h''=2*normal(-abs(_b[``h'']/_se[``h'']))
