...
{it:characteristic_list} is a list of {it:characteristic_name}s and/or asterisks ({hi:*}) separated by spaces,
{it:attributes_list} is a list of items from the list

{p 8 18}
{cmd:type} {cmd:format} {cmd:vallab} {cmd:varlab}
...
These variable attributes are items in the list {cmd:type}, {cmd:format}, {cmd:vallab} and {cmd:varlab},
specifying variable {help type:types}, {help format:formats}, {help label:value labels} and {help label:variable labels},
respectively.
...
{p 4 8 2}
{cmd:charlist(}{it:characteristic_list}{cmd:)} specifies a list of characteristic names and/or asterisks ({hi:*}),
separated by spaces.
...
with {hi:omit} appearing first and {hi:missing} appearing second.
In the second case, the output variable {hi:char1} will contain the {hi:omit} characteristics,
and the output variable {hi:char2} will contain the {hi:missing} characteristic.
(See {hi:Output dataset created by {cmd:descsave}} below for details on output variables.)
...
observations.
This is useful if the output dataset is concatenated with other {hi:descsave} output datasets
...
then the output dataset will be sorted by the single variable {hi:order}.
...
{p2col:{hi:order}}Variable order{p_end}
{p2col:{hi:name}}Variable name{p_end}
{p2col:{hi:type}}Storage type{p_end}
{p2col:{hi:format}}Display format{p_end}
{p2col:{hi:vallab}}Value label{p_end}
{p2col:{hi:varlab}}Variable label{p_end}
{p2col:{hi:char}{it:n}}Characteristic varname[{it:characteristic_name}]{p_end}
{p2line}
{p2colreset}

{pstd}
The variable {hi:order} contains the sequential order of the variable in the input {varlist} specified for {cmd:descsave},
...
under the current version of Stata). All of these variables can be renamed using the {cmd:rename()} option,
...
categorical factors from the variable {hi:label} in the {helpb parmest} output dataset,
...
{pstd}
The following example will work in the {hi:auto} data
...
{p 4 13 2}
{bind: }Manual: {hi:[D] describe}, {hi:[D] destring}, {hi: [D] gsort}, {hi:[D] insheet}, {hi:[D] label}, {hi:[D] outsheet}
 {break}{hi:[R] tabulate}, {hi:[R] xi}
 {break}{hi:[U] 12.8 Characteristics}
 {break}{hi:[P] char}
{p_end}
{p 4 13 2}
