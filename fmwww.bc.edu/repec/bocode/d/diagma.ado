* DiagMA version 1.3 - 15 October 2021
* Authors: Luis Furuya-Kanamori (l.furuya@uq.edu.au) & Suhail AR Doi

	*version 1.1 - se replaced with se*1.2
	*version 1.2 - figure plots observed (not estimated) sens and spec
	*version 1.3 - keep all data after if/in selected
...
*Study specific Sens, Spec, and DOR
	gen __sens_fig = (__tp/(__tp+__fn))
	gen __spec_fig = (__tn/(__tn+__fp))
...
*Pooled Sens, Spec, DOR, AUC, LR
	...
	*SE Sens/Spec
		...
		gen __ma_se_sens = sqrt((0.5 -__prop_sens+__prop_spec)*(1.2*__ma_se_dor^2))
		...
		gen __ma_se_spec = sqrt((0.5 +__prop_sens-__prop_spec)*(1.2*__ma_se_dor^2))
...
if "`graph'" != "nograph" & "`lfk'"=="" & "`cutplot'"==""{
	...
	quietly gen __gsens = __sens_fig
	quietly gen __gspec = 1-__spec_fig
