...
{p2colset 5 19 21 2}{...}
{p2col:{hi: dqd} {hline 2} Difference-in-differences estimations under alternative Parallel-{it:q} assumptions.}
{p2colreset}{...}

{title:Syntax}
	
{p 8 16 2}
{opt dqd} {depvar} [{indepvars}] {ifin} {weight} {cmd:,} {opt tr:eated(treatvar)} {opt t:ime(timevar)} [{it:options}]
...
{pstd}
{cmd:dqd} performs difference-in-differences estimations under alternative Parallel-{it:q} assumptions as proposed by Mora and Reggio (2012).
...
When the fully flexible model is chosen, {cmd:dqd} displays point estimates and standard errors of treatment effects under each Parallel-{it:q} assumption between Parallel-1 and Parallel-{it:#q}. 
For each {it:q}, {cmd:dqd} computes the treatment effects up to {it:S} periods, where {it:S}={it:#e}-{it:#b}+1. By default, effects only up to {it:s}=3 are displayed.
...
In this case, {cmd:dqd} will obtain for Parallel-1, Parallel-2, and Parallel-3 assumptions the treatment estimates one period ({cmd:s}={cmd:1}) and two periods ({cmd:s}={cmd:2}) after treatment. 
{cmd:dqd} additionally displays tests for 
...
In addition, for the computations in dqd to be meaningful, the difference between any two consecutive periods should be equal to one.

{pstd}
Only one model may be specified. 

{pstd}
{cmd: dqd} is an e-class ado. It first estimates an auxiliary regression and then it computes the treatment effects as linear combinations of the estimates of the auxiliary regression. 
...
{p 4 8 2}
{stata "use dqd_examples.dta, clear"}
...
{p 8 16 2}{cmd:. dqd output, treated(D) time(t) standard }{p_end}

{p 4 4 2}
Using the original data set:{p_end}

{p 8 16 2}{cmd:. dqd output if (t==3 | t==4), treated(D) time(t) standard }{p_end}
...
{p 8 16 2}{cmd:. dqd output if (t>1 & t<5), treated(D) time(t)}{p_end}
...
{p 8 16 2}{cmd:. dqd output if (t>1 & t<5), treated(D) time(t) linear} {p_end}
...
{p 8 16 2}{cmd:. dqd output, treated(D) time(t) begin(4) end(4) }{p_end}

{p 8 16 2} // fully flexible model, q=1,2, s=1,2{p_end}
{p 8 16 2}{cmd:. dqd output, treated(D) time(t) begin(4) end(5) q(2)} {p_end} 

{p 4 4 2}Additional controls can be added{p_end}

{p 8 16 2}{cmd:. dqd output x1 x2, treated(D) time(t) begin(4) end(4) }{p_end}

{title:Saved results}

{pstd}
{cmd:dqd} saves the following in {cmd:e()}:
