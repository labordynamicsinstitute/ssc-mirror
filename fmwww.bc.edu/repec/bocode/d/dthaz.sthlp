...
{p 8 18 2}
{cmd:dthaz} [{it:varlist}] [{cmd:if}] {weight}
	    [{cmd:, {ul on}sp{ul off}ecify(}{it:numlist}{cmd:)} 
	    {cmd:{ul on}tp{ul off}ar(}{it:#}{cmd:)} 
	    {cmd:{ul on}t{ul off}runcate(}{it:#}{cmd:)} 
	    {cmd:pretrunc(}{it:#}{cmd:)} 
	    {cmd:{ul on}cl{ul off}oglog}
	    {cmd:{ul on}clus{ul off}ter(}{it:varname}{cmd:)} 
	    {cmd:{ul on}d{ul off}isplay(}{it:#}{cmd:)}
	    {cmd:{ul on}l{ul off}evel(}{it:#}{cmd:)} 
	    {cmd:{ul on}m{ul off}odel}
	    {cmd:suppress}
	    {cmd:{ul on}gr{ul off}aph(}{it:#}{cmd:)}
	    {it:graph_twoway_options} {cmd:copyleft}]
...
{synopt :{opt cl:oglog}}use a complimentary log-log link (see {help cloglog}){p_end}
...
{phang}{cmd:{ul on}cl{ul off}oglog} This option switches the estimate of the 
hazard function to a complementary log-log link. This produces estimates under 
an assumption of proportional hazards, rather than an assumption of proportional 
odds. The general discrete time complimentary log log hazard model is:
...
{p 4 8}{inp:. dthaz party age, sp(0 1) model cloglog}{p_end}
...
{pstd}
In addition to the results returned by the estimation commands {cmd:logistic} or {cmd:cloglog}, {cmd:dthaz} saves the following in {cmd:e()}:
