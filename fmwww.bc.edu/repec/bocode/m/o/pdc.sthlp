...
The general formula for PDC is:{p_end}
	...
	days of supply on hand in the observation period
	-----------------------------------------------
...
in the observation period.{p_end}
...
with prescriptions filled for one medication or multiple medications. Moreover, {cmd:PDC} provides the user with tremendous flexibility in specifying the observation period.
...
{p 4 8 2}
{cmd:id(}{it:string}{cmd:)} the panel identifier if the data contain multiple patients. When {cmd:id()} is not specified, {cmd:PDC} assumes that there is
...
{p 4 8 2}
{cmd:credit} specifies that overlapping refill periods should be spaced out by moving the next {it:fill_date} forward to coincide with the last day of the previous refill. This
...
Here we do not specify the start date, which means that {cmd:pdc} will use the earliest {it:filldate} as the start date. We set the end date of the observation period
...
Here we do not specify either the start date or the end date, but specify the observation period as being 180 days. Additionally, we specify {cmd: credit}
