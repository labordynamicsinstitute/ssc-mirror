...
//di "`optname'_txt``optname'_line'  =  ``optname'_txt``optname'_line''"
  ...
  if strpos(`"`mh_line'"', "opt2[")~=0 { /* this is the multiple line version*/
   ...
   if strpos(`"`mh_line'"', "seealso[")~=0 {
     local see_line 1
     if (strpos(`"`mh_line'"', "]")==0) {
      local see_text`see_line' = substr(`"`mh_line'"', strpos(`"`mh_line'"',"seealso[")+8, .)
     }
     else {
      local see_text`see_line' = substr(`"`mh_line'"', strpos(`"`mh_line'"',"seealso[")+8, strpos(`"`mh_line'"', "]")-9)
     }
     local `see_line++'
     while strpos(`"`mh_line'"',"]")==0 { /* this is for multiple lines*/
       file read `fh' mh_line
 ...
 *
 * I used to do subinstr.. to remove these comments but I need to 
 * delete rest of line
 *********************************************************************/
  local synt ""
  forv ii=1/`nlines' {
 ...
 ******************************************************************************/
 if index("`opt'","]")~=0 { /* there are optional options */
   local opt = substr("`opt'", 1, index("`opt'","]")-1) 
    ...
    else { /* i.e. all options are optional */
      local opt_opt "`opt'"
      local req_opt ""
    }
 }
