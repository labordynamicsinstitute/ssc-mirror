...
{p 4 8}{cmdab:gropt(}{it:string}{cmd:)} can be used to pass graph options, such 
as scheme(), to the graphics commands.
{p_end}
...
{p 4 8}{stata "freduse GDPC1, clear" :. freduse GDPC1, clear}{p_end}
{p 4 8}{stata "gen yq = qofd(daten)" :. gen yq = qofd(daten)}{p_end}
{p 4 8}{stata "tsset yq, quarterly" :. tsset yq,quarterly}{p_end}
{p 4 8}{stata "nbercycles GDPC1 if tin(1980q1,), file(nber3.do) gropt(scheme(plotplainblind)) replace" :. nbercycles GDPC1 if tin(1980q1,), file(nber3.do) gropt(scheme(plotplainblind)) replace}{p_end}
