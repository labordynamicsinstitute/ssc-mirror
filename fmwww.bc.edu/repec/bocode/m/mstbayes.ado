...
quietly {
        ...
        syntax varlist(numeric fv) [if] [in], INTervention(varlist numeric fv max=1) RANdom(varlist numeric max=1) [, THReshold(numlist) SEPCHains DIAGnostics noIsily SAVE Cond(string) UNCond(string)*]
		...
		local unc_options: copy local options
		
		if "`cond'"!="" {
		local options `options' `cond'
		}
		if "`uncond'"!="" {
		local unc_options `unc_options' `uncond'
		}
        
        /*Unconditional Output*/
        noi bayes,saving(mcmcUncMST, replace) `isily' `unc_options': mixed `depvar' || `random':, cov(unstructured)
