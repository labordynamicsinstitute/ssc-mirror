...
qui {       
	...
	if "`basic'"!="" local ci `basic'
        ...
        tempname chk test0 test1 Beta1 Beta Cov X UschCov schCov schRand max chk b0 id_var_col1 cluster1_variance1 res_var_col ///
	res_variance1 Nu lvl res_variance2 vcovschTrt1 varB31 varB32 vcovschTrt2 b failtest group_max mstNt colnumber min Max tot1 ///
	res_star1 res_dfl1 tot2 res_star2 res_dfl2 coef summed xrecov1 xrecov2 sumvarB21 sumvarB22 sums
        tempvar res1 ressq1 res_refl1 res2 ressq2 res_refl2 fitted1 fitted2 EstmReff EstmReff_unc EstmReff_unc_sq1 brokn_fctor
		...
		scalar `sumnotconv'= 0
			...
			cap {
				...
				mata: hfunc1("`mstNt'", "`Nu'","`varB32'", "`vcovschTrt2'","`xrecov2'","`sumvarB22'","`summed'")
					...
					gCondUncondMST, i(`i') j(`j') in(`intervention') rand(`random') res_variance1(`res_variance1') ///
					res_variance2(`res_variance2') group_max(`group_max') max(`max') vartt1(`varTT1') vartt2(`varTT2') ///
					icc1(`ICC1') sums(`sums') coef(`coef') brokn_fctor(`brokn_fctor')
		...
		scalar `to' = `=`to''+ `=`sumnotconv''
                ...
                forvalues j = 1/`=`to'' {
                        forvalues i = 1/`=`max'-1' {
                                ...
                                cap gen double PermC_I`i'_W=.
                                cap gen double PermC_I`i'_T=.
                                cap replace PermC_I`i'_W = `dwC`i'`j'' in `j'
                                cap replace PermC_I`i'_T = `dtTotalC`i'`j'' in `j' /*plug in estimates*/
                                                
                                cap gen double PermUnc_I`i'_W=.
                                cap gen double PermUnc_I`i'_T=.
                                cap replace PermUnc_I`i'_W = `dwU`i'`j'' in `j'
                                cap replace PermUnc_I`i'_T = `dtTotalU`i'`j'' in `j'
                                }                       
                        }
		...
		//===================  BOOTSTRAPS  ===================//
			...
			if "`residual'" != "" { 
				...
				forvalues i=1/2 {
					...
					mata: funcchol("`group_max'", "`xrecov`i''", "`Estm_`i'Reff'") 
...
cap program drop gCondUncondMST
...
vartt2(name) icc1(name) sums(name) coef(name) brokn_fctor(name)
	...
	sqnt`i' sqnc`i' qnt`i' qnc`i'
	   ...
	   scalar `N`i'' = `Nc`i'' + `Nt`i''
        ...
        cap tab `random' `brokn_fctor'`=`i'+1' if `brokn_fctor'`=`i'+1'==1, matcell(`nt`i'')
        scalar `Mt`i''= r(r)

        cap tab `random' `brokn_fctor'`=`i'+1' if `brokn_fctor'`=`i'+1'==0, matcell(`nc`i'')
        scalar `Mc`i''= r(r)
