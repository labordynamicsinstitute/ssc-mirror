/*
maxi_boot_wrapper.ado performs the following operations:
1) It checks whether the user's data is in one of the special cases defined in 
Section 3.1 of the paper. To do so, it calls partially_sharp_ordered_or_not
2) It computes the Wald-DID, Wald-TC, Wald-CIC and LQTE point estimates 
requested by the user, running:
- maxi_boot for the Wald-DID, Wald-TC, Wald-CIC without covariates ;
- maxi_bootX for the Wald-DID, Wald-TC, Wald-CIC with covariates ;
- lqte_no_se for the LQTE.
*/

quietly capture program drop maxi_boot_wrapper
quietly program maxi_boot_wrapper, eclass
			...
			if r(N)!=0 {
				...
				foreach A of local G_star_list {			
					...
					if r(N)!=0 {					
						...
						partially_sharp_ordered_or_not is another subprogram. */
						if `special'==0 & ///
						("`tc'"!="" | "`cic'"!="" | "`lqte'"!="") {
						
							tempname is_part_sharp
							
							partially_sharp_ordered_or_not `A' `indic_t' ///
							`D' if (`A'==1 | `A'==0), ///
						...
						if `error_part_sharp'==0 | "`did'"!="" {
							...
							if "`continuous'"!="" | "`qualitative'"!="" {
								...
								if `error_part_sharp'==1 {
									...
									capture maxi_bootX `Y' `A' `indic_t' `D' if (`A'==1 | `A'==0), ///
								...
								if ("`did'"!="" | "`tc'"!="" | "`cic'"!="") {
									...
									if `error_part_sharp'==1 {
										...
										capture maxi_boot `Y' `A' `indic_t' `D' ///
								...
								if "`lqte'"!="" & `error_part_sharp'==0 {				
									...
									capture lqte_no_se `Y' `A' `indic_t' `D', ///
