/*
maxi_bootX.ado performs the following operation:
1) It computes the Wald-DID and Wald-TC with covariates, calling 
build_cond_expectation_1. If the tc option was specified, it also calls
build_cond_expectation_2. If the sieve optionwas specified, it also calls 
legendrisation to compute the Legendre polynomials in X. If the sieve option was 
specified, but sieve_order was not, it also calls five_fold_cv to determine the 
polynomial order to be used in the series approximation.
*/

quietly capture program drop maxi_bootX
quietly program maxi_bootX, eclass
