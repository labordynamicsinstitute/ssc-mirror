...
{p 4 4 2}
{cmd:. mediate (regress M T x) (regress Y T M x) , treat(T) mediate(M) sims(1000) seed(1){break}
{cmd:. mediate (probit M T x) (regress Y T M x) , treat(T) mediate(M) sims(1000) }{break}
{cmd:. mediate (regress M T x) (probit Y T M x) , treat(T) mediate(M) sims(1000) }{break}
{cmd:. mediate (regress M T x) (probit Y T M x) if x>0 , treat(T) mediate(M) sims(1000) }{break}


{title:Remarks}

{p} {cmd:mediate} only supports OLS, probit, and logit in either stage of the analysis so far. 
...
Currently the mediate function does not handle interactions between the treatment and mediator variables, or interactions between pre-treatment x variables and either the treatment or mediator.
