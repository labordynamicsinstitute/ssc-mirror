...
program  mimrgns_get_options
    ...
    global mimrgns__marg_opts  ${mimrgns__marg_opts} `marginsopts' `options'
end
...
program mimrgns_report
    ...
    if ("`e(over)'" != "") local Legend `"`Legend'"' _newline ///
        `"{txt}over{col 14}: {res}`e(over)'"'
    
    if ("`e(within)'" != "") local Legend `"`Legend'"' _newline ///
        `"{txt}within{col 14}: {res}`e(within)'"'
    
    if ("`e(margin_method)'" != "") local Legend `"`Legend'"' _newline ///
        `"{txt}Margins{col 14}: {res}`e(margin_method)'"'
    
    if ( !inlist("`e(emptycells)'", "", "strict") ) local Legend ///
        `"`Legend'"'  _newline `"{txt}Empty cells{col 14}: `e(emptycells)'"'
...
/*
    copied from StataCorp _marg_report.ado *! version 1.6.5  06may2019
    
    verbatim except ...
        
        we allow any options
        we change b<strfcn>() to <strfcn>()
        we omit a block of code in Stata version < 12
*/
program mimrgns_report_atlegend
    ...
    forval i = 1/`r' {
        if `r' > 1 {
            if (c(stata_version) < 12) local title `i'._at
            local first 1
        }
    ...
    forval g = 1/`k_by' {
        ...
        if (c(stata_version) >= 12) { // begin Stata version 12+
        ...
        } // end Stata version 12+
        
        else { // begin Stata version 11
        
            /*
                copied verbatim from StataCorp 
                _marg_report *! version 1.4.0  09may2017
            */
        if `k_by' > 1 {
...
4.0.5 16nov2020 update StataCorp. _marg_report routine to version 16
                revised help file
4.0.4 09may2020 new non-documented option -marginsopts()-
                revised help file: add factor-variables notation
                never released
