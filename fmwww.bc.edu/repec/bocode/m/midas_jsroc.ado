...
*! Joint summary ROC curve with confidence/prediction regions for
*! diagnostic test accuracy meta-analysis (MIDAS suite post-estimation)
capture program drop midas_jsroc

program define midas_jsroc, rclass byable(recall) sortpreserve
	version 13

	syntax [if] [in], [Level(cilevel) Weighted CEllipse PEllipse CRegion ///
		PRegion SCurve CURVEcolor(string asis) CURVEopts(string asis) ///
		noEXTRAPolate CONFcolor(string asis) PREDcolor(string asis) ///
		SUMMcolor(string asis) POINTopts(string asis) ///
		SUMMopts(string asis) MEan Data LABELdata LGnd LGNPos(integer 6) AREA *]

	// If no plot element was requested, default to drawing the SROC curve
	if missing("`scurve'`cregion'`cellipse'`pregion'`pellipse'`data'`mean'") {
		local scurve "scurve"
	}
	...
	qui {
		...
		// Generate points for curve and ellipses/regions
		tempvar CPI
		local NP = 1000
		...
		// Generate SROC curve and compute AUC (always on the full curve)
		tempvar x yroc yplot
		range `x' 0 1 `NP'
		...
		*/((invnormal(0.975)^2)/(4*N)))/N))/(1+((invnormal(0.975)^2)/N)))
		local aucnote: di "AUC = " %3.2f `AUC' " [" %3.2f `AUClo' " - " %3.2f `AUChi' "]"

		// Display copy of the curve; optionally restricted to the range of
		// observed specificities (no extrapolation beyond the data)
		gen double `yplot' = `yroc'
		if !missing("`extrapolate'") {
			su `spec', meanonly
			replace `yplot' = . if `x' < r(min) | `x' > r(max)
		}
		...
		// SROC curve
		if !missing("`scurve'") {
			if missing("`curvecolor'") {
				local ccol "black"
			}
			else {
				local ccol "`curvecolor'"
			}
			local _cvopts = cond(!missing("`curveopts'"), "`curveopts'", "clpat(solid) clc(`ccol') clw(medium)")
			local sroccurve `"(line `yplot' `x', sort `_cvopts')"'
			local legend `"`legend' label(`li' "SROC Curve" "`aucnote'")"'
			local order "`order' `li'"
			local ++li
		}
		...
		// Create the plot
		#delimit;
		nois twoway `pregion' `pellipse' `cregion' `cellipse' `sroccurve' `observed' `summ',
			`lgnd'
	...
	// Display AUC if curve requested
	if !missing("`scurve'") {
		di as text _n "Summary ROC Curve"
		di as text "{hline 60}"
		di as text "Area under SROC curve (AUC)" _col(30) as result %6.4f `AUC' _col(45) "(95% CI)" _col(55) "(" %6.4f `AUClo' ", " %6.4f `AUChi' ")"
		di as text "{hline 60}"
	}
	...
	return scalar beta  = `mbeta'
	return scalar alpha = `malpha'
end
