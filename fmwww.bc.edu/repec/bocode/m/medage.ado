...
program define medage, rclass byable(recall)
    ...
    if "`saving'" != "" {
        ...
        quietly {
            ...
            // Absolute sort for cumulative accuracy
            ...
            // Identify the median interval row
            ...
            // Siegel & Swanson Interpolation (5-year interval)
    ...
    // --- INTERACTIVE MODE (Display Result) ---
    else {
        quietly {
        ...
        if `n_total' > 0 & `m_age' != . {
            ...
            display as error "Zero population for `vlab'"
        }
        restore
    }
end
