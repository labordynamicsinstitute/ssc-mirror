...
*! version 1.0.5, April 21, 2022 (moranplot.ado)

capture program drop moranplot
program define moranplot
version 13.0
   syntax varlist(numeric) [if], w(name) id(varname) [note(numlist min=1 max=1)] [mlabel(varname)] [MLABVPosition(varname)]
      
	  local type: type `id'  
	  if substr("`type'", 1, 3)=="str" {
      di as err "string variable not allowed in id();"
	  di as err "`id' is a string variable;"
	  exit 198
      } 
   ...
   if "`mlabvposition'" != "" {
	  ...
	  if "`if'" != ""{
		 ...
		 tokenize `if', parse("==")
	     if "`note'" != ""{
		   quietly foreach z of local varlist {
		   quietly summarize `z'
		   quietly tempvar var
	       quietly capture generate `var'=(`z'-r(mean))/r(sd)
	       quietly capture spmat lag double `w'`var' `w' `var', id(`id')
	       quietly regress `w'`var' `var', cformat(%5.3f)
	       local num=string(_b[`var'],"%5.3f")
		   capture graph drop `z'_`3'
		   if "`mlabel'" != ""{
	       quietly graph twoway (scatter `w'`var' `var', msymbol(Oh) msize(medlarge) mcolor(black) mlabel(`mlabel') mlabvposition(`mlabvposition') ///
		                        mlabcolor(black) yscale(lcolor(none)) xscale(lcolor(none))) || ///
								(lfit `w'`var' `var', estopts(noc) lpattern(solid) lcolor(black) lwidth(medium) yscale(lcolor(none)) xscale(lcolor(none))), ///
								ytitle(Wz, size(medium) color(black)) xtitle(z,size(medium) color(black)) ///
								yline(0, lpattern(dash) lcolor(gs9)) xline(0, lpattern(dash) lcolor(gs9)) ///
								legend(off) plotregion(lstyle(yxline) lcolor(black)) ylabel(,nogrid format(%9.1f)) xlabel(,nogrid format(%9.1f))///
								title(Moran scatterplot of `z' (Moran' I = `num'), size(medium) color(black)) ///
								note(The moran scatterplot of `z' in `note', size(medsmall) color(black)) name(`z'_`3')
		                      }
		   else{
		   quietly graph twoway (scatter `w'`var' `var', msymbol(Oh) msize(medlarge) mcolor(black) yscale(lcolor(none)) xscale(lcolor(none))) || ///
		                        (lfit `w'`var' `var', estopts(noc) lpattern(solid) lcolor(black) lwidth(medium) yscale(lcolor(none)) xscale(lcolor(none))), ///
								ytitle(Wz, size(medium) color(black)) xtitle(z,size(medium) color(black)) yline(0, lpattern(dash) lcolor(gs9)) ///
								xline(0, lpattern(dash) lcolor(gs9)) legend(off) plotregion(lstyle(yxline) lcolor(black)) ylabel(,nogrid format(%9.1f)) xlabel(,nogrid format(%9.1f)) ///
								title(Moran scatterplot of `z' (Moran' I = `num'), size(medium) color(black)) ///
								note(The moran scatterplot of `z' in `note', size(medsmall) color(black)) name(`z'_`3')	
		       }
	       capture drop `w'_*
	       ...
	       if "`mlabel'" != ""{
	       quietly graph twoway (scatter `w'`var' `var', msymbol(Oh) msize(medlarge) mcolor(black) mlabel(`mlabel') mlabvposition(`mlabvposition') ///
		                        mlabcolor(black) yscale(lcolor(none)) xscale(lcolor(none))) || (lfit `w'`var' `var', estopts(noc) ///
								lpattern(solid) lcolor(black) lwidth(medium) yscale(lcolor(none)) xscale(lcolor(none))), ///
								ytitle(Wz, size(medium) color(black)) xtitle(z,size(medium) color(black)) ///
								yline(0, lpattern(dash) lcolor(gs9)) xline(0, lpattern(dash) lcolor(gs9)) legend(off) ///
								plotregion(lstyle(yxline) lcolor(black)) ylabel(,nogrid format(%9.1f)) xlabel(,nogrid format(%9.1f)) ///
								title(Moran scatterplot of `z' (Moran' I = `num'), size(medium) color(black)) name(`z'_`3')
		                      }
		   else{
		   quietly graph twoway (scatter `w'`var' `var', msymbol(Oh) msize(medlarge) mcolor(black) yscale(lcolor(none)) xscale(lcolor(none))) || ///
		                        (lfit `w'`var' `var', estopts(noc) lpattern(solid) lcolor(black) lwidth(medium) yscale(lcolor(none)) ///
								xscale(lcolor(none))), ytitle(Wz, size(medium) color(black)) xtitle(z,size(medium) color(black)) ///
								yline(0, lpattern(dash) lcolor(gs9)) xline(0, lpattern(dash) lcolor(gs9)) ///
								legend(off) plotregion(lstyle(yxline) lcolor(black)) ylabel(,nogrid format(%9.1f)) xlabel(,nogrid format(%9.1f)) ///
								title(Moran scatterplot of `z' (Moran' I = `num'), size(medium) color(black)) name(`z'_`3')
		       }
	     ...
	     if "`note'" != ""{
	       ...
	       quietly regress `w'`var' `var', cformat(%5.3f)
	       local num=string(_b[`var'],"%5.3f")
		   capture graph drop `z'
	       if "`mlabel'" != ""{
	       quietly graph twoway (scatter `w'`var' `var', msymbol(Oh) msize(medlarge) mcolor(black) mlabel(`mlabel') mlabvposition(`mlabvposition') ///
		                        mlabcolor(black) yscale(lcolor(none)) xscale(lcolor(none))) || (lfit `w'`var' `var', estopts(noc) lpattern(solid) ///
								lcolor(black) lwidth(medium) yscale(lcolor(none)) xscale(lcolor(none))), ytitle(Wz, size(medium) color(black)) ///
								xtitle(z,size(medium) color(black)) yline(0, lpattern(dash) lcolor(gs9)) xline(0, lpattern(dash) lcolor(gs9)) ///
								legend(off) plotregion(lstyle(yxline) lcolor(black)) ylabel(,nogrid format(%9.1f)) xlabel(,nogrid format(%9.1f)) ///
								title(Moran scatterplot of `z' (Moran' I = `num'), size(medium) color(black)) ///
								note(The moran scatterplot of `z' in `note', size(medsmall) color(black)) name(`z')
		                      }
		   else{
		   quietly graph twoway (scatter `w'`var' `var', msymbol(Oh) msize(medlarge) mcolor(black) yscale(lcolor(none)) xscale(lcolor(none))) || ///
		                        (lfit `w'`var' `var', estopts(noc) lpattern(solid) lcolor(black) lwidth(medium) yscale(lcolor(none)) xscale(lcolor(none))), ///
	       ...
	       if "`mlabel'" != ""{
	       quietly graph twoway (scatter `w'`var' `var', msymbol(Oh) msize(medlarge) mcolor(black) mlabel(`mlabel') mlabvposition(`mlabvposition') ///
		                        mlabcolor(black) yscale(lcolor(none)) xscale(lcolor(none))) || (lfit `w'`var' `var', estopts(noc) lpattern(solid) lcolor(black) ///
		                        lwidth(medium) yscale(lcolor(none)) xscale(lcolor(none))), ytitle(Wz, size(medium) color(black)) ///
		                        xtitle(z,size(medium) color(black)) yline(0, lpattern(dash) lcolor(gs9)) xline(0, lpattern(dash) lcolor(gs9)) ///
		                        legend(off) plotregion(lstyle(yxline) lcolor(black)) ylabel(,nogrid format(%9.1f)) xlabel(,nogrid format(%9.1f)) ///
		                        title(Moran scatterplot of `z' (Moran' I = `num'), size(medium) color(black)) name(`z')
		                      }
		   else{
		   quietly graph twoway (scatter `w'`var' `var', msymbol(Oh) msize(medlarge) mcolor(black) yscale(lcolor(none)) xscale(lcolor(none))) || ///
		                        (lfit `w'`var' `var', estopts(noc) lpattern(solid) lcolor(black) lwidth(medium) yscale(lcolor(none)) xscale(lcolor(none))), ///
		                        ytitle(Wz, size(medium) color(black)) xtitle(z,size(medium) color(black)) ///
								yline(0, lpattern(dash) lcolor(gs9)) xline(0, lpattern(dash) lcolor(gs9)) legend(off) plotregion(lstyle(yxline) ///
								lcolor(black)) ylabel(,nogrid format(%9.1f)) xlabel(,nogrid format(%9.1f)) ///
								title(Moran scatterplot of `z' (Moran' I = `num'), size(medium) color(black) name(`z')
		       }
		   capture drop `w'_*
	  ...
	  if "`if'" != ""{
	     ...
	     if "`note'" != ""{
		   ...
		   capture graph drop `z'_`3'
		   if "`mlabel'" != ""{
	       quietly graph twoway (scatter `w'`var' `var', msymbol(Oh) msize(medlarge) mcolor(black) mlabel(`mlabel') mlabcolor(black) ///
		                        yscale(lcolor(none)) xscale(lcolor(none))) || (lfit `w'`var' `var', estopts(noc) lpattern(solid) ///
								lcolor(black) lwidth(medium) yscale(lcolor(none)) xscale(lcolor(none))), ///
								ytitle(Wz, size(medium) color(black)) xtitle(z,size(medium) color(black)) ///
								yline(0, lpattern(dash) lcolor(gs9)) xline(0, lpattern(dash) lcolor(gs9)) legend(off) ///
								plotregion(lstyle(yxline) lcolor(black)) ylabel(,nogrid format(%9.1f)) xlabel(,nogrid format(%9.1f)) ///
								title(Moran scatterplot of `z' (Moran' I = `num'), size(medium) color(black)) ///
	       ...
	       quietly graph twoway (scatter `w'`var' `var', msymbol(Oh) msize(medlarge) mcolor(black) mlabel(`mlabel') mlabcolor(black) ///
		                        yscale(lcolor(none)) xscale(lcolor(none))) || (lfit `w'`var' `var', estopts(noc) lpattern(solid) ///
								lcolor(black) lwidth(medium) yscale(lcolor(none)) xscale(lcolor(none))), ytitle(Wz, size(medium) ///
								color(black)) xtitle(z,size(medium) color(black)) ///
								yline(0, lpattern(dash) lcolor(gs9)) xline(0, lpattern(dash) lcolor(gs9)) legend(off) ///
								plotregion(lstyle(yxline) lcolor(black)) ylabel(,nogrid format(%9.1f)) xlabel(,nogrid format(%9.1f)) ///
								title(Moran scatterplot of `z' (Moran' I = `num'), size(medium) color(black)) name(`z'_`3')
		                      }
	     ...
	     if "`note'" != ""{
		   ...
		   capture graph drop `z'
	       if "`mlabel'" != ""{
	       quietly graph twoway (scatter `w'`var' `var', msymbol(Oh) msize(medlarge) mcolor(black) mlabel(`mlabel') mlabcolor(black) yscale(lcolor(none)) ///
		                        xscale(lcolor(none))) || (lfit `w'`var' `var', estopts(noc) lpattern(solid) lcolor(black) lwidth(medium) ///
								yscale(lcolor(none)) xscale(lcolor(none))), ytitle(Wz, size(medium) color(black)) xtitle(z,size(medium) color(black)) ///
								yline(0, lpattern(dash) lcolor(gs9)) xline(0, lpattern(dash) lcolor(gs9)) legend(off) ///
								plotregion(lstyle(yxline) lcolor(black)) ylabel(,nogrid format(%9.1f)) xlabel(,nogrid format(%9.1f)) ///
								title(Moran scatterplot of `z' (Moran' I = `num'), size(medium) color(black)) ///
								note(The moran scatterplot of `z' in `note', size(medsmall) color(black)) name(`z')
...
* Version history
* 1.0.6 drop grss code
* 1.0.5 Add an option: MLABVPosition(varname)
* 1.0.4 Add error warning: "string variable not allowed in id();"
...
* The following program code is written by Drukker, D. M., H. Peng, I. R. Prucha, and R. Raciborski.
* Drukker D M, Peng H, Prucha I R, et al. Creating and managing spatial-weighting matrices with the spmat command[J]. Stata Journal, 2013,13(2):242-286.
*! version 1.0.5, 26nov2018 (spmat.ado)
