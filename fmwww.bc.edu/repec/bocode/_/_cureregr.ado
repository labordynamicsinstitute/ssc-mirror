*! version 2.2 fv swml abuxton 20Oct2013.
...
program define _cureregr,rclass
version 13.0
...
/* bgn if At_points exists -atopt- then do the following and exit the _cureregr */
if `"`timelst'"' ~= "" {
	preserve
...
capture noisily {
	...
	/* di `"qui _cureregr_gen if `touse' `alt_argument'"' */
	qui _cureregr_gen if `touse' `alt_argument'
	}
/* end if At_points exists -atopt- then do the following and exit the _cureregr */
