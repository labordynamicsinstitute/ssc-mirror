...
{hi:case.oesch()} {hline 2} OESCH case function

{title:Syntax}

        {cmd:case.oesch(}{help varname:{it:sempl}} [{help varname:{it:supvis}}]{cmd:)}
...
{pstd}
    {helpb crosswalk} case function for use with ISCO to OESCH
...
replace `case' = 2 if `sempl'!=0 & `sempl'<. & `touse'
if "`supvis'"!="" {
    replace `case' = 3 if `supvis'>=1  & `supvis'<10 & `case'==2 & `touse'
    replace `case' = 4 if `supvis'>=10 & `supvis'<.  & `case'==2 & `touse'
}
