...
capture program drop _teffects2_parse_canonicalize
program define _teffects2_parse_canonicalize, sclass
	version 17
	...
	syntax anything(id="`st'teffects2 `cmd' specification" name=teffects2) ///
	[if] [in] [pw], [ * ]
	
	local tmp : subinstr local teffects2 "|" "|", all count(local vert)
	if `vert' {
		di as err "{p}invalid {bf:`st'teffects2 `cmd'} " ///
	...
	local 0 `teffects2' `if' `in' `owep', `options'
	
	local tmp : subinstr local teffects2 "(" "(", all count(local left)
	local tmp : subinstr local teffects2 ")" ")", all count(local right)
