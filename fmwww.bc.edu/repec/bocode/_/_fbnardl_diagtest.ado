...
program define _fbnardl_diagtest, rclass
    version 17

    syntax, RESIDvar(string) NOBS(integer) NPARAMS(integer) ///
        [ LHS(string) RHS(string) ESAMPle(varname) NOGRAPHs ]
    ...
    local esopt ""
    if "`esample'" != "" local esopt "esample(`esample')"
    _fbnardl_cusum, lhs(`lhs') rhs(`rhs') `esopt' `nographs'
...
program define _fbnardl_cusum, rclass
    version 17

    syntax, lhs(string) rhs(string) [ ESAMPle(varname) NOGRAPHs ]

    tempvar yv esamp seq xbh sef wrec
    qui gen double `yv' = `lhs'

    if "`esample'" != "" qui regress `lhs' `rhs' if `esample'
    else                 qui regress `lhs' `rhs'
    local k = e(df_m) + 1
...
// =============================================================================
capture program drop _fbnardl_rval
program define _fbnardl_rval
    version 17
    tempname M
    local s = .
    local pp = .
