    ...
    if (_rc != 0) {
        ...
        * Extract version from yaml.ado header
        ...
        if (_rc == 0) {
            ...
            _unicefdata_parse_yaml_version "`yaml_path'"
            local yaml_version = r(version)
            
            if ("`yaml_version'" != "") {
                * Compare versions
                _unicefdata_compare_versions "`yaml_version'" "`minversion'"
                if (r(newer) == 1) {
    ...
    *---------------------------------------------------------------------------
    if (`needs_install') {
        ...
        * Get installed version
        capture findfile yaml.ado
        if (_rc == 0) {
            _unicefdata_parse_yaml_version "`r(fn)'"
            local yaml_version = r(version)
        }
    }
...
*******************************************************************************
program define _unicefdata_parse_yaml_version, rclass
    version 14.0
    args yaml_path
...
program define _unicefdata_compare_versions, rclass
    version 14.0
    args v1 v2
