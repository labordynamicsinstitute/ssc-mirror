*! _fbnardl_dynmult - dynamic multipliers for fbnardl (linear and asymmetric)
...
program define _fbnardl_dynmult, rclass
    version 17
    ...
    di as txt ""
    di as txt "{hline 78}"
    di as res _col(5) "Dynamic Multipliers: cumulative paths with confidence bands"
    ...
    foreach sv of local shocks {
        ...
        mata: _fbnardl_dm("`bb'", "`VV'", "`IDX'", `plags', `qm', `horizon', ///
                        `bands', `level')
        ...
        if "`nograph'" == "" {
            _fbnardl_dmplot, mat(`M`m'') title("Cumulative dynamic multiplier: `sv'") ///
                name(`graphprefix'dm_`sv') ytitle("Cumulative effect on `depvar'")
        }
    }
    ...
    if "`pairs'" != "" {
        di as txt ""
        di as txt "{hline 78}"
        di as res _col(5) "Dynamic Multipliers: asymmetric paths M+(h), M-(h)"
        ...
        foreach base of local pairs {
            ...
            mata: _fbnardl_dmasym("`bb'", "`VV'", "`IP'", "`IN'", `plags', ///
            ...
            if "`nograph'" == "" {
                _fbnardl_asymplot, mat(`A`pj'') base("`base'") ///
                    name(`graphprefix'asym_`base') level(`level') depvar("`depvar'")
            }
        }
    }
...
// plot helpers
// -------------------------------------------------------------------------
capture program drop _fbnardl_dmplot
program define _fbnardl_dmplot
    version 17
    syntax , MAT(string) TItle(string) NAME(string) [ YTitle(string) ]

    mat _fbnardl_dmtmp = `mat'
    local nr = rowsof(_fbnardl_dmtmp)
    preserve
    capture noisily {
        qui clear
        qui set obs `nr'
        ...
        qui gen double hi = .
        forvalues i = 1/`nr' {
            qui replace h  = el(_fbnardl_dmtmp,`i',1) in `i'
            qui replace m  = el(_fbnardl_dmtmp,`i',2) in `i'
            qui replace lo = el(_fbnardl_dmtmp,`i',3) in `i'
            qui replace hi = el(_fbnardl_dmtmp,`i',4) in `i'
        }
    ...
    restore
    capture mat drop _fbnardl_dmtmp
    capture graph display `name'
    capture qui graph export "`name'.png", replace width(1200)
    if _rc == 0 di as txt _col(5) "Graph saved: `name'.png"
end

capture program drop _fbnardl_asymplot
program define _fbnardl_asymplot
    version 17
    syntax , MAT(string) BASE(string) NAME(string) [ Level(cilevel) DEPvar(string) ]

    mat _fbnardl_astmp = `mat'
    local nr = rowsof(_fbnardl_astmp)
    preserve
    capture noisily {
        qui clear
        qui set obs `nr'
        ...
        qui gen double dhi = .
        forvalues i = 1/`nr' {
            qui replace h   = el(_fbnardl_astmp,`i',1) in `i'
            qui replace mp  = el(_fbnardl_astmp,`i',2) in `i'
            qui replace mn  = el(_fbnardl_astmp,`i',3) in `i'
            qui replace df  = el(_fbnardl_astmp,`i',4) in `i'
            qui replace dlo = el(_fbnardl_astmp,`i',5) in `i'
            qui replace dhi = el(_fbnardl_astmp,`i',6) in `i'
        }
    ...
    restore
    capture mat drop _fbnardl_astmp
    capture graph display `name'
    capture qui graph export "`name'.png", replace width(1200)
    if _rc == 0 di as txt _col(5) "Graph saved: `name'.png"
end
...
real colvector _fbnardl_dmpath(real rowvector b, real rowvector idx,
...
real matrix _fbnardl_draws(real rowvector b, real matrix V, real scalar B)
{
    real matrix L, Z, Vs
    Vs = makesymmetric(V)
    L  = cholesky(Vs)
...
void _fbnardl_dm(string scalar bn, string scalar vn, string scalar idxn,
               real scalar p, real scalar q, real scalar H,
               real scalar B, real scalar lvl)
{
    ...
    M = _fbnardl_dmpath(b, idx, p, q, H)

    out = ((0::H), M, J(H+1,1,.), J(H+1,1,.))
    if (B > 10) {
        Db = _fbnardl_draws(b, V, B)
        if (rows(Db) > 0) {
            S = J(H+1, rows(Db), .)
            for (i=1; i<=rows(Db); i++) S[.,i] = _fbnardl_dmpath(Db[i,], idx, p, q, H)
...
void _fbnardl_dmasym(string scalar bn, string scalar vn, string scalar ipn,
                   ...
                   real scalar qn, real scalar H, real scalar B, real scalar lvl)
{
    ...
    Mp = _fbnardl_dmpath(b, ip,  p, qp, H)
    Mn = _fbnardl_dmpath(b, inx, p, qn, H)
    ...
    if (B > 10) {
        ...
        if (rows(Db) > 0) {
            S = J(H+1, rows(Db), .)
            for (i=1; i<=rows(Db); i++) {
                S[.,i] = _fbnardl_dmpath(Db[i,], ip, p, qp, H) -
                         _fbnardl_dmpath(Db[i,], inx, p, qn, H)
            }
            lo = max((1, round(rows(Db)*(1-lvl/100)/2)))
            hi = min((rows(Db), round(rows(Db)*(1 - (1-lvl/100)/2))))
            for (i=1; i<=H+1; i++) {
