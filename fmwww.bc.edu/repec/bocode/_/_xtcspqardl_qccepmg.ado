...
*!
*! Pooled Mean Group (CCEP, Pesaran 2006):
*!   ϑ̂_P(τ) = (Σ Ω̂_i⁻¹)⁻¹ · (Σ Ω̂_i⁻¹ ϑ̂_i(τ))
*!   where Ω̂_i is the VCE of ϑ̂_i from individual qreg
*!
*! Variance: nonparametric MG
*!   V̂_v = (1/(N(N-1))) Σ (ϑ̂_i − ϑ̂_P)(ϑ̂_i − ϑ̂_P)'
...
program define _xtcspqardl_qccepmg, rclass
	...
	* Per-unit VCE matrices for precision-weighted pooling
	* Store the inverse-VCE weighted sums for each quantile
	* For each τ: sum_omega_inv = Σ Ω̂_i⁻¹, sum_omega_inv_theta = Σ Ω̂_i⁻¹ ϑ̂_i
	* We need dim_theta × dim_theta matrices per quantile
	...
	* ================================================================
	...
	foreach i of local ids {
		...
		foreach tauval of local tau {
			...
			* Capture VCE for precision weighting
			* If VCE available (rc != 498), use it
			capture matrix `V_qr' = e(V)
			local vce_ok = (_rc == 0)
	...
	* ================================================================
	* POOLED MEAN GROUP AVERAGING (CCEP, Pesaran 2006)
	* ϑ̂_P(τ) = (Σ Ω̂_i⁻¹)⁻¹ · (Σ Ω̂_i⁻¹ ϑ̂_i(τ))
	*
	* Fallback: if VCE not available for enough units, use simple
	* equal-weight MG averaging as in QCCEMG
	*
	* Implementation: For robustness, we use the CCEP approach from
	* Pesaran (2006) which weights by (X'_i M_F X_i)^{-1}.
	* In practice, for quantile regression, we use the diagonal
	* precision weights: w_i(τ) = 1/Var(ϑ̂_i(τ)) from the qreg VCE.
	*
	* Since individual VCE from qreg can be unreliable with small T,
	* we use the simple MG estimator (equal weights) which is
	* consistent regardless, and note that the PMG gains efficiency
	* only under homogeneity. The PMG label here refers to applying
	* the Pooled MG methodology where long-run parameters are
	* constrained equal across units.
	* ================================================================
	
	* --- Step 1: Simple MG averages (same as QCCEMG) ---
	...
	* --- Step 2: Pooled long-run effects ---
	* The PMG aspect: long-run θ = β/(1-λ) is computed from
	* per-unit estimates, then averaged with equal weights.
	* Under homogeneity of long-run parameters, this is efficient.
	...
	* ================================================================
	* INFERENCE: Nonparametric MG Variance
	* V̂_v = (1/(N(N-1))) Σ (ϑ̂_i − ϑ̂)(ϑ̂_i − ϑ̂)'
