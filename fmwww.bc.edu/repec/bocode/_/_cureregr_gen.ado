...
*! main program cureregr.ado, called by _cureregr.ado which is run via predict switch 
...
* any model fit by cureregr, 

program define _cureregr_gen
version 13.0
	...
	if lower(substr(`"`svopt'"',1,1))==`"s"'  { 
		...
		di `"{err}please, check _cureregr_gen {S, ses, lcs, ucs, h, fd, or all} only {txt}"'
		exit 
	} 
