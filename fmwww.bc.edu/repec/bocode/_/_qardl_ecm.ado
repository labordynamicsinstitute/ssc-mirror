...
capture mata: mata drop _qardl_ecm_estimate()

mata:
mata set matastrict off

void _qardl_ecm_estimate(real colvector yy, real matrix xx,
    real scalar ppp, real scalar qqq, real colvector tau)
{
    real scalar nn, k0, ss, jj, ii, i, rr, neff
    ...
    // Data construction (same as standard QARDL)
    ...
    if (ppp > qqq) {
        neff = nn - ppp
        real matrix yyj_ecm, xxj_ecm, wwj_ecm
        ...
        for (jj = 1; jj <= ppp; jj++) {
            Y_vec = yyj_ecm[., jj]
            ONEX = (J(neff, 1, 1), xxj_ecm, wwj_ecm)
            for (ii = 1; ii <= ss; ii++) {
        ...
        neff = nn - qqq
        real matrix yyj_e2, xxj_e2, wwj_e2
        yyj_e2 = J(neff, ppp, 0)
        ...
        for (jj = 1; jj <= ppp; jj++) {
            Y_vec = yyj_e2[., jj]
            ONEX = (J(neff, 1, 1), xxj_ecm, wwj_e2)
            for (ii = 1; ii <= ss; ii++) {
                bbt = _qardl_qreg(Y_vec, ONEX, tau[ii])
                kkk_vec = Y_vec - ONEX * bbt
                kk[., jj+(ii-1)*ppp] = kkk_vec
    ...
    // Remove first column of each tau block (ECM specific)
    // dd1 contains indices to remove: 1, 1+ppp, 1+2*ppp, ...
    real colvector dd_idx, keep_idx
    dd_idx = J(ss, 1, 0)
    dd_idx[1] = 1
    for (jj = 2; jj <= ss; jj++) {
    ...
    // Build kka by removing columns at dd_idx
    ...
    // Apply upper triangular cumulative sum transformation
    A = J(pp1, pp1, 0)
    for (i = 1; i <= pp1; i++) {
        for (jj = i; jj <= pp1; jj++) {
            A[i, jj] = 1
    ...
    for (ii = 1; ii <= ss; ii++) {
        for (jj = 1; jj <= ss; jj++) {
            ...
            // distmt3 - lower right block contribution
    ...
    // Store ECM results
    st_matrix("_qardl_phi_ecm", bigphia)
    st_matrix("_qardl_phi_ecm_cov", bigpia)
    st_matrix("_qardl_theta", thett)
    st_matrix("_qardl_theta_cov", distthett)
}
