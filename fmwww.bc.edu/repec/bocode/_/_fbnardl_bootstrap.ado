*! _fbnardl_bootstrap — recursive, null-imposed bootstrap engine for fbnardl
...
program define _fbnardl_bootstrap, rclass
    ...
    mata: _fbnardl_bootrun("`depvar'", "`xvars'", "`detvars'", "`touse'", `p', ///
...
real matrix _fbnardl_W(real colvector y, real matrix X, real matrix D,
...
real matrix _fbnardl_M(real colvector y, real matrix X, real matrix D,
                     real colvector dy, real matrix dX,
...
real rowvector _fbnardl_wrow(real colvector ys, real matrix xs, real colvector dys,
...
real rowvector _fbnardl_mrow(real colvector ys, real matrix xs, real colvector dys,
                           real matrix dxs, real matrix D, real scalar t,
...
real matrix _fbnardl_vcov(real matrix W, real colvector e, real matrix XXi,
...
real scalar _fbnardl_waldF(real colvector b, real matrix V, real colvector idx)
{
    real matrix RVR, IR
    real colvector Rb
    real scalar q
...
real rowvector _fbnardl_stats(real colvector yv, real matrix W,
                            ...
                            real scalar vcode, real scalar hlag)
{
    ...
    e = yv - W*b
    V = _fbnardl_vcov(W, e, XXi, vcode, hlag)
    if (V[it,it] <= 0) tst = .
    else               tst = b[it]/sqrt(V[it,it])
    return((_fbnardl_waldF(b,V,ifov), tst, _fbnardl_waldF(b,V,ifind)))
}

// ---- Restricted fit: coefficient vector padded with zeros -----------------
real colvector _fbnardl_restfit(real colvector yv, real matrix W,
...
real scalar _fbnardl_cvupper(real colvector s, real scalar alpha)
{
    real scalar B, j
    B = rows(s)
    if (B < 3) return(.)
...
real scalar _fbnardl_cvlower(real colvector s, real scalar alpha)
{
    real scalar B, j
    B = rows(s)
    if (B < 3) return(.)
...
// ---- Main driver ----------------------------------------------------------
void _fbnardl_bootrun(string scalar yn, string scalar xn, string scalar dn,
                    ...
                    real scalar xrw)
{
    ...
    W  = _fbnardl_W(y, X, D, dy, dX, p, qv, s0)
    yv = dy[|(s0+1) \ T|]
    kk = cols(W)
    ...
    // ---- observed statistics ---------------------------------------------
    obs = _fbnardl_stats(yv, W, ifov, it, ifind, vcode, hlag)
    ...
    resid = J(N, 1, 0)
    for (s=1; s<=nnull; s++) {
        if (s == 1)      byn[.,s] = _fbnardl_restfit(yv, W, ifov,  resid)
        else if (s == 2) byn[.,s] = _fbnardl_restfit(yv, W, J(1,1,it), resid)
        else             byn[.,s] = _fbnardl_restfit(yv, W, ifind, resid)
        nu[.,s] = resid
    }
    ...
    pm = p
    M  = _fbnardl_M(y, X, D, dy, dX, pm, s0, useylev, 1 - xrw)
    bm  = J(cols(M), K, 0)
    eps = J(N, K, 0)
    for (i=1; i<=K; i++) {
        bm[.,i]  = _fbnardl_restfit(dX[|(s0+1),i \ T,i|], M, J(0,1,.), resid)
        eps[.,i] = resid
    }
    ...
    for (b=1; b<=B; b++) {
        for (s=1; s<=nnull; s++) {
            ...
            for (t=s0+1; t<=T; t++) {
                nn   = t - s0
                mrow = _fbnardl_mrow(ys, xs, dys, dxs, D, t, pm, useylev, 1 - xrw)
                for (i=1; i<=K; i++) dxs[t,i] = mrow*bm[.,i] + epsb[nn,i]
                xs[t,] = xs[t-1,] + dxs[t,]
                wrow   = _fbnardl_wrow(ys, xs, dys, dxs, D, t, p, qv)
                dys[t] = wrow*bfull + nub[nn]
                ys[t]  = ys[t-1] + dys[t]
            }

            st = _fbnardl_stats(dys[|(s0+1) \ T|],
                              _fbnardl_W(ys, xs, D, dys, dxs, p, qv, s0),
    ...
    st_matrix("_fbnardl_bootdist", boot)
    nvalid = 0
    for (ic=1; ic<=3; ic++) {
        cl = boot[.,ic]
        ...
        if (ic == 1) nvalid = nn
        if (ic == 2) {
            st_numscalar("r(tDV_cv10)", _fbnardl_cvlower(sc, 0.10))
            st_numscalar("r(tDV_cv5)",  _fbnardl_cvlower(sc, 0.05))
            st_numscalar("r(tDV_cv1)",  _fbnardl_cvlower(sc, 0.01))
            st_numscalar("r(tDV_bp)",   (nn>0 ? mean(sc :<= obs[2]) : .))
        }
        else {
            nm = (ic == 1 ? "Fov" : "Find")
            st_numscalar("r("+nm+"_cv10)", _fbnardl_cvupper(sc, 0.10))
            st_numscalar("r("+nm+"_cv5)",  _fbnardl_cvupper(sc, 0.05))
            st_numscalar("r("+nm+"_cv1)",  _fbnardl_cvupper(sc, 0.01))
