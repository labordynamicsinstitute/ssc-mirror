*! opencagegeo version 1.1.0 (06/02/2017)
*! Lars Zeigermannn
...
* Version 1.1.0:	a bug affecting user of paid keys has been fixed and a
		...
		quietly {
			...
			*** Generate tempvar work containing the location
			...
			if ("`type'" == "address") {
				...
				if ("`postcode'" != "") replace `work' = `work' + "," + `temppostcode' if `touse' & `work' != "" & `temppostcode' != "" 
				if ("`city'" != "" & "`postcode'" != "") replace `work' = `work' +" " + `city' if `touse' & `city' != ""
				if ("`city'" != "" & "`postcode'" == "") {
					replace `work' = `work' + "," if `touse' & `city' != "" & `work' != ""
					replace `work' = `work' + `city' if `touse' & `city' != ""
				}
				if ("`county'" != ""){
					replace `work' = `work' + "," if `touse' & `work' != "" & `county' != ""
					replace `work' = `work' + `county' if `touse'
				}
				if ("`state'" != "") {
					replace `work' = `work' + "," if `touse' & `work' != "" & `state' != ""
					replace `work' = `work' + `state' if `touse'	
				}
				if ("`country'" != "") {
					replace `work' = `work' + "," if `touse' & `work' != "" & `country' != ""
			...
			if ("`type'" == "latlon") {
				gen `work' = `templat' + "," + `templon' if `touse'
			}
			...
			*** Generate tempvars
			...
			g_residential g_pedestrian g_remaining g_timestamp
			...
			cap gen str224 `g_remaining' = ""
			cap gen str224 `g_timestamp' = ""
			...
			*** Generate local containing column selectors
			...
			results:1:components:pedestrian 
			
			if "`paidkey'" == "" {
				local selectors `selectors' rate:remaining timestamp:created_unix
			}
			...
			*** Loop over observations to be geocoded
			...
			forval i = `start'/`cnt' {
				...
				cap {
					...
					copy "http://api.opencagedata.com/geocode/v1/json?q=`query'&key=`key'&no_annotations=1&language=`language'&countrycode=`countrycode'&limit=1" `result_opencagegeo'.json , replace			
						
					if "`paidkey'" == "" {
						insheetjson `tobefilled' `g_town' `g_village' `g_hamlet' `g_street_name' `g_road' ///
						`g_footway' `g_residential' `g_pedestrian' `g_remaining' `g_timestamp' ///
						using `result_opencagegeo'.json, col(`selectors') flatten limit(1) offset(`offset') replace
						...
						if (`g_remaining'[`i'] == "0" ) {
							...
							* Calculate remaining time
							local tillreset = ceil(real(`g_timestamp'[`i'])/86400)*86400 - real(`g_timestamp'[`i'])

							* Following code is adapted from elap of the egenmore package written by Nick Cox
							local hh = int(`tillreset'/3600)
							local mm = int((`tillreset' - `hh'*3600)/60)
							local ss = int(`tillreset' - `hh'*3600 - `mm'*60)
							local hh = string(`hh',"%02.0f")
							local mm = string(`mm',"%02.0f")
							local ss = string(`ss',"%02.0f")
							noi di as err "Query limited exceeded: Continue geocoding in `hh':`mm':`ss' using resume option"
							continue, break
						}
					}
					else{
						insheetjson `tobefilled' `g_town' `g_village' `g_hamlet' `g_street_name' `g_road' ///
						`g_footway' `g_residential' `g_pedestrian' ///
						using `result_opencagegeo'.json, col(`selectors') flatten limit(1) offset(`offset') replace
				...
				if _rc!=0 {
					local oldrc = _rc
					qui replace `touse' = 0 if _n >= `i'
					if (`i' == `start') local immediateexit = 1
					continue, break
				}
			...
			*** Exit if error occured
			if ("`oldrc'" != "") {
				if (`oldrc' == 2) {
					noi di as err "Connection timed out. Check internet connection"
				}
				if (`oldrc' == 672) {
					if (`immediateexit' == 1) {
						noi disp as err "Invalid key or query limit exceeded: Check key or resume later"
					}
					else {
						noi disp as err "Query limit exceeded: resume later"
					}
				}
				exit `oldrc'
			}
