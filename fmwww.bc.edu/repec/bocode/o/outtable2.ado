*! outtable 1.0.9  CF Baum and JP Azevedo  20oct2025
...
* 1.0.9: Oct2025 updated to recognize N as row label and report as ( N ), integer
*
...
forv i=1/`nr' {
    ...
    forv j=1/`jlim' {
		...
		if "`dots'" ~= "nodots" & "`rnw'"!="N"  { 
			file write `hh' `fmt' (`mat'[`i',`j']) " & "
		} 
		else {
// deal with rowlabel N
			if "`rnw'"=="N" {
				file write `hh' %5.0f  "(" (`mat'[`i',`j']) " )" " & "
			}
			else {
			if mi(`mat'[`i',`j']) {
				file write `hh'  " & "
...
// end of jlim line; deal with last line
    if "`format'"!="" {
        ...
        if "`dots'" ~= "nodots" & "`rnw'"!="N" { 
                ...
                if !mi(`mat'[`i',`klim']) & "`rnw'"=="N" {
                        file write `hh'  %5.0f  "(" (`mat'[`i',`klim']) " )"
                }
        }
        file write `hh' " \" "\ `hl' " _n
    }
