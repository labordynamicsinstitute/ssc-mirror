*!version 1.0.6  Ben Jann  14jul2008
...
// post results
    ...
    PostResults `b' `V', b0(`b0') v0(`V0') esample(`touse') suest(`suest') se(`se') ///
        model(`model') weights(`weights') refcoefs(`refcoefs') ///
        detail(`detail') legend(`dlegend') ///
...
program PostResults, eclass
    syntax anything,  b0(str) esample(str) suest(str) se(str) [ ///
        model(str) weights(str) refcoefs(str) ///
    ...
    eret post `anything', esample(`esample') obs(`r(N)')
    foreach opt in prefix clustvar vcetype vce wexp wtype  {
        eret local `opt' `"``opt''"'
    }
    foreach opt in se suest {
        if ``opt'' {
            eret local `opt' `opt'
        }
    }
    foreach opt in fixed adjust legend detail refcoefs weights model {
        eret local `opt' `"``opt''"'
    }
