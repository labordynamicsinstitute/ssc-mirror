...
{synopt:{opt self:contained}}codebook is viewable on device without internet access{p_end}
...
unless either the {cmd:files()} or the {cmd:dir()} option is specified.
...
each variable it always displays the variable label, notes, and the numbers of missing 
...
specifies the Stata data files that the codebook will describe. Specifying 
multiple files is particularly useful for a dataset that is stored in multiple
files, so that you want to describe all files in one codebook.
...
{phang}
{opt self:contained} codebook is viewable on a device without internet 
access. The codebook uses the W3.CSS framework, and by default a link to the
w3.css file. If one intends to use the codebook on a device without internet
...
information. Especially in a large dataset, {cmd:htmlcb} can take a while. The 
