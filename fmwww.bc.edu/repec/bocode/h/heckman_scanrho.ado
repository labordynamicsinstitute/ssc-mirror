...
program  heckman_scanrho, eclass
    version 11
	...
	qui heckman_fixedrho `varlist' `if' `in', sel(`select') rho(`beginning')
    matrix `list_ll' = e(ll)
	di as text "Performing estimation for each value of rho..."
	local minrho2 = `minrho' + `step'
	forval i = `minrho2'(`step')`maxrho' {
		matrix `list_rho' = `list_rho' \ `i'
        qui heckman_fixedrho `varlist' `if' `in', sel(`select') rho(`i') `maxopts'
	...
	heckman_fixedrho `varlist' `if' `in', sel(`select') rho(`max_rho') level(`level') `maxopts'
	...
	* Return values for init
	...
	local nrow = colsof(`init_values')
	matrix `init_values'[1,`nrow'] = `athrho'
	matrix `init_values' = `init_values' , e(b)[1,`nrow']
