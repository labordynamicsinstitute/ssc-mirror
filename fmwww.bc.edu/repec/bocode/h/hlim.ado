...
program define hlim, eclass byable(recall) sortpreserve
	version 14
	if !replay(){
	syntax [anything(name=0)] [if] [in][, noCONStant boots(real 0) modelbased wild]
	marksample touse
	tempname b V
...
mata:  mywork("`lhs'", "`endo'", "`inexog'","`exexog'", "`touse'","`constant'", "`boots'", "`modelbased'","`wild'") 
...
void mywork( string scalar lhs,  string scalar endo, 
			 ...
			 string scalar constant, scalar boots , string scalar modelbased, string scalar wild) 
{
...
/*HLIM estimates*/
delta_tilde_HLIM=luinv(X'*P1*S*X-PXX-alpha_tilde*X'*X)*(X'*P1*S*y-PXy-alpha_tilde*X'*y)

/************************************************/
/*Variance of the HLIM estimator                */

Epsilon_hat=y-X*delta_tilde_HLIM
...
X_b=X_b1*X_b2;
Z_tilde=Z*luinv(Z'*Z)
H_hat=X'*P1*S*X-PXX-alpha_tilde*X'*X
...
/*Variance estimator*/
V_hat_HLIM=luinv(H_hat)*Sigma_hat*luinv(H_hat)

/*Standard error estimator*/
se_hat_HLIM=(diagonal(V_hat_HLIM)):^(1/2)
...
/*Case1: default    */
if (modelbased == "" && wild=="") {
HLIM_estimator=(delta_tilde_HLIM,se_hat_HLIM)
 b = delta_tilde_HLIM
 V = V_hat_HLIM
}
...
/*************************************************/
/*Bootstrap HLIM estimator (model-based sampling)*/
rseed(111)

delta_tilde_HLIM_b=J(boot,G,0)
for (s=1;s<=boot;s++){
    ...
    y_boot=X*delta_tilde_HLIM+Epsilon_hat_boot
    X_bar_boot=(y_boot,X)
    ...
    delta_tilde_HLIM_boot=luinv(X'*P1*S*X-PXX_boot-alpha_tilde_boot*X'*X)*(X'*P1*S*y_boot-PXy_boot-alpha_tilde_boot*X'*y_boot)
    delta_tilde_HLIM_b[s,.]=Re(delta_tilde_HLIM_boot')
}

se_hat_HLIM_boot=diagonal(variance(delta_tilde_HLIM_b)):^(1/2)

b = mean(delta_tilde_HLIM_b)'
V = variance(delta_tilde_HLIM_b)
}


/*Case3: model-based wild    */
if (modelbased == "" && wild!="") {

/*******************************************************/
/*Bootstrap HLIM estimator (model-based sampling, wild)*/
rseed(111)

delta_tilde_HLIM_bw=J(boot,G,0)
for (s=1;s<=boot;s++){
    Epsilon_hat_boot=J(n,1,0)
    for (t=1;t<=n;t++){
        Epsilon_hat_boot[t]=Re(Epsilon_hat[1+floor(n*runiform(1,1))])
    ...
    delta_tilde_HLIM_boot=luinv(X'*P1*S*X-PXX_boot-alpha_tilde_boot*X'*X)*(X'*P1*S*y_boot-PXy_boot-alpha_tilde_boot*X'*y_boot)
    delta_tilde_HLIM_bw[s,.]=Re(delta_tilde_HLIM_boot')
}

se_hat_HLIM_boot_w=diagonal(variance(delta_tilde_HLIM_bw)):^(1/2)
b = mean(delta_tilde_HLIM_bw)'
V = variance(delta_tilde_HLIM_bw)
}
 st_matrix("V",V)
 st_matrix("b",b)
}
