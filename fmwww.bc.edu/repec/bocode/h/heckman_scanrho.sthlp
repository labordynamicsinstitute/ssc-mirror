...
{cmd:help heckman_scanrho}
{hline}

{title:Title}

{p2colset 5 21 23 2}{...}
{p2col :{cmd:heckman_scanrho} {hline 2}}A linear regression with sample selection that maximizes the likelihood function by scanning over values of "rho"{p_end}
{p2colreset}{...}

{title:Syntax}

{p 8 17 2}
{cmd:heckman_scanrho} {depvar} {indepvars} {ifin} {cmd:,}
...
{p2colreset}{...}
{p 4 6 2}
* {opt select()} is required. Note that, unlike {cmd:heckman}, {it:depvar_s} = must be specified.
...
{pstd}
{cmd:heckman_scanrho} This is a modification of Stata's {cmd:heckman} that allows the user to scan over values of "rho," the correlation between the unobservables. This command can be used to verify that the results reported by {cmd:heckman} are the maximum likelihood estimates. For more details on {cmd:heckman}, see {manhelp heckman R}.
...
{phang2}{cmd:. heckman_scanrho  lwage educ exper expersq city,sel(inlf= age  agesq nwifeinc  child educ) }
{p_end}
...
{pstd}
{cmd:heckman_scanrho} saves the following in {cmd:e()}:
...
{synopt:{cmd:e(init_values)}}a vector to pass as initial values to {cmd:heckman}{p_end}
