*! version 5.12.9 01Apr2024 // official Stata version of -glm- modified by Ariel Linden
							// to allow for repeated dates in multiple panel data
							// and allow for aweights in time-series data 

*! version 5.12.8  28oct2021
program glm2, eclass byable(onecall) prop(ml_score swml svyb svyj svyr mi bayes)
...
/* Parse. */
	...
	syntax varlist(numeric fv ts) [aw]	/// weight only relevant for newey-west (Ariel) 
		[if] [in] [,				///
		EForm					///
		FROM(string)				///
		LEvel(cilevel)				///
		...
		DISP(real 1)				///
		VFactor(real 1)				///
		FORCE						/// not documented (Ariel's addition)
        	EXPosure(varname numeric)		///
        	moptobj(passthru)			/// not documented
		*					///
	]

	local FROM_ORIG : copy local from
...
************** force allows use of "hac for multiple group ITSA ******
	if "`force'" == "" {
...
**************** force suppresses error messages for weights ********************************************	
	if "`force'" == "" {
		if "`robust'" != "" {
...
capture noisily break {
	...
	est local m	   "$SGLM_m"
	if !inlist("$SGLM_m", "", "1") {
		est hidden local marginsprop atslow
	}
