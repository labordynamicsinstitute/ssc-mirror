*! version 1.2.1    20jun2000      C F Baum/VWiggins   stb distribution
*  rev 1.1.1 0421 correction to pval of asy z
program define gphudak6, eclass
	version 6
	...
	while "``i''" != "" {
		...
		else    di in blue "  gphudak could not be calculated "   /*
			*/ " for power = ``i''"
		local i = `i' + 1
	}
	di _dup(78) in gr "-"
...
program define GPHEst1, rclass
	...
	quietly {
		...
		gen double `lsin' = log( 4.0 * sin(_pi*(`n')/r(N))^2 ) /*
			*/ if `touse' 
