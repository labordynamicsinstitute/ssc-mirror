...
program entropyetc2, rclass    
	...
	quietly { 
		...
		forval i = 1/`ng' { 
			...
			mat `matname'[`i',2] = exp(`results'[1,1])
			mat `matname'[`i',3] = `results'[1,2]
		...
		Shannon exp_H Simpson rec_R dissim 
		...
		capture label var `group' "`=uchar(160)'" 
		if _rc label var `group' "Group" 
