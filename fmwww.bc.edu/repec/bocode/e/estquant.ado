...
** 
** [Reference]
** Kondo, K. (2017) "Quantile approach for distinguishing agglomeration from 
** firm selection in Stata," RIETI TP 17-T-001. 
** GitHub: https://github.com/keisukekondokk/estquant
** 
...
** URL: https://doi.org/10.3982/ECTA8442
** 
** [Contact]
...
** Added OPTECHnique() option 
** Added OPNMSimplexdeltas() option 
** Version: 1.10
** Added INITR() option 
** Version: 1.04
** Improved program code
...
class quantilef {
	...
	string scalar optechnique
	real scalar opnmsdeltas
...
real scalar quantilef::init(class quantilef Q)
{
	...
	if(optechnique == "nr" | optechnique == "dfp" | optechnique == "bfgs"){
		optimize_init_technique(Sb, optechnique)
		optimize_init_singularHmethod(Sb, "hybrid")
	}
	else if(optechnique == "nm") {
		optimize_init_technique(Sb, optechnique)
		optimize_init_nmsimplexdeltas(Sb, opnmsdeltas)
	}
...
program define estquant, sortpreserve eclass
	...
	if( `bstrata' == 1 & "`bvar'" == "on" ){
		display as text "note: strata option is ignored in the case of bvariable(on)."
	}

	/* Bootstrap Option: Replication and Sampling */
	local nbrep = `breplication'
	scalar nbsample = `bsampling'
	
	/* Bootstrap Option: Error Check for bvar option */
	if( "`bvar'" == "on" ){
	...
	else if( "`optechnique'" == "nr" | "`optechnique'" == "dfp" | "`optechnique'" == "bfgs" | "`optechnique'" == "nm" ){
		local optech = "`optechnique'"
		if( "`optechnique'" == "nm" ){
			display as text "Value of delta to built the simplex required by the Nelder–Mead method (`opnmsimplexdeltas')"
		}
	}
	else {
		display as error "optechnique() requires either nr, dfp, bfgs, or nm."
		exit 198
	}

	/* Optimization Option for the NM Method */
	scalar opnmsdeltas = `opnmsimplexdeltas'

	/* Convegence Criteria in Optimization */
	scalar maxIt = `maxiteration'
	scalar eps1 = `eps1'
	scalar eps2 = `eps2'
	
	/* Display Results of Each Iteration: 0: OFF, 1: ON: */
	local disp = 0
	
	/* Bootstrap Replications */
	if( `nbrep' == 0 ){
		display as text "Bootstrap replications skipped."
		...
		matrix colnames mP = A D S
		display as text "Bootstrap replications (`nbrep')
		if( `trnc' == 1 | `shft' == 1 | `dltn' == 1 ){
			/* Bootstrap Case 1: bvariable option off */
			if( "`bvar'" == "off" ){
				forvalues i = 1(1)`nbrep' {
					preserve
					local bcnt = 0
					local bconv = 0
					/* BOOTSTRAP WITH ERROR CHECK */
					while `bconv' == 0 {
						if( `bstrata' == 0 ){
						...
						/* scalar: 0: not converged, 1: converged */
						local bconv = converged
						local bcnt = `bcnt' + 1
						if( `bcnt' > 50 ){
							/* If convergence not achieved */
							disp as error "{c |}Convergence Err{c |}"
							matrix vp = J(1, 3, .)
						}
					}
					matrix vP = vp
			...
			/* Bootstrap Case 2: bvariable option on */
			if( "`bvar'" == "on" ){
				forvalues i = 1(1)`nbrep' {
					scalar nbsample = .
					/* ESTIMATION FOR BOOTSTRAP */
	...
	/* Display Main Results: 0: OFF, 1: ON: */
	local disp = 1
...
void estquant(vY, category, trnc, shft, dltn, initrnc, touse, bname, Vname, mP, ci, disp)
{
	...
	opnmsdeltas = st_numscalar("opnmsdeltas")
	maxIt = st_numscalar("maxIt")
	eps1 = st_numscalar("eps1")
	eps2 = st_numscalar("eps2")
	optechnique = st_local("optechnique")
	...
	if( Q.truncation == 1 ){
		...
		/* QUESTION: Why each result is different in nr algorithm? */
		/* Find initial values for the NR method */
		/* Repeat this step if the result is unsteady */
		if( optechnique == "nr" ){
			Sg = optimize_init()
			optimize_init_which(Sg, "min")
			optimize_init_evaluator(Sg, &eval_critbisf())
			optimize_init_evaluatortype(Sg, "d0")
			optimize_init_technique(Sg, "nm")
			optimize_init_nmsimplexdeltas(Sg, 0.1)
			...
			pgamm0 = optimize(Sg)
			voirs = optimize_result_value(Sg)
		...
		/* Optimization wrt gamma and beta */
		{
			...
			optimize_init_evaluatortype(Sg, "d0")
			if( optechnique == "nr" | optechnique == "dfp" | optechnique == "bfgs" ){
				optimize_init_technique(Sg, optechnique)
				optimize_init_singularHmethod(Sg, "hybrid")
			}
			else if( optechnique == "nm" ) {
				optimize_init_technique(Sg, optechnique)
				optimize_init_nmsimplexdeltas(Sg, opnmsdeltas)
			}
			optimize_init_argument(Sg, 1, Q)
			optimize_init_params(Sg, pgamm0)
			optimize_init_tracelevel(Sg, "none")
	...
	*/
	...
	if( disp == 1 ){
		/* Summary Statistics */
		printf("\n\n")
		printf("{txt}%16s {c |}{space 5}Obs. {space 6}Mean {space 6}S.D. {space 7} Number of obs = {res}%8.0f \n", scategory, Q.nobs)
		printf("{hline 17}{c +}{hline 31} {txt}{space 7} Replications  = {res}%8.0f \n", nbrep)
		printf("{txt}%16.0f {c |}{res} %8.0f %11.4g %9.4g {space 8}{txt}BS Sample (%%) = {res}%8.4g \n", cref1, Q.nobs_c1, Q.mean_y1, Q.sd_y1, nbsample)
		printf("{txt}%16.0f {c |}{res} %8.0f %11.4g %9.4g {space 8}{txt}Pseudo R2     = {res}%8.4f \n", cref2, Q.nobs_c2, Q.mean_y2, Q.sd_y2, r2)
		printf("{hline 17}{c +}{hline 31} \n")
		printf("{txt}{space 11}Total {c |}{res} %8.0f %11.4g %9.4g \n", Q.nobs, Q.mean_y, Q.sd_y)
		printf("\n")
		printf("{hline 17}{c TT}{hline 64} \n")
		if( ci == "bootstrap" ){
			printf("{txt}{space 16} {c |}{space 3}Observed{space 3}Bootstrap{space 23}Bootstrap-based \n")
		}
		else if( ci == "normal" ){
			printf("{txt}{space 16} {c |}{space 3}Observed{space 3}Bootstrap{space 25}Normal-based \n")
		}
		printf("{txt}%16s {c |}{space 6}Coef.{space 3}Std. Err.{space 6}z{space 4}P>|z|{space 3}[%3.1f%% Conf. Interval]\n", sY, cilevel)
		printf("{hline 17}{c +}{hline 64} \n")
		if( Q.shift == 1 & Q.dilation == 1 & Q.truncation == 1 ){
			printf("{txt}%16s {c |}{res}%11.7g %10.5g   %6.2f %7.3f %12.6g %11.6g\n", 
			...
			printf("{txt}%16s {c |}{res}%11.7g %10.5g   %6.2f %7.3f %12.7g %11.7g\n", 
				sPName[1], mStat[1,1],.,.,.,.,. )
			printf("{txt}%16s {c |}{res}%11.7g %10.5g   %6.2f %7.3f %12.7g %11.7g\n", 
				sPName[2], mStat[2,1], mStat[2,2], mStat[2,3], mStat[2,4], mStat[2,5], mStat[2,6] )
			printf("{txt}%16s {c |}{res}%11.7g %10.5g   %6.2f %7.3f %12.7g %11.7g\n", 
