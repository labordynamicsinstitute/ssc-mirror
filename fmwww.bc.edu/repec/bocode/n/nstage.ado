*! version 5.0.0 BO/PR/IW 04oct2021.
program define nstage, rclass
version 10.0

syntax , Nstage(int) ACcrue(numlist integer >1) ALpha(numlist >0 <1) ARms(numlist integer >1) HR0(string) HR1(string) Omega(numlist >0 <1) ///
 [ ARAtio(string) corr(real 0.6) noIterate INCludedroppedarms ///
 PRobs S(string) T(string) TStop(real 0) TOl(real 0.005) TUnit(int 1) SIMcorr(string) seed(int -1) ///
 OUTfile(string) NOFwer fwerreps(int 250000) esb(string) NONBINDing FWERcontrol(string) OLD ]		

if `fwerreps'<1E5 {	
	di as err "fwerreps() should be at least 100000"
	exit 198
}
local old = ("`old'" == "old")
/*
...
global title1 "n-stage trial design                   version 5.0.0, 04 Oct 2021"	
global title2 "based on Royston et al. (2011) Trials 12:81 and Blenkinsop et al."
global title3 "(2019) Clinical Trials 16(2)"
local hline = length("$title1")
/*
	Calculate sample size required
*/
samplesize, nstage(`nstage') accrue(`accrue') alpha(`alpha') arms(`arms') ///
 hr0(`hr0') hr1(`hr1') omega(`omega') aratio(`aratio') s(`s') t(`t') ///
 tstop(`tstop') tunit(`tunit') seed(`seed') fwerreps(`fwerreps') `iterate' old(`old')
...
*noi di in red "`lndelS1'"
local deltaS1 = exp(`lndelS1')
return scalar deltaS1 = `deltaS1'
return scalar lndelS1 = `lndelS1'
...
if "`esb'" != "" & `have_D' & "`simcorr'"=="" {
/*
	Add condition that forces simulated correlation if esb specified
	when I!=D, to ensure correlation is based on D-D outcomes.
*/
	local simcorr 250
	local corr 0.6
}
if "`simcorr'" != "" & `have_D' {
	di as txt _n "Simulations are used to estimate the correlation structure."
	di as txt "Depending on the number of replicates, specified by {cmd:simcorr(`simcorr')},"
	di as text "the results might take some minutes to appear. Progress is shown below."
}
if `have_D' di as text _n "Correlation assumed between hazard ratios on I and D: corr() = `corr'"
local sampl
local events
...
if `nstage' == 2 {
	...
	if `have_D' {
		if "`simcorr'" != "" {
			hrcorrnstage, accrue(`accrue') hr0(`hr0') hr1(`hr1') ///
			 n(`sampl') e(`events') t(`t') aratio(`Rstar') ///
			 nstage(`nstage') rep(`simcorr') rho(`corr') seed(`seed') ///
			 savehr(`outfile')
			forvalues k = 0/1 {	
				...
				if "`esb'" != ""{
					...
					forvalues j = 0/1 {
						forvalues m = 1 / `nstage' {
							local d`m'`j' = r(d`m'`j')
							if `j'==0 {
								// Store D-events at interim stages when I!=D
								return scalar D`m' = `d`m'`j''
							}
						}
					}
				}
	...
	matrix `RH1' = I(`nstage')
	if (`have_D') & ("`simcorr'" != "") {
		hrcorrnstage, accrue(`accrue') hr0(`hr0') hr1(`hr1') n(`sampl') ///
		 e(`events') t(`t') aratio(`Rstar') nstage(`nstage') rep(`simcorr') ///
		 rho(`corr') seed(`seed') savehr(`outfile')		
	}
	local ns1 = `nstage' - 1
	forvalues i = 1 / `ns1' {
		local i1 = `i' + 1
...
*/
if !`have_D' & "`fwercontrol'"!=""{
	// If I=D and FWER control specified, assume non-binding boundaries
...
*******************************************************************************/
		if "`fwercontrol'" !=""{
			...
			di as text "        "
			di as text _n "Searching for design which controls the FWER at `fwercontrolpct'% ..."
			if `have_D' local ineqd ineqd
...
if "`esb'" != "" di as text "LOB = lack of benefit; ESB = efficacy stopping boundary"
else di as text "LOB = lack of benefit"
...
di as text "     Time is expressed in cumulative periods."

if "`fwercontrol'" != "" {
	di as text _n "{bf:Note}: the value of alpha specified at the final stage has been"
	di as text "replaced with the value required to control the FWER at `fwercontrol'."
}
...
noi di as txt _n "NSTAGE has completed successfully."
...
forvalues j = 1 / `nstage' {
	local j1 = `j' - 1
	local d`j' = `t`j'' - `t`j1''
*noi di in red "j=`j' tj=`t`j'' tj1=`t`j1'' dj=`d`j''"
/*
	if `d`j'' < 0 {
...
/*
	...
	It uses survival times simulated in Stage 1 but cuts off after a 
	higher number of events and runs survival analysis need to add
	extra patients accrued after Stage 1.
*/
	if `intstage'>1	{
		forvalues k = 2 / `intstage' {
			...
			if "`intoutcome'"=="" | "`intoutcome'"=="I" {
				...
				mkbilogn `la40' `la50',r(`rho') // la40 and la50 are correlated lognormals
				// The following commands generate PS and OS under H0 and H1
				forval i = 4/5{
...
version 8
/*
	Calculates hazard for minimum of 2 correlated exponentials with
	correlation rho in original bivariate normals.
*/
...
version 8
/*
	Evaluates lambda (hazard of min of 2 correlated exponentials)
	for given l1, l2, rho, assuming nomedian.
*/
...
quietly {
	gen `c1' = exp(invnorm(uniform()))
	gen `c2' = exp(invnorm(uniform()))
...
/*
	Note: mean,variances,corr refer to mean,variances,corr
	of the log of the variables.
...
di in smcl in gr "{hline `len'}{c +}{hline 25}" 
local pad = `len' - 5
di in smcl in gr `"Total"' _skip(`pad') `"{c |}"' in ye %10.0g `sum' `"     "' %10.2f `sum'
if `"`strata'"' != `""' {
	di in gr _n `"(*) sum over calculations within `strata'"'	
}

local pad = `len' + 7
return scalar df = colsof(`w') - 1
return scalar chi2 = `V'[1,1]
...
* DB v0.09
program define nstagefwer, rclass
version 10
...
syntax, Nstage(int) ACcrue(string) ALpha(string) ARms(string) HR0(string) HR1(string) ///
	 Omega(string) [ARAtio(string) S(string) T(string) TStop(real 0) TUnit(int 1) seed(int -1) ///
	 fwerreps(int 1000000) noIterate old(int 0)]
if "`t'"=="" {		
...
if "`s'" == "" {
	local s = cond(`nstage'==1, "0.5", "0.5 0.5")
}
	...
	if `nstage' > 1 {
		...
		if c(rc) {
			...
			if "`opt'" == "t" {
				...
				else local hazard`nstage' = -ln(`surv') / ``opt'`stage''
			}
			if `nstage' > 2 {
				local ns1 = `nstage' - 1
				forvalues j = 2 / `ns1' {
...
// Error checking of arms() improved, PR 23sep2021.
forvalues i = 1 / `nstage' {
	capture confirm integer number `armsS`i''
	if _rc {
		local ++errcount
		local error`errcount' "invalid number of arms at stage `i'"
		*exit 198
	}
	else if `i' > 1 {
		local im1 = `i'-1
		if `armsS`i'' > `armsS`im1''{
			local ++errcount
			local error`errcount' "number of arms in stage `i' must not exceed number in stage `im1'"
		}
	}
}
if `errcount' > 0 {
	...
	if "`iterate'" == "noiterate" {
		* Calcs done under H0, no iteration necessary
		if `Rstar' == 1 {
			...
			if !`old' local lndelS1 = ln(`hr0S1') + `sign1' * `zalphaS1' * sqrt(1 / `eS1' + 1 / `eS1star')  /* This is added by Babak following Ian's suggestion on 24 March 2021 */
		...
		* Compute I-events at end of stage 1 in experimental arm under H1.
		...
		if "`olddef'" != "" {
			...
			// This was added by Babak following Ian's suggestion on 24 March 2021
			if !`old' local lndelS1 = ln(`hr0S1') + `sign1' * `zalphaS1' * sqrt(1 / `eS1' + 1 / `eS1star')
			local zomega = `sign1' * (`lndelS1' - ln(`hr1S1')) / sqrt(1 / `eS1' + 1 / `eS1star')				
		}
		* Update eS1
				...
				* Calcs done under H0, no iteration necessary
				if `Rstar' == 1 {
					...
					if "`olddef'"!="" {
						...
						// This was added by Babak following Ian's suggestion on 24 March 2021
						if !`old' local lndelS`m' = ln(`hr0S`m'') + `sign`m'' * `zalphaS`m'' * sqrt(1 / `eS`m'' + 1 / `eS`m'star')
						local zomega`m' = `sign`m'' * (`lndelS`m'' - ln(`hr1S`m'')) / sqrt(1 / `eS`m'' + 1 / `eS`m'star')	
					}
				}
				local done 1
...
Notes and history
-----------------
* renamed to nstage_beta and minor tidying IW 15feb2022
* nstageipr PR 21sep2021 based on nstagei IW fix 24mar2021 	
* nstagei IW fix 24mar2021 	
