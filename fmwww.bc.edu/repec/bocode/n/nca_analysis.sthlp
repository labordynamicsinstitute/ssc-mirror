...
{title:Title}
{phang}
{bf:nca_analysis} {hline 2} Necessary Condition Analysis (NCA)
...
{synopt:{opt nograph}}  Prevents {cmdab:nca_analysis} from showing the graph. {p_end}
{synopt:{opt nosumm:aries}}  Prevents {cmdab:nca_analysis} from displaying the summaries. {p_end}
...
{phang2} {bf: ce_fdh} requests {opt nca_analysis} to estimate a Ceiling Envelopment with Free Disposal Hull (default) {p_end}
{phang2} {bf: cr_fdh} requests {opt nca_analysis} to estimate a Ceiling Regression with Free Disposal Hull (default) {p_end}
{phang2} {bf: ce_vrs} requests {opt nca_analysis} to estimate a Ceiling Envelopment with Variable Returns to Scale {p_end}
{phang2} {bf: cr_vrs} requests {opt nca_analysis} to estimate a Ceiling Regression with Variable Returns to Scale {p_end}
{phang}
...
{phang}
{opt flipx}  forces {opt nca_analysis} to reverse the sign of all the conditions  {p_end}
{phang}
{opt flipy}  forces {opt nca_analysis} to reverse the sign of the outcome  {p_end}
{phang}
{opt cor:ner(numlist)}  an integer constant or a vector of integers indicating the corners to analyze, this option is equivalent to the combinations of {opt flipx} and {opt flipy}. If a single integer is specified by the user {opt nca_analysis} estimates the same corner for all the conditions. Users can specify a different corner for each condition. The default is 1 and means that neither the outcome nor the conditions are reversed in sign. {opt cor:ner(2)} is equivalent to {opt flipx}, {opt cor:ner(3)} is equivalent to {opt flipy} and {opt cor:ner(4)} is equivalent to {opt flipx flipy}.
{p_end}
{dlgtab:Bottlenecks table}
{phang}
{opt bottlenecks}[{bf:(}{it:numlist}{bf:)}]  the values of the {it: outcome} to be used in the bottleneck table. numlist is optional and, when it is not specified, the values of the outcome to be displayed in the bottleneck table are controlled by {opt steps} and {opt stepsize}. If numlist is specified {cmdab:nca_analysis} overrides {opt steps} and {opt stepsize}  {p_end}
{phang}
...
{dlgtab:Graph}
{phang}
{opt nograph}  Prevents {cmdab:nca_analysis} from showing the graph. Estimation is considerably faster.  {p_end}
{phang}
...
{dlgtab:Display}
{phang}
{opt nosum:maries}  Prevents {cmd:nca_analysis} from displaying the ceiling summaries, this is useful for the users who are only interested in the bottlenecks table.  {p_end}
...
{phang2}{cmd:. nca_analysis individualism innovationperformance, ceilings( ce_vrs cr_vrs ce_fdh cr_fdh) bottlenecks}{p_end}
...
{phang2}{cmd:. nca_analysis individualism risktaking  innovationperformance , corner(4) bottlenecks(10 20(7.5)50) xbottlenecks(percentile)}{p_end}
...
{phang2}{cmd:. nca_analysis individualism innovationperformance, ceilings( ce_fdh) testrep(1000) nograph}{p_end}
...
{synopt:{cmd:e(cmd)}} {cmd:nca_analysis}{p_end}
...
{pstd}We are grateful to Ricardo Ernesto Buitrago, Marno Verbeek and Caroline Witte for their beta testing. {p_end}
