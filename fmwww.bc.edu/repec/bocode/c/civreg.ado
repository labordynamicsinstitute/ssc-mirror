...
// end replay
		...
		syntax anything(name = 0) [if] [in] , [ /*
			...
			*/	TOLerance(real 1e-8)			/*
			*/	NOLOG							/*
			*/	NOCONStant FIRST FFIRST 		/*
			*/	NOFOoter NOOUTput				/*
			*/	EForm(string) PLUS				/*
			*/	Level(integer $S_level)			/*
			*/	liml gmm2s gmmw					/*	// to be partialed out
			*/	*								/*
			*/	]
...
di as error /*
	*/	"time variable not set; use {cmd:xtset} {it:panelvar timevar}"
		...
		//	Check whether y_it is time & group-variant	
...
// group-demean
		...
		qui mata: make_civ("endo_rmc", "lhs", "inexog_rmc", "touse", /*
...
/*			ereturn display , level(`level') `plus' `first' `ffirst' /*
				*/	`nofooter' `nooutput' eform(`eform')
...
real colvector civ_maker(real colvector y,
                     real colvector x,
                     real scalar d_or,
					 real scalar delt,
					 real scalar dmax,
...
// ---------------------------------------------
				...
				if (k_x==1) {
					...
					if (r_code == 1) {
						...
						civ[,i] = civi + V012[., hete+1]
					}
					else {
						
						civ[,i] = civi + rnormal(N, 1, 0, sd_x)
					}
