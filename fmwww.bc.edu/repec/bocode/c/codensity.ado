*! codensity 1.0.0 NJC 16 April 2025
* multidensity 1.0.0 NJC 9 July 2020
program codensity
	version 8 
	...
	if length("`cmd'") == 1 & "`cmd'" == "s" { 
		di as err "super or stack?"
		exit 498 
	}
	...
	else if "`cmd'" == substr("stack", 1, length("`cmd'")) {
		_stack `0'
		exit 0 
	}
	else if "`cmd'" == "clear" { 
...
program _generate
	...
	labelwith(str) densitylabel(str) showchar 

	capture syntax varlist(numeric max=1) [if] [in] [fweight aweight], ///
	over(varname) [ MISSing `opts'] 
	...
	quietly if _rc == 0 { 
		...
		tempname stub 
		separate `varlist' if `touse', generate(`stub') by(`over') veryshortlabel 
	...
	foreach v of local varlist { 
		...
		local ktext "`r(kernel)'"
		if inlist(substr("`ktext'", 1, 1), "e", "g", "p") { 
			local ktext = proper("`ktext'") 
		} 
		char `fstub'`j'[kernel] "`ktext'"

		char `fstub'`j'[bwidth] "`r(width)'"  
		
		if "`showchar'" != "" {
		    local toshow`j' `fstub'`j' `: char `fstub'`j'[kernel]' `: char `fstub'`j'[bwidth]'
		}

		if "`trans'" != "" { 
			local T : subinstr local T "_" " ", all 
			char `fstub'`j'[trans] "`T'" 
			if "`showchar'" != "" local toshow`j' `toshow`j'' `: char `fstub'`j'[trans]'
		}
		
		// variable labels are 
		// 	variable labels of original variables 
		//      otherwise variable names if they do not exist 
		// 	otherwise group values if used -over()- option 
...
program _juxta 
	...
	if `nf' == 0 & `nx' == 0 { 
		di as err "no variables found; run codensity generate first?" 	
		exit 498 
	} 
	else if `nf' != `nx' { 
...
program _stack 
	...
	syntax [, fstub(str) xstub(str) recast(str) Height(real 0.8) VERTical `opts' ///
	addplot(str asis) * ] 
	...
	// find maximum density to rescale 
	local max = 0 
    forval j = 1/`nf' { 
		su `fstub'`j', meanonly 
	...
	if "`vertical'" != "" {
		...
		// recast(rarea) the most obvious alternative 
		local plot = cond("`recast'" != "", "`recast'", "rline") 
		
		quietly forval j = 1/`nf' { 
			tempvar show`j'
			gen `show`j'' = `j' + `height' * `fstub'`j'/`max'
			
			if "`plot'" == "rarea" { 
				tempvar base`j' 
				gen `base`j'' = `j'
				local call `call' || rarea `show`j'' `base`j'' `xstub'`j' if `fstub'`j' > 0, horizontal `opt`j''
			} 
			else local call `call' || `plot' `show`j'' `show`j'' `xstub'`j' if `fstub'`j' > 0, horizontal `opt`j''
			
			local label : var label `xstub'`j'
			local where = `j' + `height'/2 
			local labels `labels' `where' `"`label'"'
		}	

		twoway `call' ///
		legend(off) xtitle("") xla(`labels', noticks) `options' ///
		|| `addplot'
		...
		quietly forval j = 1/`nf' { 
			tempvar show`j'
			gen `show`j'' = `j' + `height' * `fstub'`j'/`max'
			if "`plot'" == "area" local base base(`j')
			local call `call' || `plot' `show`j'' `xstub'`j' if `fstub'`j' > 0, `base' `opt`j''
			local label : var label `xstub'`j'
			local where = `j' + `height'/2 
			local labels `labels' `where' `"`label'"'
		}	
		...
		twoway `call' ///
		legend(off) ytitle("") yla(`labels', noticks) `options' ///
		|| `addplot'
