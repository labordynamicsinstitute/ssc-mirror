* weakiv cert script MS 26mar2014
...
* (d) Vs. original rivtest (basic VCV variants only)
...
* Starting with version 01.0.08:
* (h) LM AR stat by hand
* (i) AR, K, J by hand in Mata, MD versions, linear model only
* (j) AR, K, J by hand in Mata, LM versions, linear model only
* (k) usage with xtivreg2, fe and fd
...
foreach opt in	" " "rob" "cluster(age)" "cluster(age exper)"	///
	...
	foreach small in " " "small"	{
		foreach null of numlist 0 0.1 {
			...
			qui weakiv ivreg2 lwage exper expersq (educ = fatheduc motheduc), `opt' `small' null(`null') lm
...
foreach opt in " " "rob" "cluster(id)"	{
		foreach null of numlist 0 0.1 {
			...
			qui weakiv ivreg2 lwage exper expersq id_* (educ = fatheduc motheduc), `opt' null(`null') partial(id_*) nocons dofminus(`dofminus')
			savedresults save wivreg2 e()
			qui weakiv xtivreg2 lwage exper expersq (educ = fatheduc motheduc), `opt' null(`null') fe
...
foreach opt in " " "rob" "cluster(id)"	{
		foreach null of numlist 0 0.1 {
			di "opt=`opt' null=`null'"
			qui weakiv ivreg2 D.lwage D.exper D.expersq (D.educ = D.fatheduc D.motheduc), `opt' null(`null')
			savedresults save wivreg2 e()
			qui weakiv xtivreg2 lwage exper expersq (educ = fatheduc motheduc), `opt' null(`null') fd
			savedresults comp wivreg2 e(), tol(1e-7) exclude(macros: xtmodel scalars: N_g singleton)
	}
}
...
* Also checks small option
		foreach null of numlist 0 0.1 {
			di "opt=`opt' null=`null'"
			qui weakiv ivreg2 lwage exper expersq id_* (educ = fatheduc motheduc), null(`null') partial(id_* exper expersq) nocons small
			savedresults save wivreg2 e()
			qui weakiv xtivreg2 lwage exper expersq (educ = fatheduc motheduc), null(`null') fe small
			savedresults comp wivreg2 e(), tol(1e-7) exclude(macros: xtmodel scalars: N_g singleton)
			qui weakiv xtivreg lwage exper expersq (educ = fatheduc motheduc), null(`null') fe small
			savedresults comp wivreg2 e(), tol(1e-7) exclude(macros: xtmodel scalars: N_g singleton)
	}
* First differences
		foreach null of numlist 0 0.1 {
			di "opt=`opt' null=`null'"
			qui weakiv ivreg2 D.lwage D.exper D.expersq (D.educ = D.fatheduc D.motheduc), `opt' null(`null') small
			savedresults save wivreg2 e()
			qui weakiv xtivreg2 lwage exper expersq (educ = fatheduc motheduc), `opt' null(`null') fd small
			savedresults comp wivreg2 e(), tol(1e-7) exclude(macros: xtmodel scalars: N_g singleton)
			qui weakiv xtivreg lwage exper expersq (educ = fatheduc motheduc), `opt' null(`null') fd small
			savedresults comp wivreg2 e(), tol(1e-7) exclude(macros: xtmodel scalars: N_g singleton)
	}
...
foreach opt in	" " "rob" "cluster(age)" "cluster(age exper)"	///
	...
	global b2=_b[educ]
	qui weakiv ivreg2 lwage exper expersq (hours educ = fatheduc motheduc hushrs), null1(0.01) null2($b2) `opt'
	scalar ar_chi2=e(ar_chi2)
	scalar k_chi2=e(k_chi2)
	scalar j_chi2=e(j_chi2)
	qui weakiv ivreg2 lwage exper expersq (hours educ = fatheduc motheduc hushrs), null(0.01) strong(educ) `opt'
	assert reldif(wald_chi2,e(wald_chi2))< 1e-7
	assert reldif(ar_chi2,e(ar_chi2))< 1e-7
	assert reldif(k_chi2,e(k_chi2))< 1e-7
...
qui weakiv ivreg2 $y $X2 ($X=$Z), partial($X2) null($null) lm
...
assert reldif(J,e(j_chi2))< 1e-6
* LM with partialling-out (required)
qui weakiv ivreg2 $y $X2 ($X=$Z), partial($X2) null($null) lm rob
assert reldif(ARLM,e(ar_chi2))< 1e-6
assert reldif(KLM,e(k_chi2))< 1e-6
assert reldif(JLM,e(j_chi2))< 1e-6
...
* MD (Wald), with/without partialling-out
qui weakiv ivreg2 $y $X2 ($X=$Z), null1($null_1) null2($null_2)
assert reldif(AR,e(ar_chi2))< 1e-6
assert reldif(K,e(k_chi2))< 1e-6
assert reldif(J,e(j_chi2))< 1e-6
qui weakiv ivreg2 $y $X2 ($X=$Z), partial($X2) null1($null_1) null2($null_2)
assert reldif(AR,e(ar_chi2))< 1e-6
assert reldif(K,e(k_chi2))< 1e-6
assert reldif(J,e(j_chi2))< 1e-6
* LM with partialling-out (required)
qui weakiv ivreg2 $y $X2 ($X=$Z), partial($X2) null1($null_1) null2($null_2) lm
assert reldif(ARLM,e(ar_chi2))< 1e-6
assert reldif(KLM,e(k_chi2))< 1e-6
assert reldif(JLM,e(j_chi2))< 1e-6
...
* MD (Wald), with/without partialling-out
qui weakiv ivreg2 $y $X2 ($X=$Z), null1($null_1) null2($null_2) rob
assert reldif(AR,e(ar_chi2))< 1e-6
assert reldif(K,e(k_chi2))< 1e-6
assert reldif(J,e(j_chi2))< 1e-6
qui weakiv ivreg2 $y $X2 ($X=$Z), partial($X2) null1($null_1) null2($null_2) rob
assert reldif(AR,e(ar_chi2))< 1e-6
assert reldif(K,e(k_chi2))< 1e-6
assert reldif(J,e(j_chi2))< 1e-6
* LM with partialling-out (required)
qui weakiv ivreg2 $y $X2 ($X=$Z), partial($X2) null1($null_1) null2($null_2) lm rob
assert reldif(ARLM,e(ar_chi2))< 1e-6
assert reldif(KLM,e(k_chi2))< 1e-6
assert reldif(JLM,e(j_chi2))< 1e-6
