...
out_prediction(name) seed(numlist max=1 integer) n_folds(numlist max=1 integer) ///
[save_graph_cv(name)]
********************************************************************************
...
di in red "Please, enconde this feature to become numeric.                              "
...
python script "`c(sysdir_plus)'py/c_multinomial.py"
ereturn clear
}
********************************************************************************
* PYTHON CODE - END 
