...
{p 8 17 2}
{cmdab:clocktominb:} var1 var2, epnum(string) diaryid(string) diaryst(n) 

{title:Description}

{pstd} {bf:clocktominb} {hline 2} takes the string variables containing the start and end time of an episode of activity and creates the variables 'start' and 'end', 
...
If only the beginning or the end of the episode of activity is provided in the original dataset, use the programs {cmd:clocktomins} or {cmd:clocktomine} instead. 
...
{phang}
{bf:epnum(string)}: name of the variable containing the episode number. 
It is the variable that indicates the sequence in which the different episodes of activity happen. 
If no such variable exists in the dataset but the episodes of activity are ordered, simply create it like this: "bysort diaryid: gen epnum=_n".
