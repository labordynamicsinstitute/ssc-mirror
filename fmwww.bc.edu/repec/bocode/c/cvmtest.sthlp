...
{pstd}
{cmd:cvmtest} performs a two-sample Cramer-von Mises (CVM) permutation test 
to assess whether two samples come from the same distribution. The CVM statistic 
measures the integrated squared difference between the empirical cumulative 
distribution functions (ECDFs) of the two samples. The permutation test provides 
an exact p-value by comparing the observed statistic to its distribution under 
random reassignment of group labels.
...
the default is {opt power(2)}, which gives the standard Cramer-von 
Mises statistic with quadratic weighting. Other values modify sensitivity:
{opt power(1)} linear weighting (less sensitive to large differences),
{opt power(3+)} which is more sensitive to large distributional differences.
...
{pstd}{opt cvmtest} using defaults{p_end}
{phang2}{cmd:. cvmtest x, by(group)}

{pstd}{opt cvmtest} with specific seed and more permutations{p_end}
{phang2}{cmd:. cvmtest x, by(group) reps(2000) seed(12345)}{p_end}

{pstd}change power to linear weighting{p_end}
{phang2}{cmd:. cvmtest x, by(group) reps(2000) seed(12345) power(1)}{p_end}
...
{synopt:{cmd:r(stat)}}observed CVM statistic{p_end}
