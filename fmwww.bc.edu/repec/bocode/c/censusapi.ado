...
cap program drop censusapi
cap program drop censusapi_range
cap program drop censusapi_saveoption


* Define Programs
program define censusapi, rclass
	version 12
	syntax, [url(string)] [DESTination(string)] [dataset(string) VARiables(string) predicate(string)] [key(string) savekey] [nostatefix]
	...
	* Parse key
	...
	local keyAbbrev = substr("`keyDefined'", 1, 4) + "x"*(length("`keyDefined'")-4)
	if "`keyDefined'" == "${censusapi_key}" & "`keyDefined'" != "`key'" di as result _col(3) "Using stored key: " _newline as text _col(6) "`keyAbbrev'"
	di as result _col(3) `"curl command:"' _newline as text `"!curl "`url'`keyAbbrev'" -o "`destination'" "'
	di as result _col(3) "Note: " _newline as text _col(6) "Key was abbreviated for security reasons"
	...
	** Open file
	di as result _col(3) _newline "Opening file"
	...
	* Option saving (to profile.do)
	...
	return local requestUrl = `"`url'`keyText'"'
	return local curlCommand = `"!curl "`url'`keyText'" -o "`destination'""'
end
