*! version 1.0.0  06Nov2023 Ariel Linden

/* 		-centile2- is a modification of official Stata's -centile- command,
		allowing the user to choose between 6 different definitions
		for computing the given quantile as described in: 
		Hyndman, R. J. and Fan, Y. (1996) Sample quantiles in statistical 
		packages, American Statistician 50, 361--365.
					
		-centile2- implements the continuous sample quantile types 4 through 9,
		where type 6 is the definition used in -centile-, and type 7 is the
		default definition used in R's quantile function. Hyndman and Fan (1996) 
		recommended using type 8.
*/ 
...
program define centile2, rclass byable(recall) sort
		...
		* make sure that the chosen type is valid
		if !inlist(`type',4,5,6,7,8,9) {
			di as err "type(#) must be an integer between 4 and 9
			exit 198
		}
...
*/
				...
				***********************************
				* set alpha and beta for algorithms
				*********************************** 
				if `type' == 4 {
					local a = 0
					local b = 1
