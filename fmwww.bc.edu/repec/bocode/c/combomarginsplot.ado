...
oprobit rep78 i.foreign weight mpg price  
...
combomarginsplot file1 file2 file3, savefile(file123, replace) ///
	labels("Full model" "Restricted model" "Gear Model") ///
	noci ///
	lplot1opts(mfcolor(white)) ///
	lplot2opts(mfcolor(yellow)) ///
	lplot3opts(mfcolor(orange)) ///
	file1opts(pstyle(p1)) ///		
	file2(pstyle(p2)) ///
	file3(pstyle(p3)) ///
	file4(pstyle(p4)) ///
...
combomarginsplot file1 file2 file3, labels("Full model" "Restricted model" "Gear Model") noci lplot1(mfcolor(white))

clear *
sysuse auto
gen hirep = rep78>3 if !mi(rep78)

reg mpg i.foreign i.hirep gear_ratio
margins foreign#hirep, saving(m1, replace)

reg mpg i.foreign##i.hirep gear_ratio
margins foreign#hirep, saving(m2, replace)

combomarginsplot m1 m2



*THROWS ERROR (but OK, because margins are calculated for different -at- combinations, so they don't logically fit together
oprob rep78 i.foreign mpg turn weight length
margins foreign, at(weight=(1500(1500)4500) mpg=(10(10)50) ) predict(outcome(3)) saving(file1z, replace)
margins foreign, at(turn=(10(5)20) mpg=(10(10)50) ) predict(outcome(3)) saving(file1a, replace)
combomarginsplot file1z file1a, labels("file1z" "file1a") file1opts(msym(S)) file2opts(msym(O)) saving(file1za)


* Works - different levels for at variables
oprob rep78 i.foreign mpg turn weight length
margins foreign, at(weight=(1500(1500)4500) mpg=(10(20)50) ) predict(outcome(3)) saving(file3a, replace)
margins foreign, at(mpg=(10(20)50) weight=(1500(500)4500) ) predict(outcome(5)) saving(file3b, replace)
combomarginsplot file3a file3b, labels("file3a" "file3b") file1opts(msym(S)) file2opts(msym(O))  by(mpg)

* works
reg price i.foreign##i.hirep
margins foreign hirep, saving(interact)
reg price i.foreign i.hirep
margins foreign hirep, saving(noint, replace)
combomarginsplot noint interact , labels("No Interaction" "Interaction") by(_filenumber)

/*
// NEED TO DEAL
		_dta[over] and _dta[within] are not populated, tho they aren't used by -marginsplot-
*/
