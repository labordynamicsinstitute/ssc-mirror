* weakiv cert script MS 11Feb2015
...
log using cs_weakiv_2403, replace
about
which weakiv
which ivreg2
which avar
...
foreach opt in	" " "rob" "cluster(age)"						///
				...
				{
	foreach small in " " "small" {
		di "opt=`opt' small=`small'"
		qui weakiv ivregress 2sls lwage exper expersq (educ = i.fatheduc), `opt' `small' md
		savedresults save wivfv e()
		cap drop _I*
		qui xi i.fatheduc
		qui weakiv ivregress 2sls lwage exper expersq (educ = _I*), `opt' `small' md
...
foreach opt in	" " "rob" "cluster(age)"						///
		...
		foreach small in " " "small" {
		di "opt=`opt' small=`small'"
		qui weakiv ivregress 2sls lwage exper expersq i.fatheduc (educ = motheduc hushrs), `opt' `small' md
		savedresults save wivfv e()
		cap drop _I*
		qui xi i.fatheduc
		qui weakiv ivregress 2sls lwage exper expersq i.fatheduc (educ = motheduc hushrs), `opt' `small' md
