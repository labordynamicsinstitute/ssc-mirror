*! v2.3 FRA. Adds scale(index|response) option per Wooldridge suggestion
...
program define jwdid_simple, rclass
        syntax [pw], [* post estore(str) esave(str asis) replace over(varname) scale(string) ///
        ...
        ** scale option: response (default, level APE) or index (log-scale / proportionate effect)
        if !inlist("`scale'","","response","index") {
            display as error "scale() must be response or index"
            error 198
        }
        local predict_opt
        if "`scale'"=="index" local predict_opt predict(xb)
        ...
        if "`asis'"=="" {
            qui:margins `weightexp',  subpop(if __etr__==1 & `tosel') at(__tr__=(0 1)) ///
                    ...
                    `options' `predict_opt' post over(`etr')  
        }
        else {
...
program define jwdid_group, rclass
        syntax [pw], [* post estore(str) esave(str asis) replace  OREStriction(passthru) scale(string) ///
                        over2(varname) asis ]
        tempvar aux
...
program define jwdid_attgt, rclass
        syntax [pw], [* post estore(str) esave(str asis) replace  OREStriction(passthru) scale(string) asis ]
        tempvar aux
        qui:bysort `e(gvar)' `e(ivar)':egen `aux'=min(`e(tvar)') if e(sample)
...
program define jwdid_event, rclass
    syntax [pw], [post estore(str) esave(str asis) replace  OREStriction(passthru) scale(string) asis * pretrend ///
