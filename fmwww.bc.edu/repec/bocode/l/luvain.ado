...
program def luvain
	...
	*check python-louvain is present
	...
	if _rc != 0 {
		...
		di as smcl "You need to install the Python module python-louvain: see {help luvain args: help luvain}"
		exit
	}
	python: reset = "yes"
	if "`reset'" == "no"{
...
def luvain(g):
	...
	if draw=="yes":
		...
		#save figure
		plt.savefig(ourpwd+'/luvain.pdf')
		#close figure
		#plt.clf()
		plt.close("all")
		# use OS default app to pop drawing up
		import subprocess
		subprocess.run(['open', 'luvain.pdf'], check=True)
	
	#reindex, will help putting into the frame
	results=results.sort_index()
	results = results.reset_index()
	# reset luvain frame by going through all frames and dropping the one called luvain.
	if reset == "yes":
		nf = frame.getFrameCount()
		for i in range(nf):
			framename = frame.getFrameAt(i)
			if framename == "luvain":
				frh = frame.connect("luvain")
				frh.drop()
		#now create the luvain frame anew
		frame.create("luvain")
