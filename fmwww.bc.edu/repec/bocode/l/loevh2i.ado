...
*!
*! New immediate command (analogous to rioci version 1.0.0 by Daniel Klein)

program define loevh2i, rclass

   version 14.0

   syntax anything(id="integer") [ , noTAB Table Level(cilevel) Pearson Small ]
   ...
   preserve
      ...
      // table (passed through below). If notab is specified, no table is
      // shown at all.
