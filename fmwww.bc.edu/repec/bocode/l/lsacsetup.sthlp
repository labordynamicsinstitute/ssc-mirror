...
{smcl}
{* *! Version 1.2.0 by Francisco Perales 19-April-2016}{...}
{bf:help lsacsetup}
{hline}
...
Users are required to specify the directories in which the original data is located and where the new file will be stored; the variables in the longitudinal LSAC files that are required;
...
only the general release of LSAC and does not handle variables from the PLE, TUD or Medicare data files.
...
{p 0 4}{cmdab:rel:ease}({it:integer}) specifies the release of the LSAC data to be used (e.g. 6).

{p 0 4}{cmdab:hh:file} specifies the subset of variables from the LSAC household files (plegrhhb & plegrhhk) to be used.
The use of stars to denote groups of variables beginning with a common prefix is allowed.
If a variable which does not exist in LSAC is requested, the request will be ignored.

{p 0 4}{cmdab:naplan:file} specifies the subset of variables from the LSAC naplan file (lsacnaplanacara_gr) to be used.
...
{p 2 2}{inp:. lsacsetup, data("D:\Data\LSAC") saving("D:\Mydata\LSAC") filename(LSAC) release(6) var(zhb05c zbf6mth bwpct prel asdqta asdqtb asdqtb csdqtb) hhfile(aweights cdwt) naplanfile(stream y3write) nomid}



{title:References and useful links}
  
{p 4 4} Australian Government Department of Social Services (DSS). (2015). The Longitudinal Study of Australian Children: An Australian Government Initiative. Data User Guide, November 2015. Australian Government Department of Social Services, Canberra
