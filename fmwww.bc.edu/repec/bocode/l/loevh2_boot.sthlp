...
{pstd}
{cmd:loevh2_boot} extends {help loevh2} by providing bootstrap confidence intervals for Loevinger's H
coefficient. While van der Ark et al. (2008) provide asymptotic standard errors for H, bootstrap
resampling offers a complementary approach that may be more robust when sample sizes are small or
asymptotic assumptions are not met.
...
{pstd}
The program first computes Loevinger's H and its asymptotic standard error following van der
Ark et al. (2008), then performs bootstrap resampling with BCa (bias-corrected and accelerated) confidence
intervals. This dual approach allows comparison between asymptotic and bootstrap-based inference for
the scalability coefficient introduced by Mokken (1971). For a pair of items, H > 0.3 suggests
sufficient scalability for Mokken scale analysis.
...
(2) for non-constant variance and and skewness (a = acceleration). This can be particularly
useful when assessing item scalability in small samples or when the data structure deviates
from conditions assumed in the asymptotic theory (normal approximation) that is likely
to happen for H in the interval [-1,1].
...
in the output. The default is {cmd:abbreviate(8)}.
