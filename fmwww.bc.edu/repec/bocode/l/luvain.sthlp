...
{phang}
{cmd: luvain} exploits the Python integration which, Stata introduced starting 
...
{p 8 17 2}
{cmd:. luvain snode tnode weight}

{phang}
The command creates a frame called "luvain" in which it saves two variables: 
...
{phang}
If you run the command again it wipes the frame "luvain" away and starts over 
unless you say: 

{p 8 17 2}
{cmd:. luvain snode tnode weight, reset(no)} 

{phang}
in which case it adds "cluster_2" to the frame "luvain". A third run of: 
...
{phang}
You might also ask it to plot the clusters. It will save "luvain.pdf" in c(pwd)
...
{title:Examples}
{phang}
The command {cmd: luvain_gcd} takes an integer argument n. It then creates all 
...
this weight graph to play around with {cmd: luvain}. 

{phang}
For example, create the gcd weighted graph of 1,...10:

{p 8 17 2}
1.{stata luvain_gcd 10}.

{phang}
Now partition it with{cmd: luvain}:

{p 8 17 2}
2.{stata luvain snode tnode weight},

{phang}
append a second partition to it with resolution .7, plot it:

{p 8 17 2}
3.{stata luvain snode tnode weight, resolution(.7) reset(no) draw(yes) },

{phang}
and check the results:

{p 8 17 2}
4. {stata frame change luvain}
