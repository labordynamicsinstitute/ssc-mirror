...
{synopt:{opt xz:ero}}If this option is specified the graph(s) includes a dashed line at the value time-axis = 0.{p_end}
...
{synopt:{opt legend(string)}}Specifies the desired legend and/or its options in the same way as in any other Stata graph.{p_end}
...
{p 4 8 2}. {stata use AED_INTERESTRATES.dta, clear}{p_end}
...
{p 4 8 2}. {stata lpgraph level cmlt quad quad_cmlt, ti1(Levels) ti2(Cumulative) ti3(Quadratic) ti4(Quadratic & Cumulative) separate lcolor(red) tti(Number of Days) h(12) title(1-Year Treasury IRF, size(*0.9))}{p_end}
