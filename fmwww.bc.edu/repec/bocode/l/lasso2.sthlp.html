...
{p_end}
{synopt:{cmd:fdev}}
minimum fractional change in deviance (R-sq) to stop looping through lambdas (path)
{p_end}
{synopt:{cmd:devmax}}
maximum fraction of explained deviance (R-sq) to stop looping through lambdas (path)
{p_end}
{synopt:{cmd:nodevcrit}}
override criteria to exit path; loop through all lambdas in list
{p_end}
...
{p_end}
{synopt:{cmd:psolver(}{it:string}{cmd:)}}
...
{cmd:lasso2} will warn if collinear variables are dropped when partialling out.
...
{synopt:{cmd:e(df)}}L0 norm ("effective degrees of freedom"){p_end}
...
{synopt:{cmd:e(lambdamat)}}column vector of lambdas used for estimations{p_end}
